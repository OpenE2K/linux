...
#define SAVE_PRINT_APIC

#ifdef SAVE_PRINT_APIC
...
static int io_apic_setup_irq_pin(unsigned int irq, int node,
				 struct io_apic_irq_attr *attr, int recovery);
#else
static int io_apic_setup_irq_pin(unsigned int irq, int node,
...
static int pin_2_irq(int idx, int apic, int pin)
{
	...
	if (test_bit(bus, mp_bus_not_pci) && apic == 0) {
#else
	if (test_bit(bus, mp_bus_not_pci)) {
#endif
...
static int get_domain_to_io_apicid(int domain)
{
	int apicid;
	...
	if (domain < 0 || domain >= MAX_NUMIOLINKS) {
		pr_err("get_domain_to_io_apicid() invalid domain # %d (< 0 or >= max %d)\n",
				domain, MAX_NUMIOLINKS);
		return -1;
	}

	apicid = mp_find_iolink_io_apicid(node, link);
	if (apicid < 0) {
		pr_err("get_domain_to_io_apicid() could not find IO APIC id for node #%d link #%d\n",
				node, link);
		return -1;
	}
...
static int mp_find_srcbus_io_apicid(int busnum, int slot)
{
	int i;
	...
	for (i = 0; i < mp_irq_entries; i++) {
		...
		if (busnum == lbus && PCI_SLOT(mp_irqs[i].srcbusirq) == slot)
...
#if defined CONFIG_E2K || defined CONFIG_E90S
# ifdef CONFIG_IOHUB_DOMAINS
	apicid = get_domain_to_io_apicid(domain);
# else
	apicid = get_bus_to_io_apicid(bus);
# endif

#ifdef __e2k__
	if (IS_MACHINE_E1CP && bus == 0)
#else
	if (0)
#endif
               apicid = mp_find_srcbus_io_apicid(bus, slot);


	if (apicid < 0) {
		pr_warn("Could not find IO APIC domain #%d (node %d, link %d) for bus %d, slot %d\n",
				domain, iohub_domain_to_node(domain),
				iohub_domain_to_link(domain), bus, slot);
		return -1;
	}
	for (ioapic_idx = 0; ioapic_idx < nr_ioapics; ioapic_idx++) {
		if (mpc_ioapic_id(ioapic_idx) == apicid)
			break;
	}
	if (ioapic_idx >= nr_ioapics) {
		pr_warn("Could not find IO APIC domain #%d (node %d, link %d) at MP table\n",
				domain, iohub_domain_to_node(domain),
...
# endif /* CONFIG_IOHUB_DOMAINS */
	...
	if (apicid < 0) {
		printk(KERN_WARNING "Could not find IO APIC domain #%d (node "
			"%d, link %d) for bus %d, IRQ %d\n",
			domain, iohub_domain_to_node(domain),
			iohub_domain_to_link(domain), bus, irq);
		return -1;
	}
	for (apic = 0; apic < nr_ioapics; apic++) {
		apic_printk(APIC_DEBUG, "Local APIC #%d id %d\n",
			apic, mpc_ioapic_id(apic));
		if (mpc_ioapic_id(apic) == apicid)
			break;
	}
	if (apic >= nr_ioapics) {
		printk(KERN_WARNING "Could not find IO APIC domain #%d (node "
			"%d, link %d) at MP table\n",
			domain, iohub_domain_to_node(domain),
			iohub_domain_to_link(domain));
		return (-1);
	}
	apic_printk(APIC_DEBUG, "IO APIC id %d found as #%d\n", apicid, apic);
	for (i = 0; i < mp_irq_entries; i++) {
		int lbus = mp_irqs[i].srcbus;
...
#ifdef CONFIG_RECOVERY
	/* On recovery path the vector is already assigned
	 * and stored in irq_cfg structure for this IRQ.  */
	if (!recovery)
#endif
...
#ifdef CONFIG_RECOVERY
	/* On recovery path the chip and handler are already
	 * registered within the generic framework. */
	if (!recovery)
#endif
...
#ifdef CONFIG_RECOVERY
static void __io_apic_setup_irqs(unsigned int ioapic_idx, int recovery)
#else
		...
		if (idx == -1) {
			...
			if (idx == -1) {
				...
				if ( pin == 1 ) {
					...
					set_io_apic_irq_attr(&attr, ioapic_idx, pin, 1, 0);

					#ifdef CONFIG_RECOVERY
					io_apic_setup_irq_pin(irq, node, &attr, recovery);
					#else
...
#ifdef CONFIG_RECOVERY
static void setup_IO_APIC_irqs(int recovery)
#else
...
#endif
{
	...
	for (ioapic_idx = 0; ioapic_idx < nr_ioapics; ioapic_idx++)
#ifdef CONFIG_RECOVERY
		__io_apic_setup_irqs(ioapic_idx, recovery);
#else
...
/* We don't have RRR even though maxlvt is 3 */
#if 0
	/*
...
#ifdef SAVE_PRINT_APIC
void print_IO_LOCAL_APICS(void)
{
	int saved_show_lapic = show_lapic;
...
#define A2_BUA2		0x454   /* 32/0xffffffff  "ioapic"  Mem Base Address 2
       ...
       for_each_online_node(i) {
		...
		printk(KERN_DEBUG "node %d: NodeId = 0x%08x;"
			"Nodeconfig = 0x%08x\n",
			i, __raw_readl(&nodeid), __raw_readl(nodeconfig));
		printk(KERN_DEBUG "node %d: IOAPICMESSAGEBASE = 0x%08x\n",
			i, __raw_readl(ioapic_message_base));
		printk(KERN_DEBUG "node %d: LAPICMESSAGEBASE = 0x%08x\n",
			i, __raw_readl(lapic_message_base));
	}
...
#endif /* CONFIG_E90S */
       show_lapic = CONFIG_NR_CPUS;
       print_local_APICs(CONFIG_NR_CPUS);
       print_IO_APICs();
       show_lapic = saved_show_lapic;
}
#else
void print_IO_LOCAL_APICS(void)
{
...
static void irq_complete_move(struct irq_cfg *cfg)
{
	...
	__irq_complete_move(cfg, (unsigned int)
			current_thread_info()->pt_regs->interrupt_vector);
#elif defined CONFIG_E90S
	__irq_complete_move(cfg, (unsigned int)
...
EXPORT_SYMBOL(native_ioapic_set_affinity);
...
#ifdef	CONFIG_RECOVERY
void setup_IO_APIC(int recovery)
#else
			    ...
			    struct msi_msg *msg, u8 hpet_id)
{
	...
	pr_debug("MSI interrupt for %s: irq %d : address_lo = 0x%08x,"
		"data = 0x%08x\n", pdev->bus->name, irq,
...
)
{
	...
	if (!ret)
#ifdef CONFIG_RECOVERY
		setup_ioapic_irq(irq, cfg, attr, recovery);
#else
...
#ifdef CONFIG_RECOVERY
	ret = io_apic_setup_irq_pin(irq, node, attr, 0);
#else
...
# ifdef CONFIG_E2K
	if (IS_MACHINE_E3M && (mpc_ioapic_addr(idx) !=
			       IO_APIC_DEFAULT_PHYS_BASE)) {
		pr_warning("BIOS bug, I/O APIC #%d base is 0x%lx for E3M! fixing up to 0x%x. (tell your hw vendor)\n",
			   mpc_ioapic_id(idx), mpc_ioapic_addr(idx),
			   IO_APIC_DEFAULT_PHYS_BASE);
		ioapics[idx].mp_config.apicaddr = IO_APIC_DEFAULT_PHYS_BASE;
	}
# endif /* CONFIG_E2K */
