...
#define UNKNOWN_FREQ	0
#define FREQ_NO_CMD	1
#define FREQ_CMD	2
int freq_set_cmdline = UNKNOWN_FREQ;
			      ...
			      struct clock_event_device *evt)
{
	...
	case CLOCK_EVT_MODE_ONESHOT:
		...
		__setup_APIC_LVTT(lapic_timer_frequency,
				cpu_has(CPU_HWBUG_LAPIC_TIMER) ? 0 :
...
#else
	/* TODO If full cpu disable by stopping the clock signal is implemented,
	 * then the usage of the lapic timer could become inappropriate. If that
	 * is the case the CLOCK_EVT_FEAT_C3STOP flag might help. Some info:
	 * http://marc.info/?l=linux-kernel&m=111482097708717&w=2 */
	.features	= CLOCK_EVT_FEAT_PERIODIC | CLOCK_EVT_FEAT_ONESHOT
			| CLOCK_EVT_FEAT_DUMMY | CLOCK_EVT_FEAT_NOHWSTOP,
#endif
	.shift		= 32,
...
int __init calibrate_APIC_clock(void)
{
...
#else
	/* Using shift==32 somehow lead to overflows on e90s,
	 * so calculate shoft along with mult */
	clocks_calc_mult_shift(&lapic_clockevent.mult,
			       &lapic_clockevent.shift,
			       TICK_NSEC * LAPIC_CAL_LOOPS, delta, 4);
#endif
...
#else
	lapic_clockevent.max_delta_ns =
		clockevent_delta2ns(0xFFFFFFFF, &lapic_clockevent);
#endif
	lapic_clockevent.min_delta_ns =
		clockevent_delta2ns(0xF, &lapic_clockevent);
	...
	if (cpu_has_tsc) {
#if defined CONFIG_E2K || defined CONFIG_E90S
		lapic_calibration_result_ticks = (deltatsc * HZ) /
				LAPIC_CAL_LOOPS;
#endif
		apic_printk(APIC_VERBOSE, "..... CPU clock speed is "
			    ...
			    (deltatsc / LAPIC_CAL_LOOPS) % (1000000 / HZ));
		if (freq_set_cmdline == UNKNOWN_FREQ) {
			if (cpu_freq_hz == UNSET_CPU_FREQ)
				freq_set_cmdline = FREQ_NO_CMD;
			else
				freq_set_cmdline = FREQ_CMD;
		}
		if (freq_set_cmdline == FREQ_CMD) {
			lapic_calibration_result_ticks = cpu_freq_hz;
			pr_warning("CPU frequency %d Hz was set by cmdline\n",
				cpu_freq_hz);
		}
	}
...
#else
		regs_new.stacks.sbr = READ_SBR_REG_VALUE();
		regs_new.next = NULL;
#endif
...
__visible void __irq_entry smp_apic_timer_interrupt(struct pt_regs *regs)
{
	...
	if ((rts_act_mask & RTS_POSTP_TICK) || next_time) {
		if (next_time) {
...
void clear_local_APIC(void)
{
	...
	if (maxlvt >= 3) {
#ifdef CONFIG_E2K
		e2k_saved_lapic[cpu][3] = apic_read(APIC_LVTERR);
#endif
	...
	v = apic_read(APIC_LVTT);
#ifdef CONFIG_E2K
	e2k_saved_lapic[cpu][0] = v;
#endif
	apic_write(APIC_LVTT, v | APIC_LVT_MASKED);
	v = apic_read(APIC_LVT0);
#ifdef CONFIG_E2K
	e2k_saved_lapic[cpu][1] = v;
#endif
	apic_write(APIC_LVT0, v | APIC_LVT_MASKED);
	v = apic_read(APIC_LVT1);
#ifdef CONFIG_E2K
	e2k_saved_lapic[cpu][2] = v;
#endif
	apic_write(APIC_LVT1, v | APIC_LVT_MASKED);
	if (maxlvt >= 4) {
		v = apic_read(APIC_LVTPC);
#ifdef CONFIG_E2K
		e2k_saved_lapic[cpu][3] = v;
#endif
...
#ifdef CONFIG_E2K
/**
 * enable_local_APIC - restore the local APIC
 */
void enable_local_APIC(void)
{
	...
	apic_printk(APIC_DEBUG, "Enabling local APIC on cpu %d.\n", smp_processor_id());
	 ...
	 * any level-triggered sources.
	 */
	apic_write(APIC_LVTT, e2k_saved_lapic[cpu][0]);
	apic_write(APIC_LVT0, e2k_saved_lapic[cpu][1]);
	apic_write(APIC_LVT1, e2k_saved_lapic[cpu][2]);
	if (maxlvt >= 4)
		apic_write(APIC_LVTPC, e2k_saved_lapic[cpu][3]);
	...
	apic_printk(APIC_DEBUG, "Local APIC enabled on cpu %d.\n",
			smp_processor_id());
}
#endif
...
#ifdef CONFIG_E2K
		if (!HAS_MACHINE_E2K_FULL_SIC && !IS_MACHINE_E3M_IOHUB &&
		    value_lvt2 != 0x00018000) {
			rdma_apic_init = 0;
			pr_info("RDMA lapic is absent\n");
		} else {
#endif
			cpu = smp_processor_id();
#ifdef CONFIG_NUMA
#ifdef CONFIG_E90S
			node = e90s_cpu_to_node(cpu);
#else
			...
			if (!rdma_node[node]) {
				...
				if (IS_MACHINE_E2S)
					rdma_node[node] = 1;
#endif				
				printk("RDMA lvt2 enabled on NODE#%d CPU#%d.\n",
				       node, cpu);
			} else {
				/* Disable LVT2 */
				value_lvt2 = APIC_LVT_MASKED;
				printk("RDMA lvt2 masked on NODE#%d CPU#%d.\n",
				       node, cpu);
			}
...
#else
	/* E90S machine with sparse CPU numbering (for example,
	 * phys_cpu_present_map == 0x5) will not boot. Most probably
	 * it is because smp_processor_id() != hard_smp_processorid()
	 * in this case and trap_block[] array is indexed using both
	 * these functions randomly.
	 * Fixing it is too much hassle for nothing.
	 *
