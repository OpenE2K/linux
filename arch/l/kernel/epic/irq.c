...
void epic_init_system_handlers_table(void)
{
	 ...
	 */
	setup_APIC_vector_handler(EPIC_RESCHEDULE_VECTOR,
			epic_smp_reschedule_interrupt, 1,
			"epic_smp_reschedule_interrupt");

	/* IPI for generic function call */
	setup_APIC_vector_handler(EPIC_CALL_FUNCTION_VECTOR,
			epic_smp_call_function_interrupt, 1,
			"epic_smp_call_function_interrupt");

	/* IPI for generic single function call */
	setup_APIC_vector_handler(EPIC_CALL_FUNCTION_SINGLE_VECTOR,
	...
	setup_APIC_vector_handler(LINP0_INTERRUPT_VECTOR,
			epic_hc_emerg_interrupt, 1,
			"hc_emerg_interrupt");

	setup_APIC_vector_handler(LINP1_INTERRUPT_VECTOR,
			epic_iommu_interrupt, 1,
			"iommu_interrupt");

	setup_APIC_vector_handler(LINP2_INTERRUPT_VECTOR,
			epic_uncore_interrupt, 1,
			"uncore_interrupt");

	setup_APIC_vector_handler(LINP3_INTERRUPT_VECTOR,
			epic_ipcc_interrupt, 1,
			"ipcc_interrupt");

	setup_APIC_vector_handler(LINP4_INTERRUPT_VECTOR,
			epic_hc_interrupt, 1,
			"hc_interrupt");

	setup_APIC_vector_handler(LINP5_INTERRUPT_VECTOR,
