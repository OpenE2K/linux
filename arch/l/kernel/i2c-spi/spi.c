...
static u32 l_spi_read(const struct l_spi *l_spi, int reg)
{
	u32 res;
...
static void l_spi_write(const struct l_spi *l_spi, u32 val, int reg)
{
...
static u32 l_spi_wait_completion(const struct l_spi *l_spi)
{
	u32 status;
	u32 loops = 0;
...
static int l_spi_transfer_one_message(struct spi_controller *ctlr, struct spi_message *m)
{
	struct spi_device *spi = m->spi;
	const struct l_spi *l_spi = spi_master_get_devdata(spi->master);
	const struct device *dev = l_spi->dev;
...
next_transaction:
	 ...
	 * i2c-driver will do buffer locking for synchronization
	 * of 64-bytes buffer between i2c and spi. */
	if (buf_i && buf_write) {
#ifdef DEBUG
		int i;
#endif
...
static int l_spi_probe(struct platform_device *pdev)
{
	...
	master->transfer_one_message = l_spi_transfer_one_message;
