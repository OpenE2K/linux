...
typedef struct mpc_config_iolink {
	...
	unsigned long mpc_reserv2[2];	/* reserved */
} mpc_config_iolink_t;
...
enum mp_iolink_ver {
	MP_RDMA_SIC_VER		= 0x01, /* version of main RDMA for e3s/es2 */
	MP_IOHUB_E3M_VER	= 0x04, /* E3M IO controller is PIIX4 */
	MP_IOHUB_FPGA_VER	= 0x10, /* IOHUB implemented on FPGA (Altera) */
	MP_IOHUB_CHIP_VER	= 0x11  /* IOHUB implemented as chip */
};
...
enum mp_timertype {
	MP_PIT_TYPE,		/* programmed interval timer - E3M */
	MP_LT_TYPE,		/* Elbrus chipset timer - E3S */
...
#ifdef __KERNEL__
extern void print_floating_point(struct intel_mp_floating *mpf);
extern void print_boot_info(boot_info_t *boot_info);
