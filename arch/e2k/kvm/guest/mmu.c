	...
	case ATOMIC_GET_AND_CLEAR: {
		...
		if (pte_none(pteval)) {
			if (!pte_valid(pteval)) {
				...
				DebugKVMPTE("pte 0x%lx is alreada now and new "
		...
		u64 opc_ext, int chan, int qp_store, int atomic_store)
{
	...
	if (IS_HOST_KERNEL_ADDRESS(address)) {
	...
	if (!hret) {
		...
		DebugKVMREC("hypervisor could not recover store operation "
			"for address 0x%lx, error %ld\n", address, hret);
		return -EFAULT;
	}
}
					...
					u64 ld_rec_opc, int chan)
{
	...
	if (unlikely(IS_HOST_KERNEL_ADDRESS(address) ||
	...
	if (!hret) {
		...
		DebugKVMREC("hypervisor could not recover load operation for "
			"address 0x%lx, error %ld\n", address, hret);
		return -EFAULT;
	}
}
		...
		int qp_load, int atomic_load)
{
	...
	if (unlikely(IS_HOST_KERNEL_ADDRESS(addr_from) ||
	...
	if (!hret) {
		...
		DebugKVMREC("hypervisor could not move from "
			"address 0x%lx to 0x%lx, error %ld\n",
			addr_from, addr_to, hret);
...
static inline long kvm_do_move_tagged_data(int word_size, e2k_addr_t addr_from,
				e2k_addr_t addr_to)
{
	long hret;
	...
	if (!hret) {
		...
		DebugKVMREC("hypervisor could not move quad value from "
			"address 0x%lx, to 0x%lx error %ld\n",
			addr_from, addr_to, hret);
...
long kvm_move_tagged_word(e2k_addr_t addr_from, e2k_addr_t addr_to)
{
	return kvm_do_move_tagged_data(sizeof(u32), addr_from, addr_to);
}
long kvm_move_tagged_dword(e2k_addr_t addr_from, e2k_addr_t addr_to)
{
	return kvm_do_move_tagged_data(sizeof(u64), addr_from, addr_to);
}
long kvm_move_tagged_qword(e2k_addr_t addr_from, e2k_addr_t addr_to)
{
	return kvm_do_move_tagged_data(sizeof(u64) * 2, addr_from, addr_to);
}
...
int kvm_flush_icache_range(e2k_addr_t start, e2k_addr_t end)
{
	e2k_addr_t addr;
	u64 dummy = 0;
	int ret = 0;
	...
	for (addr = start; addr < end; addr += PAGE_SIZE) {
		...
		do {
			...
			if (!pte_valid(pte)) {
				/* access to address to fetch the page */
				dummy = *((u64 *)addr);
/*				pr_err("%s(): invalid address 0x%lx to flush "
					"ICACHE\n",
					__func__, addr);
				return -EFAULT;
*/
			...
			ret = HYPERVISOR_flush_icache_range(addr,
						addr + PAGE_SIZE, dummy);
			if (ret == 0) {
				break;
			} else if (ret == -EFAULT) {
				try++;
				if (try >= 3) {
					pr_err("%s(): could not do present "
						"address 0x%lx to flush "
						"ICACHE, error %d\n",
						__func__, addr, ret);
					break;
				}
			} else if (ret != 0) {
				pr_err("%s(): hypervisor could not flush "
					"ICACHE address 0x%lx, error %d\n",
					__func__, addr, ret);
				break;
			}
		} while (true);
