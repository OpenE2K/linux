...
{
	KVM_BUG_ON(level < gpt_walker->min_level || level > gpt_walker->max_level);
				...
				bool reinit)
{
	...
	if (likely(!reinit)) {
		...
		KVM_BUG_ON(gpt_entry->gpt_page == NULL ||
				gpt_entry->gpt_page_atomic == NULL);
	}
}
				...
				const pt_level_t *pt_level, int level)
{
	...
	KVM_BUG_ON(arch_is_error_gpa(gpte_gpa));
				...
				const pt_level_t *pt_level, int level)
{
	...
	KVM_BUG_ON(pages_num < PTE_PREFETCH_NUM);
				...
				const pt_level_t *pt_level, int start_level)
{
	...
	cur_level = gpt_entry->level;
	KVM_BUG_ON(start_level < cur_level);
				...
				gpa_t gpt_root, gpt_walker_t *gpt_walker)
{
	...
	KVM_BUG_ON((start & ~PAGE_MASK) != 0 || (end & ~PAGE_MASK) != 0);
	end_addr = end - 1;
	level = gpt_walker->max_level;
	KVM_BUG_ON(gpt_root != gpt_walker->gpt_root);
	gpt_base = gpt_root;
	gpt_entry = get_walk_addr_gpte_level(gpt_walker, level);
	pt_level = &pt_struct->levels[level];
	...
	do {
		...
		if (unlikely(is_last_gpte(&vcpu->arch.mmu, level, gpte))) {
			KVM_BUG_ON(!is_huge_gpte(vcpu, gpte));
				...
				gpt_walker_t *gpt_walker, int start_level)
{
	...
	do {
		...
		if (unlikely(!is_present_gpte(gpte))) {
			...
			KVM_BUG_ON(cur_level > start_level);
			break;
		}
					...
					int len, hva_t *hvap, bool *writable)
{
	...
	KVM_BUG_ON(len + offset > PAGE_SIZE);
			...
			const pt_level_t *pt_level, int level)
{
	...
	index = get_pt_level_addr_index(addr, pt_level);
	KVM_BUG_ON(index < gpt_entry->start_index ||
	...
	if (likely(get_gpte_priv_mask(vcpu))) {
		if (unlikely(priv_access)) {
			if (likely(vcpu->arch.is_hv)) {
				...
				KVM_BUG_ON(!is_guest_kernel_gva(gva));
			}
		} else {
...
retry_walk:
	...
	do {
		...
		KVM_BUG_ON((pte_access & ACC_PRIV_MASK) &&
			 ...
			 pgprot_t *sptep)
{
	...
	KVM_BUG_ON(start_index < gpt_entry->start_index ||
...
static bool
is_self_change_mapping(struct kvm_vcpu *vcpu, gpt_walker_t *gpt_walker,
		       unsigned gpte_access, gfn_t gfn,
		       int user_fault, bool *write_fault_to_shadow_pgtable)
{
	int level;
...
retry_pf_handle:
	...
	is_self_change_map |= is_self_change_mapping(vcpu, &gpt_walker,
					walker.pte_access, walker.gfn, user_fault,
				   ...
				   struct list_head *invalid_list)
{
	...
	if (unlikely(level == gpt_walker->max_level)) {
		KVM_BUG_ON(level != PT_ROOT_LEVEL);
			...
			int level, int split_to_level, pgprot_t *root_pte_hva)
{
	...
	int ind, max_host_level,  guest_level;
	...
	is_guest_pt_area = is_self_change_mapping(vcpu,
	...
	if (is_guest_pt_area) {
		 ...
		 */
		spin_unlock(&vcpu->kvm->mmu_lock);
		max_host_level = mapping_level(vcpu, gfn, &force_pt_level);
		spin_lock(&vcpu->kvm->mmu_lock);
			...
			gpa_t guest_root, gva_t vptb)
{
	...
	KVM_BUG_ON(IS_E2K_INVALID_PAGE(guest_root));
