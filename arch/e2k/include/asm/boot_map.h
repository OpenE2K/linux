 ...
 */
#ifndef _E2K_BOOT_MAP_H
#define _E2K_BOOT_MAP_H
...
typedef	struct e2k_tlb_item {		/* an item of TLB */
	...
	int		large_page;	/* large page (4Mb) flag */
} e2k_tlb_item_t;
typedef	struct e2k_tlb_line {		/* a line of TLB */
	e2k_tlb_item_t	sets[E2K_TLB_SETS_NUM];
	int		sets_num;	/* number of valid entries in */
					/* the line */
} e2k_tlb_line_t;
typedef	struct e2k_tlb {		/* all TLB */
	e2k_tlb_line_t	lines[E2K_MAX_TLB_LINES_NUM];
...
#else
extern	e2k_tlb_t __initdata_recv	dtlb_contents[NR_CPUS];
extern	e2k_tlb_t __initdata_recv	itlb_contents[NR_CPUS];
...
extern void __init	boot_init_mapping(void);
#ifdef	CONFIG_NUMA
extern void __init	boot_node_init_mapping(void);
#endif	/* CONFIG_NUMA */

extern int __init_recv boot_map_phys_area(e2k_addr_t phys_area_addr,
			e2k_size_t phys_area_size, e2k_addr_t area_virt_addr,
			pgprot_t prot_flags, e2k_size_t page_size,
			int ignore_busy);
extern int __init_recv boot_map_to_equal_virt_area(e2k_addr_t area_addr,
			e2k_size_t area_size,
			pgprot_t prot_flags, tlb_tag_t tlb_prot_flags,
			e2k_size_t page_size, int tlb_mask, int va);
extern int __init boot_unmap_virt_area(e2k_addr_t area_virt_addr,
			e2k_size_t area_size, e2k_size_t page_size,
			int ignore_absence);
extern int __init_recv init_clear_temporary_ptes(int tlb_mask, int cpuid);

#ifdef	CONFIG_RECOVERY
extern int boot_clear_temporary_ptes(int tlb_mask, int cpuid);
#endif	/* CONFIG_RECOVERY */

#endif /* _E2K_BOOT_MAP_H */
