...
static void __exit rtcfs_clean_kmemcache(void)
{
...
static void rtcfs_free_inode(struct inode *inode)
{
	...
	i = RTCFS_I(inode);
	path_put(&i->path);
	kmem_cache_free(rtcfs_inode_cachep, i);
}
...
MODULE_AUTHOR("denis.m.karpov@mcst.ru");
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("procfs support for binary compiler");
MODULE_VERSION("0.1");
