 ...
 *
 * 3) Because of the half-way implementation of PCIe "Enable No Snoop" mode
 * in iset v6 CPUs (it is ignored for writes) we need to flush cache every
 * time RAM is remapped to GnC type.  Otherwise DMA writes that are supposed
 * to be No Snoop will actually snoop the old data from cache.
 *
 * Also we _must_ map WC/UC as GnC for the same reason.  This means that
 * flushes from 1) and 2) above do not apply to any iset v6 CPUs if PCIe
 * No Snoop mode is used.
 *
 * Flush must be done after remapping.
 */
		...
		enum page_cache_mode req_memtype, bool is_ram)
{
	...
	if (/* 1st case above */ !req_coherent && prev_coherent ||
			/* 2nd case above */ cpu_has(CPU_FEAT_HW_PREFETCHER) &&
	...
	if (cpu_has(CPU_HWBUG_PCIE_NOSNOOP_PARTIAL)) {
		/* 3rd case above. */
		if (use_pcie_no_snoop &&
		    (memtype2pte_mem_type(req_memtype) == GEN_NON_CACHE_MT ||
		     memtype2pte_mem_type(req_memtype) == GEN_NON_CACHE_ORDERED_MT) &&
		    memtype2pte_mem_type(prev_memtype) == GEN_CACHE_MT) {
			return true;
		}
	}
...
/*
 * This must be called _before_ remmaping memory back to WB type.
