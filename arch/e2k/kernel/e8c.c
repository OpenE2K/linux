...
e8c_setup_cpu_info(cpuinfo_e2k_t *cpu_info)
{
	...
	IDR = read_IDR_reg();
	strncpy(cpu_info->vendor, E8C_CPU_VENDOR, 16);
	cpu_info->family = E8C_CPU_FAMILY;
	cpu_info->model  = IDR.IDR_mdl;
	cpu_info->revision = IDR.IDR_rev;
	cpu_info->L1_size = E8C_L1_CACHE_SIZE;
	cpu_info->L1_bytes = E8C_L1_CACHE_BYTES;
	cpu_info->L2_size = E8C_L2_CACHE_SIZE;
	cpu_info->L2_bytes = IDR_WBL_TO_BYTES(IDR.IDR_wbl);
	cpu_info->L3_size = E8C_L3_CACHE_SIZE;
	cpu_info->L3_bytes = E8C_L3_CACHE_BYTES;
}
