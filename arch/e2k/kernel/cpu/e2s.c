...
e2s_setup_arch(void)
{
	machine.setup_cpu_info = e2s_setup_cpu_info;
}

void __init
e2s_setup_machine(void)
{
	machine.setup_arch = e2s_setup_arch;
	machine.arch_reset = NULL;
	machine.arch_halt = NULL;
	machine.get_irq_vector = apic_get_vector;
	machine.get_nsr_area_phys_base = e2s_get_nsr_area_phys_base;
	machine.setup_apic_vector_handlers = e2s_setup_apic_vector_handlers;
}
