...
static void
e3s_setup_cpu_info(cpuinfo_e2k_t *cpu_info)
{
	e2k_idr_t IDR;

	IDR = read_IDR_reg();
	strncpy(cpu_info->vendor, E3S_CPU_VENDOR, 16);
	cpu_info->family = E3S_CPU_FAMILY;
	cpu_info->model  = IDR.IDR_mdl;
	cpu_info->revision = IDR.IDR_rev;
	cpu_info->L1_size = E3S_L1_CACHE_SIZE;
	cpu_info->L1_bytes = E3S_L1_CACHE_BYTES;
	cpu_info->L2_size = E3S_L2_CACHE_SIZE;
...
e3s_setup_arch(void)
{
	...
	for_each_node_has_dup_kernel(nid) {
		the_node_machine(nid)->setup_cpu_info = e3s_setup_cpu_info;
	}
}

void __init
e3s_setup_machine(void)
{
	int nid;

	for_each_node_has_dup_kernel(nid) {
		the_node_machine(nid)->iset = NULL;
		the_node_machine(nid)->setup_arch = e3s_setup_arch;
