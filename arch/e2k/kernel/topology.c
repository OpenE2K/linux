...
static void zero_page_duplicate(void)
{
	...
	/* Duplicate zero page */
	WARN_ON(kernel_image_duplicate_page_range(empty_zero_page,
...
static int __init duplicate_kernel_image(void)
{
	...
	WARN_ON(kernel_image_duplicate_page_range(_stext,
			_etext - _stext, false));
	WARN_ON(kernel_image_duplicate_page_range(__start_rodata_notes,
			__end_rodata_notes - __start_rodata_notes, false));
	WARN_ON(kernel_image_duplicate_page_range(__special_data_begin,
			__special_data_end - __special_data_begin, true));
	WARN_ON(kernel_image_duplicate_page_range(__node_data_start,
			__node_data_end - __node_data_start, false));
	WARN_ON(kernel_image_duplicate_page_range(__common_data_begin,
			__common_data_end - __common_data_begin, true));
	WARN_ON(kernel_image_duplicate_page_range(__init_text_begin,
			__init_text_end - __init_text_begin, false));
	WARN_ON(kernel_image_duplicate_page_range(__init_data_begin,
	...
	for_each_mem_pfn_range(i, MAX_NUMNODES, &start_pfn, &end_pfn, NULL) {
		...
		WARN_ON(kernel_image_duplicate_page_range((void *) start,
				end - start, true));
