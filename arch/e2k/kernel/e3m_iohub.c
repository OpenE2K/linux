...
void __init
boot_e3m_iohub_setup_arch(void)
{
...
static void
e3m_iohub_setup_cpu_info(cpuinfo_e2k_t *cpu_info)
{
 ...
 */
static void __init
e3m_iohub_create_io_config(void)
{
	iolinks_num = 1;
...
e3m_iohub_setup_arch(void)
{
	...
	for_each_node_has_dup_kernel(nid) {
		the_node_machine(nid)->setup_cpu_info =
			e3m_iohub_setup_cpu_info;
	}
#ifdef CONFIG_IOHUB_DOMAINS
	e3m_iohub_create_io_config();
#endif /* CONFIG_IOHUB_DOMAINS */
}

void __init
e3m_iohub_setup_machine(void)
{
	int nid;

	for_each_node_has_dup_kernel(nid) {
		the_node_machine(nid)->iset		= NULL;
		the_node_machine(nid)->setup_arch	= e3m_iohub_setup_arch;
