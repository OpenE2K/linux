...
void __init
boot_e3m_iohub_lms_setup_arch(void)
{
...
static void
e3m_iohub_lms_setup_cpu_info(cpuinfo_e2k_t *cpu_info)
{
...
/*
 * This e3m machine has only one IOLINK to connect to IOHUB,
 * so it has only one IO bus and PCI domain # 0
 */
static void __init
e3m_iohub_lms_create_io_config(void)
{
	iolinks_num = 1;
	...
	printk(KERN_INFO "IO LINK is connected to IOHUB controller ON\n");
}
#endif /* CONFIG_IOHUB_DOMAINS */

void __init
e3m_iohub_lms_setup_arch(void)
{
	int nid;
	...
	for_each_node_has_dup_kernel(nid) {
		the_node_machine(nid)->setup_cpu_info =
			e3m_iohub_lms_setup_cpu_info;
	}
#ifdef CONFIG_IOHUB_DOMAINS
	e3m_iohub_lms_create_io_config();
#endif /* CONFIG_IOHUB_DOMAINS */

}

void __init
e3m_iohub_lms_setup_machine(void)
{
	int nid;

	for_each_node_has_dup_kernel(nid) {
		the_node_machine(nid)->iset		= NULL;
		the_node_machine(nid)->setup_arch	= 
						e3m_iohub_lms_setup_arch;
