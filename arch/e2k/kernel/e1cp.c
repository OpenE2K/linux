...
e1cp_setup_cpu_info(cpuinfo_e2k_t *cpu_info)
{
	...
	IDR = read_IDR_reg();
	strncpy(cpu_info->vendor, E1CP_CPU_VENDOR, 16);
	cpu_info->family = E1CP_CPU_FAMILY;
	cpu_info->model  = IDR.IDR_mdl;
	cpu_info->revision = IDR.IDR_rev;
	cpu_info->L1_size = E1CP_L1_CACHE_SIZE;
	cpu_info->L1_bytes = E1CP_L1_CACHE_BYTES;
	cpu_info->L2_size = E1CP_L2_CACHE_SIZE;
