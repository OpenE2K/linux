...
map_skb:
		...
		if (unlikely(!(i & (E1000_RX_BUFFER_WRITE - 1)))) {
			...
			if (adapter->flags2 & FLAG2_PCIM2PCI_ARBITER_WA)
				...
				writel(i, rx_ring->tail);
#ifdef CONFIG_MCST
			l_fix_pcie_povozka_bug(&adapter->hw);
#endif
...
static int e1000_open(struct net_device *netdev)
{
	...
	struct pci_dev *pdev = adapter->pdev;
	int err;
	if(adapter->eeprom_bad) {
    	    printk(KERN_ERR "The EEPROM Checksum Is Not Valid."
		             " You must write EEPROM properly\n");
	    return -1;
	}
...
#if defined(CONFIG_MCST) && defined(CONFIG_PM)
	...
	for (i = 0;; i++) {
		...
		if (i == 2) {
			dev_err(pci_dev_to_dev(pdev), "The NVM Checksum Is Not Valid\n");
               {
                   unsigned char a[6] = {0xde, 0xad, 0xbe, 0xef, };
                   adapter->eeprom_bad = 1;
                   memcpy(netdev->dev_addr, a, sizeof(a));
                   strcpy(netdev->name, "eth%d");
                   register_netdev(netdev);
                   printk(KERN_ERR "%s: Intel(R) PRO/1000 Network Connection."
                                   " You must write EEPROM properly\n",
                                   netdev->name);
                   cards_found++;
