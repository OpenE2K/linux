...
#define DEBUG_MDIO_RD if (DEBUG_MDIO_RD_ON) pr_warn
...
static void mdio_write(struct net_device *dev, int phy_id, int reg_num, int val)
{
	...
	if (reg_num == 0) {
		pr_warn("%s: attempt to write 0x%04x to BMCR",
			dev->name, val);
		WARN_ON(1);
	}
...
static void lance_print_media(struct mii_if_info *mii)
{
	...
	pr_warn("link %s, %sMbps, %s-duplex, lpa 0x%04x advertise 0x%04x\n",
		mii_link_ok(mii) ? "up" : "down",
		media & (ADVERTISE_100FULL | ADVERTISE_100HALF) ? "100" : "10",
		(media & ADVERTISE_FULL) ? "full" : "half",
		lpa, advertise);
	pr_warn("BMCR = 0x%04x, BMSR = 0x%04x, PHYDID1 = 0x%04x, PHYSID2 = 0x%04x\n",
	...
	pr_warn("reg16 = 0x%04x, reg17 = 0x%04x, red18 = 0x%04x, reg19 = 0x%04x\n",
...
static int our_ioctl (struct net_device *dev, struct ifreq *rq, int cmd)
{
		...
		pr_warn("csr0= 0x%hx (addr:%p)   init_block_dvma= 0x%llx\n",
			lance_readw(lp->lregs.rdp), lp->lregs.rdp,
			(unsigned long long)lp->init_block_dvma);
		pr_warn("DMA_CSR= 0x%x  DMA_ADDR= 0x%x DMA_COUNT= 0x%x DMA_TEST=0x%x\n",
...
int lance_change_mtu(struct net_device *dev, int new_mtu)
{
	dev->mtu = new_mtu;
	return 0;
}
