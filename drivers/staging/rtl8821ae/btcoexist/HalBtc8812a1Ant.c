	...
	)
{
	...
	else
	{
		...
		if(retry_count == 0)  // no retry in the last 2-second duration
		{
			...
			if(up >= n)	// if ЁsдР n ╜с2╛М retry count╛╟0, ╚h╫у╪eWiFi duration
			{
				wait_count = 0; 
				n = 3;
				up = 0;
		...
		else if (retry_count <= 3)	// <=3 retry in the last 2-second duration
		{
			...
			if (dn == 2)	// if ЁsдР 2 ╜с2╛М retry count< 3, ╚h╫у╞╤WiFi duration
			{
				if (wait_count <= 2)
					m++; // ав╖K╓@╙╫╕b╗Б╜сlevel╓╓╗с╕^
				else
				...
				if ( m >= 20) //m Ёл╓j╜х = 20 ' Ёл╓j120╛М recheck╛O╖_╫у╬Ц WiFi duration.
					m = 20;
		...
		else  //retry count > 3, ╔u╜n1╕╦ retry count > 3, ╚h╫у╞╤WiFi duration
		{
			if (wait_count == 1)
				m++; // ав╖K╓@╙╫╕b╗Б╜сlevel╓╓╗с╕^
			else
