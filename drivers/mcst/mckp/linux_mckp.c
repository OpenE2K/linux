 ...
 * MC Board Driver general functions (MCB).
 */
...
#include "linux_mckp_match.h"
#include <linux/mcst/linux_me90_int.h>

#include <linux/of_device.h>

#define DBGMCKP_MODE 0
#define dbgmckp if(DBGMCKP_MODE) printk

#define DBGMCKPSPIN_MODE 0
#define dbgmckpspin if(DBGMCKPSPIN_MODE) printk

#define DBGMCKPDETAIL_MODE 0
#define dbgmckpdetail if(DBGMCKPDETAIL_MODE) printk

#define MCKP_INTERRUPT_DEBUG 0

#if MCKP_INTERRUPT_DEBUG
...
({	__asm__ __volatile__("sta %0, [%1] %2" \
			     ...
			     : "memory"); })
#endif /* MCKP_INTERRUPT_DEBUG */

#define CHP		printk(KERN_ERR "%s:%s():%d\n", __FILE__, __func__, __LINE__);

static int mckp_instances = 0;
static int mckp_major;

#define	MAX_MCKP_INSTANCES	16
static mcb_state_t	*mckp_states[MAX_MCKP_INSTANCES];

/*
 * Prototypes for this module
 */
static	int mckp_open(struct inode *inode, struct file *file);
static	int mckp_close(struct inode *inode, struct file *file);
extern	long mckp_ioctl (struct file *filp, unsigned int cmd, unsigned long arg);
static	unsigned int mckp_chpoll(struct file *file, struct poll_table_struct *wait);
static int mckp_mmap(struct file *file, struct vm_area_struct *vma);
static ssize_t mckp_read(struct file *pfile, char *buf, size_t sz, loff_t *lf);
static ssize_t mckp_write(struct file *pfile, const char *buf, size_t sz, loff_t *lf);
static irqreturn_t mckp_interrupt(int irq, void* arg);
static irqreturn_t mckp_intr_thread_handler(int irq, void* arg);

static struct file_operations mckp_fops = {
	owner:   THIS_MODULE,
	unlocked_ioctl:    mckp_ioctl,
	open:	  mckp_open,
	poll:     mckp_chpoll,
	mmap:     mckp_mmap,
	read:     mckp_read,
...
static inline int mckp_clock_freq(mcb_state_t *state)
{
	return state->op->clock_freq;
}

static int inline mckp_request_irq(mcb_state_t *state)
{
#ifdef __e2k__
	printk("%s(): request irq PCI 2 SBUS\n", __func__);
#else
	return request_threaded_irq(state->op->irqs[0], mckp_interrupt,
				mckp_intr_thread_handler,
				IRQF_SHARED | IRQF_DISABLED | IRQF_ONESHOT,
				"mcka", state);
#endif
}

static void inline mckp_free_irq(mcb_state_t *state)
{
#ifdef __e2k__
...
static inline int mckp_io_remap_page(mcb_state_t *state, int reg_set_num,
...
static inline void *mckp_ioremap(mcb_state_t *state, int reg_set_num)
{
	struct of_device *op = state->op;

	return of_ioremap(&op->resource[reg_set_num], 0,
                    resource_size(&op->resource[reg_set_num]), MCKP_NAME);
}

static void inline mckp_iounmap(mcb_state_t *state, int reg_set_num, void *addr)
{
	of_iounmap(&state->op->resource[reg_set_num], addr,
			resource_size(&state->op->resource[reg_set_num]));
}

static inline unsigned long mckp_dma_alloc_coherent(mcb_state_t *state, size_t size,
                                       dma_addr_t *dma_handle)
{
	return (unsigned long)dma_alloc_coherent(&state->op->dev, size, dma_handle, GFP_KERNEL);
}

static inline void mckp_dma_free_coherent(mcb_state_t *state, size_t size,
				unsigned long cpu_addr, dma_addr_t dma_handle)
{
	dma_free_coherent(&state->op->dev, size, (void *)cpu_addr, dma_handle);
}

static inline int mckp_dma_sync(mcb_state_t *state, dma_addr_t ba,
				 size_t size, int dir)
{
...
static void mckp_buf_trans_done(
		mcb_state_t *	state,
		int		channel,
		/*buf_t *		bp*/
		uio_t 		*uio_p
...
static int start_mckp_dma_engine(
		mcb_state_t	*state,
		int			channel
							);
...
static int	mckp_set_dma_trans_results(
		mcb_state_t		*state,
		int				channel,
...
static void mckp_init_trans_results(trans_spec_t *transfer_spec);

static int	mckp_create_trans_header(
		mcb_state_t	*state,
		int		buf_byte_size,
		int		pseudobuf_flag,
		int		flags,
...
static void	mckp_init_trans_header(
		mcb_state_t	*state,
		trans_buf_t	*trans_buf_p
);

static int mckp_create_drv_buf(
		mcb_state_t	*state,
		uio_t *		uio_p,
		int		op_flags,
		trans_spec_t 	*transfer_spec,
...
static void mckp_delete_drv_buf(
		mcb_state_t	*state,
		mcb_drv_buf_t	*trans_drv_buf_p
);

static void mckp_connection_polling_intr(
		mcb_state_t	*state,
...
static int mckp_strategy(
	mckp_state_t *state,
	int			channel,
	uio_t		*uio_p
);

static	int mckp_drv_buf_strategy(
	mckp_state_t	*state,
	int				channel,
	uio_t			*uio_p,
	int				op_flags,
...
static int mckp_dostrategy(
	uio_t			*uio_p,
	mckp_state_t	*state
);

static	int mckp_take_new_trans(
	mckp_state_t	*state,
	int				channel,
	trans_buf_t 	*trans_buf_p
);
...
#ifdef DEBUG_BUF_USE
char me90_debug_msg_buf[ME90_DEBUG_MSG_LINE_SIZE * ME90_DEBUG_MSG_LINE_NUM];
#else
...
void me90_log(mcb_state_t *state, int level, const char *fmt, ...)
{
	...
	case CE_PANIC:
		...
		if ( state ) {
			(void) sprintf(name, "%s%d", MCKP_NAME, state->inst);
		} else {
			(void) sprintf(name, "mckp");
		}
...
/*	case ME90_DL_MINOR:     if (me90drv_debug < 4) break; */
	...
	default:
		...
		sprintf(&me90_debug_msg_buf[cur_line * ME90_DEBUG_MSG_LINE_SIZE], "%s:\t%s", name, buf);
#else
	...
	me90drv_trans_buf_t	**trans_buf_pp)
{
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_wait_async_trans started for channel %d\n", channel);
	...
	if (channel_state -> async_trans_num <= 0) {
		...
		ME90_LOG(state, ME90_DL_ERROR,
			"mckp_wait_async_trans there are no outstanding"
...
/*			rval = cv_spin_timedwait(&state -> atrans_end_cv,
			...
			rval = cv_wait_sig(&state -> atrans_end_cv,
					&state->mutex);
			/*rval = cv_spin_wait(&state -> atrans_end_cv,
					&state -> lock);*/
		if ( rval < 0 ) {
			rval = ETIME;
			break;
	...
	if ( rval != 0 || channel_state -> ready_atrans_start == NULL ) {
		mutex_exit(&state->mutex);	/* end MUTEX */
		ME90_LOG(state, ME90_DL_TRACE,
			"mckp_wait_async_trans waiting finished inefficiently"
			" for channel %d\n", channel);
		return rval;
	}
	...
	*trans_buf_pp = channel_state -> ready_atrans_start;
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_wait_async_trans succeeded for channel %d\n", channel);
 ...
 */
/*ARGSUSED*/
u_int	mckp_rotate_word_bytes(u_int	source_word)
{
	u_int     new_word = 0;
	...
	int			flags)
{
	int reqlen;
#ifdef DEBUG
	printk("mckp_alloc_trans_bufs started with buffer byte size %d\n",
		buf_byte_size);
#endif
...
/*	reqlen = ((buf_byte_size + (ME90_ENABLE_BURST_SIZES - 1)) / 
	...
	new_trans_buf->dma.dma = mckp_dma_alloc_coherent(state, reqlen,
			 &new_trans_buf->dma.prim_dev_mem );
#if 0
	if (ddi_dma_mem_alloc(state->dip, reqlen, 
			&new_trans_buf->dma.prim_dev_mem,
			&new_trans_buf->dma.real_size,
			&new_trans_buf->dma.dma) != DDI_SUCCESS) {
	...
	dbgmckp("mckp_alloc_trans_bufs: new_trans_buf -> buf_address = 0x%lx, "
		"new_trans_buf -> buf_size = 0x%lx\n", 
		(u_long)new_trans_buf -> buf_address, (u_long)new_trans_buf -> buf_size);

#ifdef DEBUG
	printk(	"mckp_alloc_trans_bufs succeeded for buf 0x%08x with buffer"
		" byte size %d\n", (int )new_trans_buf, buf_byte_size);
#endif
...
/*ARGSUSED*/
void
mckp_drv_buf_io_error(
	me90drv_trans_buf_t	*trans_buf_p,
	int			error_code)
{
	ME90_LOG(NULL, ME90_DL_TRACE,
		"mckp_drv_buf_io_error started for trans_buf 0x%08x and "
	...
	if ( !trans_buf_p -> trans_buf_desc.drv_buf_used ||
		...
		ME90_LOG(NULL, ME90_DL_TRACE,
			"mckp_drv_buf_io_error the transfer has not private"
			" buffer\n");
		return;
	}
#endif	/* __BLOCK_BUFFER_USE__ */
	...
	ME90_LOG(NULL, ME90_DL_TRACE,
		"mckp_drv_buf_io_error succeeded for trans_buf 0x%08x and"
		" error %d\n", trans_buf_p, error_code);
}
...
/*ARGSUSED*/
void mckp_drv_buf_io_done(
	mcb_state_t		*state,
	me90drv_trans_buf_t	*trans_buf_p,
	int			withot_mutex_enter)
{
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_drv_buf_io_done started for trans_buf 0x%08x\n",
		trans_buf_p);
	...
	if ( !trans_buf_p -> trans_buf_desc.drv_buf_used ||
		trans_buf_p -> drv_buf_p == NULL ) {
		ME90_LOG(NULL, ME90_DL_TRACE,
			"mckp_drv_buf_io_done the transfer has not private"
			" buffer\n");
		return;
	}
#endif	/* __BLOCK_BUFFER_USE__ */
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_drv_buf_io_done succeeded for trans_buf 0x%08x\n",
		trans_buf_p);
}
	...
	int			trans_canceled)
{
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_finish_trans started for channel %d\n", channel);
	channel_state = &state -> all_channels_state[channel];
#ifdef	__BLOCK_BUFFER_USE__
	...
	if ( drv_buf_used ) {
		if ( trans_buf_p -> drv_buf_p == NULL ) {
			ME90_LOG(state, ME90_DL_ERROR,
				"mckp_finish_trans drv buffer is NULL in"
				" trans buf 0x%08x\n", trans_buf_p);
			return;
		}
	...
	if ( async_trans_flag && !trans_canceled ) {
		...
		if ( channel_state -> ready_atrans_start == NULL )
			...
			channel_state -> ready_atrans_end -> next_trans_buf = trans_buf_p;
...
#endif	/* __BLOCK_BUFFER_USE__ */
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_finish_trans finished for channel %d\n", channel);
}
...
/*ARGSUSED*/
int
mckp_finish_dma_engine_on_error(
	mcb_state_t		*state,
	int			channel,
	...
	int			error_code)
{
	...
	channel_state = &state -> all_channels_state[channel];
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_finish_dma_engine_on_error started for channel %d\n",
		channel);
	trans_buf_p = channel_state -> in_progress_start;
	if ( trans_buf_p == NULL ) {
		ME90_LOG(state, ME90_DL_ERROR,
			"mckp_finish_dma_engine_on_error no transfer in "
	...
	if ( error_code == ETIME && transfer_complited ) {
		ME90_LOG(state, ME90_DL_ERROR,
			"mckp_finish_dma_engine_on_error transfer with timer "
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_finish_dma_engine_on_error I/O transfer finished"
		" for channel %d\n", channel);
	...
	int		channel)
{
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_terminate_dma_trans started for channel %d\n", channel);
	channel_state = &state -> all_channels_state[channel];
	mutex_enter(&state->mutex);			/* start MUTEX */
	if (channel_state -> term_trans_processed ||
		...
		mutex_exit(&state->mutex);		/* end MUTEX */
		ME90_LOG(state, ME90_DL_TRACE,
			"mckp_terminate_dma_trans no transfers to terminate for"
			" channel %d\n", channel);
		return;
	}
...
#endif	/* __BLOCK_BUFFER_USE__ */
			mckp_finish_trans(state, channel, cur_trans_buf_p, 0, 0);

#ifdef	__BLOCK_BUFFER_USE__
		  else {
			ME90_LOG(state, ME90_DL_ERROR,
				"mckp_terminate_dma_trans unknown transfer "
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_terminate_dma_trans successed for channel %d\n", channel);
}
	...
	int		channel)
{
	...
	if (channel < 0) {
		ME90_LOG(state, ME90_DL_TRACE,
			"mckp_delete_timeout general timeout\n");
	} else {
		ME90_LOG(state, ME90_DL_TRACE,
			"mckp_delete_timeout for channel %d\n", channel);
	}
	if (channel < 0) {		/* general timeout */
		if (state -> timeout_type == no_timeout_type) {
			ME90_LOG(state, ME90_DL_ERROR,
				"mckp_delete_timeout : no general timeout\n");
		}
		...
		if (channel_state -> timeout_type == no_timeout_type) {
				ME90_LOG(state, ME90_DL_ERROR,
					"mckp_delete_timeout : no timeout for"
					" channel %d\n", channel);
				return;
		}
		if ((!channel_state -> busy &&
		    ...
		    (state -> drv_general_modes & MULTI_REQ_CHANNEL_DRV_MODE))) {
			ME90_LOG(state, ME90_DL_ERROR,
				"mckp_delete_timeout : channel %d is not busy"
				" by I/O\n", channel);
		}
	...
	state -> timeouts_num --;
	ME90_LOG(state, ME90_DL_TRACE, "mckp_delete_timeout successed\n");
}
...
static	void
mckp_general_timeout(mcb_state_t	*state)
{
	ME90_LOG(state, ME90_DL_TRACE, "mckp_general_timeout unused so far\n");
}
	...
	int		channel)
{
	...
	if (channel < 0) {
		ME90_LOG(state, ME90_DL_TRACE,
			"mckp_reset_timeout general timeout\n");
	} else {
		ME90_LOG(state, ME90_DL_TRACE,
			"mckp_reset_timeout for channel %d\n", channel);
	}
	mutex_enter(&state->mutex);			/* start MUTEX */
	mckp_delete_timeout(state, channel);
	mutex_exit(&state->mutex);			/* end MUTEX */

	ME90_LOG(state, ME90_DL_TRACE, "mckp_reset_timeout successed\n");
}
	...
	int		mutex_locked)
{
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_delete_trans_in_progress started for channel %d\n",
		channel);
	...
	while (channel_state -> in_progress_start != NULL) {
		...
		if (cur_trans_buf_p != channel_state -> in_progress_start)
			continue;
		mckp_finish_dma_engine_on_error(state, channel, gen_reg_state,
			error_code);
	}
	if (!mutex_locked)
	{
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_delete_trans_in_progress finished for channel %d\n",
		channel);
}
	...
	int		mutex_locked)
{
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_delete_all_exec_trans started for channel %d\n", channel);
	channel_state = &state -> all_channels_state[channel];
	mckp_delete_trans_in_progress(state, channel, error_code,
	...
	while (1) {
		...
		if (!cur_trans_buf_p -> trans_buf_desc.drv_buf_used ||
			...
			mckp_finish_trans(state, channel, cur_trans_buf_p,
				mutex_locked, 0);
#ifdef	__BLOCK_BUFFER_USE__
		}
		else {
			ME90_LOG(state, ME90_DL_ERROR,
				"mckp_delete_all_exec_trans unknown transfer "
				"method for channel %d\n", channel);
		}
#endif	/* __BLOCK_BUFFER_USE__ */
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_delete_all_exec_trans succeeded for channel %d\n",
		channel);
}
	...
	int		channel)
{
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_channel_timeout started for channel %d\n", channel);
	channel_state = &state -> all_channels_state[channel];
	mutex_enter(&state->mutex);			/* start MUTEX */
	if ((!channel_state -> busy &&
	    !(state -> drv_general_modes & MULTI_REQ_CHANNEL_DRV_MODE)) ||
		...
		if (channel_state -> timeout_type != no_timeout_type ||
		    channel_state -> timeout_rem > 0) {
			ME90_LOG(state, ME90_DL_ERROR,
				"mckp_channel_timeout: channel %d is free,"
				" but timeout is set\n", channel);
			mckp_reset_timeout(state,channel);
		}
		ME90_LOG(state, ME90_DL_TRACE,
			"mckp_channel_timeout channel %d : transfer completed"
			" already\n", channel);
		return 0;
	}
	if (channel_state -> busy && channel_state -> dma_intr_handled) {
		mutex_exit(&state->mutex);		/* end MUTEX */
		ME90_LOG(state, ME90_DL_TRACE,
			"mckp_channel_timeout channel %d : transfer now"
			" completed\n", channel);
		return 0;  /* interrupt handler deletes timeout */
	}
	if (channel_state -> timeout_type == no_timeout_type) {
		ME90_LOG(state, ME90_DL_ERROR,
			"mckp_channel_timeout: channel %d is busy, but "
			"timeout does not set\n", channel);
	} else if (channel_state -> timeout_rem > 0) {
		ME90_LOG(state, ME90_DL_ERROR,
			"mckp_channel_timeout: channel %d remainder time %d"
	...
	gen_reg_state.ME90DRV_RGEN_read = 0;
	rval = mckp_finish_dma_engine_on_error(state, channel, gen_reg_state,
			ETIME);
	mutex_exit(&state->mutex);			/* end MUTEX */
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_channel_timeout finished for channel %d\n", channel);
	return 1;
}

static void mckp_watchdog(caddr_t arg);

/*
 * Timeout (watchdog services) callback function
 */
/*ARGSUSED*/
void mckp_watchdog_handler(struct work_struct *work)
{
	...
	do
     	{
waiting_mode:
		dbgmckp(KERN_ALERT "MCKP: waiting for mckp_watchdog started !!!!\n");
		current->policy = SCHED_FIFO;
		interruptible_sleep_on(&state->mckp_watchdog_handler);
		if (state->waking_up_mckp_watchdog_handler == 1){
			dbgmckp(KERN_ALERT "waking up mckp_watchdog_handler !!!\n");
			state->waking_up_mckp_watchdog_handler = 0;
			del_timer_sync(&(state -> timeout_idnt));
			break;
		}
		if (state->mckp_watchdog_handler_shutdown == 1) {
			ME90_LOG(state, ME90_DL_TRACE, "MCKP: mckp_watchdog_handler exit by signal\n");
			state->mckp_watchdog_handler_shutdown = 0;
			return 0;
		}
     	}  while (1);
#endif
	ME90_LOG(state, ME90_DL_TRACE, "mckp_watchdog started\n");

	mutex_enter(&state->mutex);			/* start MUTEX */
	if (state -> timeout_type != no_timeout_type) {	/* general timeout */
		if (state -> timeouts_num == 0) {
			ME90_LOG(state, ME90_DL_ERROR,
				"state_watchdog : not handled general "
				"timeout, bat timeouts num is 0\n");
		}
		state -> timeout_rem -= ME90DRV_WATCHDOG_DEF_VALUE;
		if (state -> timeout_rem <= 0)		/* time is over */
			mckp_general_timeout(state);
	}
	for (cur_chnl = 0; cur_chnl < MAX_ME90DRV_BOARD_CHANNEL_NUM;
		...
		if (channel_state -> timeout_type != no_timeout_type) {
			if (state -> timeouts_num == 0) {
				ME90_LOG(state, ME90_DL_ERROR,
					"mckp_watchdog : not handled chanel "
			...
			ME90_LOG(state, ME90_DL_TRACE,
				"mckp_watchdog : chanel %d timeout rem = %d\n",
				cur_chnl, channel_state -> timeout_rem);
			if (channel_state -> timeout_rem <= 0) {
				/* time is over */
				mutex_exit(&state->mutex);	/* end MUTEX */
				if (mckp_channel_timeout(state,cur_chnl) != 0) {
					mckp_terminate_dma_trans(state,cur_chnl);
	...
	if (state -> timeouts_num > 0) {	/* watchdog services are needed */
		...
		tm->function = (void *)mckp_watchdog;
...
//		mutex_exit(&state->mutex);			/* end MUTEX */
		...
		mutex_exit(&state->mutex);			/* end MUTEX */
		ME90_LOG(state, ME90_DL_TRACE,
			"mckp_watchdog services finished\n");
	}
//	goto waiting_mode;
}

void mckp_watchdog(caddr_t arg)
{
	mcb_state_t		*state = (mcb_state_t *) arg;

	ME90_LOG(state, ME90_DL_TRACE, "MCKP: mckp_watchdog start\n");
//	state->waking_up_mckp_watchdog_handler = 1;
     
//	wake_up(&state->mckp_watchdog_handler);

	schedule_work(&state->watchdog_tqueue);
	ME90_LOG(state, ME90_DL_TRACE, "MCKP: mckp_watchdog finish\n");

	dbgmckp("MCKP: mckp_watchdog\n");
}
	...
	int		mutex_started)
{
	...
	if (channel < 0) {
		dbgmckp("mckp_set_timeout general timeout: type %d value %d\n",
			(int)timeout_type, (int)timeout_value);
	} else {
		dbgmckp("mckp_set_timeout for channel %d type %d value %d\n",
	...
	if (timeout_value <= 0) {
		switch (timeout_type) {
			...
			default :
				ME90_LOG(state, ME90_DL_ERROR,
					"mckp_set_timeout failed : undefined"
					" timeout type %d\n", timeout_type);
				return 1;
		}
	}
	...
	if (channel < 0) {				/* general timeout */
		if (state -> timeout_type != no_timeout_type ||
			...
			ME90_LOG(state, ME90_DL_ERROR,
				"mckp_set_timeout failed : general"
		...
		if (channel_state -> timeout_type != no_timeout_type ||
			...
			int	rval = 0;
			ME90_LOG(state, ME90_DL_ERROR,
				"mckp_set_timeout failed : channel %d timeout "
		...
		if ((!channel_state -> busy &&
			...
			ME90_LOG(state, ME90_DL_ERROR,
				"mckp_set_timeout failed : channel %d is not"
				" busy by I/O cmd\n", channel);
			return 1;
		}
	...
	dbgmckp("mckp_set_timeout: channel_state -> timeout_type = %d\n", (int)channel_state -> timeout_type);
        dbgmckp("mckp_set_timeout: channel_state -> timeout_rem = %d\n", (int)channel_state -> timeout_rem);
        dbgmckp("mckp_set_timeout: channel_state -> last_timeout_type = %d\n", (int)channel_state -> last_timeout_type);
        dbgmckp("mckp_set_timeout: channel_state -> last_timeout_value = %d\n", (int)channel_state -> last_timeout_value);
	...
	if (start_timeout) {
		ME90_LOG(state, ME90_DL_TRACE,
			"mckp_set_timeout : watchdog started for a time %d"
			" mks\n", ME90DRV_WATCHDOG_DEF_VALUE);
	} else if (timeout_exist) {
		ME90_LOG(state, ME90_DL_TRACE,
			"mckp_set_timeout : watchdog was been started "
			"already\n");
	}
	dbgmckp("mckp_set_timeout successed\n");
	return 0;
}
...
/*	buf_t			*bp = NULL;*/
	...
	dbgmckp("mckp_set_trans_timeout started for channel %d\n", channel);
	udelay(10000);
	channel_state = &state -> all_channels_state[channel];
	if (!mutex_started){
		mutex_enter(&state->mutex);		/* start MUTEX */
	...
	if (cur_trans_buf == NULL) {
		if (!mutex_started)
     			mutex_exit(&state->mutex);	/* end MUTEX */
		ME90_LOG(state, ME90_DL_TRACE,
			"mckp_set_trans_timeout no any transfer in progress for"
			" channel %d\n", channel);
		return;
	}
	if (channel_state -> timeout_type != no_timeout_type ||
		...
		ME90_LOG(state, ME90_DL_TRACE,
			"mckp_set_trans_timeout timeout is set already for"
			" channel %d\n", channel);
		return;
	}
#ifdef	__BLOCK_BUFFER_USE__
	...
	if (transfer_spec_p != NULL) {
		...
		timeout_type = read_timeout_type;
	mckp_set_timeout(state, timeout_type, timeout_value, channel, 1);
	if (!mutex_started)
	{
		mutex_exit(&state->mutex);		/* end MUTEX */
	}
	dbgmckp("mckp_set_trans_timeout finished for channel %d\n", channel);
}


/*ARGSUSED*/
int
mckp_set_transfer_done(
	mcb_state_t	*state,
	int		channel,
	trans_state_t	transfer_state)
{
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_set_transfer_done for channel %d started\n", channel);
	...
	cv_broadcast(&state -> channel_cv);
        ...
        ME90_LOG(state, ME90_DL_TRACE,
		"mckp_set_transfer_done for channel %d successed\n", channel);
...
me90_clean_base_memory(mcb_state_t	*state)
{
	...
	ME90_LOG(state, ME90_DL_TRACE, "mckp_clean_base_memory started\n");
	...
	ME90_LOG(state, ME90_DL_TRACE, "mckp_clean_base_memory finished\n");
}
...
/*ARGSUSED*/
void
mckp_clean_drv_communication(me90drv_state_t	*state)
{
	me90drv_drv_intercom_t	*drv_communication = NULL;
	int			cur_arg = 0;

	ME90_LOG(state, ME90_DL_TRACE, "mckp_clean_drv_communication started\n");
	drv_communication =
		(me90drv_drv_intercom_t *) &state -> ME90DRV_INTERDRV_COMN_AREA;
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_clean_drv_communication successed\n");
}
	...
	int		clean_bmem)
{
	int     rval = 0;
#ifdef DEBUG
	printk( "mckp_reset_mp started\n");
#endif
	rval = me90drv_reset_general_regs(state, halt_mp);
	if (clean_bmem)
		me90_clean_base_memory(state);
	mckp_clean_drv_communication(state);
	if (rval != 0) {
		printk("mckp_reset_mp: reset board and MP finished\n");
		return 1;
	} else {
#ifdef DEBUG
		printk("mckp_reset_mp: reset board and MP successed\n");
#endif
		return 0;
	}
}
	...
	int		char_data)
{
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_write_base_memory started to copy data from addr 0x%08x"
     ...
     {
        ME90_LOG(state, ME90_DL_ERROR,
                "mckp_write_base_memory bad address and/or size of BMEM\n"
               );
        return EINVAL;
     }
     ...
     {
	ME90_LOG(state, ME90_DL_ERROR,
		"mckp_write_base_memory kernel and BMEM addresses have "
     ...
     if (begin_rem != 0)
     {
        ...
        int      cur_byte = 0;
        if (char_data)
           first_bmem_word = mckp_rotate_word_bytes(first_bmem_word);
     ...
     {
        if (char_data)
           bmem_area_to[cur_word] = mckp_rotate_word_bytes(
     ...
     if (rem != 0)
     {
        ...
        int      cur_byte = 0;
        if (char_data)
           last_bmem_word = mckp_rotate_word_bytes(last_bmem_word);
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_write_base_memory succeeded to copy data from addr 0x%08x"
		" to BMEM addr 0x%08x size 0x%x\n",
		address_from,address_to,byte_size);
	return 0;
}
	...
	int		drv_comm_area_locked)
{
	...
	ME90_LOG(state, ME90_DL_TRACE, "mckp_restart_mp started\n");

	me90_reset_mp(state, 2, 0);
	if ((!state -> mp_drv_loaded && state -> mp_debug_drv_flag ) ||
		!state -> mp_drv_started) {
		ME90_LOG(state, ME90_DL_ERROR,
			"mckp_restart_mp: MP driver was not loaded or started"
			" up\n");
		return 1;
	}
	if (state -> mp_debug_drv_flag)
		rval = mckp_write_base_memory(state,
	...
	if (rval != 0) {
		ME90_LOG(state, ME90_DL_ERROR,
			"mckp_restart_mp failed - write init code in bmem"
			" failed\n");
		return rval;
	}
	...
	if ((rval = me90drv_submit_mp_task(state, drv_load_mp_task,
		...
		ME90_LOG(state, ME90_DL_ERROR,
			"mckp_restart_mp failed - MP driver init errors\n");
		return rval;
	}
	if (drv_load_results.mp_init_results.mp_error_code != 0) {
		ME90_LOG(state, ME90_DL_ERROR,
			"mckp_restart_mp failed - with error detected by MP"
			" driver 0x%02x\n",
			drv_load_results.mp_init_results.mp_error_code & 0xff);
		return rval;
	}
	ME90_LOG(state, ME90_DL_TRACE, "mckp_restart_mp successed\n");
	return 0;
}
...
/*ARGSUSED*/
void
mckp_restart_all_sync_channel(mcb_state_t	*state)
{
	int			cur_chnl = 0;

	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_restart_all_sync_channel started \n");
	for (cur_chnl = 0; cur_chnl < MAX_ME90DRV_BOARD_CHANNEL_NUM;
	     cur_chnl ++) {
			me90drv_start_new_trans(state,cur_chnl);
	}
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_restart_all_sync_channel finished \n");
}
...
/*ARGSUSED*/
void
mckp_terminate_all_dma_trans(mcb_state_t	*state)
{
	int	cur_chnl = 0;

	ME90_LOG(state, ME90_DL_TRACE, "mckp_terminate_all_dma_trans started\n");
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_terminate_all_dma_trans successed\n");
}
	...
	me90drv_rd_reg_t	gen_reg_state)
{
	...
	ME90_LOG(state,ME90_DL_TRACE, "mckp_retrieve_trans_mode started\n");
	...
	if (!drv_comm_area_locked) {
		...
		ME90_LOG(state, ME90_DL_ERROR,
			"mckp_retrieve_trans_mode interdriver communication "
			"area not locked\n");
	...
	if (retrieve_reason != hangup_mp_state				&&
		...
		mutex_exit(&state->mutex);		/* end MUTEX */
		ME90_LOG(state,ME90_DL_TRACE,
			"mckp_retrieve_trans_mode retrieved already\n");
		return 0;
		...
		mutex_exit(&state->mutex);		/* end MUTEX */
		ME90_LOG(state,ME90_DL_TRACE,
			"mckp_retrieve_trans_mode retrieved locked by general "
			"driver mode\n");
		return rval;
	}
	ME90_LOG(state,ME90_DL_ERROR,
	...
	if (retrieve_reason == hangup_mp_state)
		...
		ME90_LOG(state,ME90_DL_ERROR,
			"mckp_retrieve_trans_mode bad MP state\n");
		error_code = EMPHANGUP;
	}
	for (cur_chnl = 0; cur_chnl < MAX_ME90DRV_BOARD_CHANNEL_NUM;
	     cur_chnl ++) {
		...
		while (cur_channel_state -> in_progress_start != NULL) {
			mckp_finish_dma_engine_on_error(state, cur_chnl,
				gen_reg_state, error_code);
		}
	}
	rval = mckp_restart_mp(state,1);
	if (rval != 0) {
		...
		mutex_exit(&state->mutex);		/* end MUTEX */
		ME90_LOG(state,ME90_DL_ERROR,
			"mckp_retrieve_trans_mode cannot restart MP\n");
		mckp_restart_all_sync_channel(state);
		mckp_terminate_all_dma_trans(state);
		return EMPCRASH;
	}
	rval = mcb_recover_trans_state(state, 1, 1);
	if (rval != 0)
		ME90_LOG(state,ME90_DL_ERROR,
			"mckp_retrieve_trans_mode cannot recover transfer"
			" state\n");
	if (!drv_comm_area_locked) {
		state -> drv_comm_busy = 0;
		cv_broadcast(&state -> drv_comm_cv);
	...
	ME90_LOG(state,ME90_DL_ERROR, "mckp_retrieve_trans_mode successed\n");
	...
	int		wait_mp_rom_drv_disable)
{
	...
	ME90_LOG(state, ME90_DL_TRACE, "mckp_interrupt_mp started\n");
	...
	if (cur_regs_value.ME90DRV_RGEN_TI_read == 1) {
		...
		if (cur_regs_value.ME90DRV_RGEN_TMI_read == 0) {
			ME90_LOG(state, ME90_DL_ERROR,
				"mckp_interrupt_mp MP interrupt is masked,"
				" so not processed\n");
		} else {
			ME90_LOG(state, ME90_DL_ERROR,
				"mckp_interrupt_mp last MP interrupt not"
				" processed by MP\n");
		}
		return EMPHANGUP;
	}
...
/* Lin mksec */		udelay(TASK_ACCEPT_BY_MP_DELAY_TIME);
		...
		if (!intr_processed) {
			ME90_LOG(state, ME90_DL_ERROR,
				"mckp_interrupt_mp interrupt not processed "
				"by MP for time %d mks + %d try on\n",
				waiting_time,cur_tryon);
			return EMPHANGUP;
		}
		ME90_LOG(state, ME90_DL_TRACE,
			"mckp_interrupt_mp interrupt was processed by MP for"
			" time %d mks + %d try on\n",
			waiting_time,cur_tryon);
	}
#endif	/* _WITHOUT_MP_INTERRUPT_ */
	if (mp_restart) {
		if (drv_communication -> mp_task == no_mp_task) {
			ME90_LOG(state, ME90_DL_ERROR,
				"mckp_interrupt_mp MP task is empty already\n");
		}
		if (me90drv_reset_general_regs(state, 0) != 0) {
			ME90_LOG(state, ME90_DL_ERROR,
				"mckp_interrupt_mp restart of MP finished"
				" with error\n");
		}
	}
...
/* Lin mksec */		udelay(ME90DRV_TASK_ACCEPT_BY_MP_DELAY_TIME);
		...
		if (rom_disable) {
			...
			ME90_LOG(state, ME90_DL_ERROR,
				"mckp_interrupt_mp Mp did not accept task for"
				" time %d mks + %d try on\n",
				waiting_time, cur_tryon);
			return EMPHANGUP;
		}
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_interrupt_mp MP accepted task for time %d"
		" mks + %d try on\n", waiting_time, cur_tryon);
	ME90_LOG(state, ME90_DL_TRACE, "mckp_interrupt_mp successed\n");
	return 0;
}
	...
	int		channel)
{
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_release_all_async_trans started for channel %d\n",
		channel);
	...
	while (1) {
		...
		ME90_LOG(state, ME90_DL_WARNING,
			"mckp_release_all_async_trans asynchronous transfer"
 ...
 */
/*ARGSUSED*/
void mckp_out_debug_msg_buf(void)
{
	int	cur_line = 0;
	...
	int			*allowed_burst_sizes)
{
	...
	ME90_LOG(state, ME90_DL_TRACE,"mckp_get_burst_sizes started\n");
	...
	if (transfer_spec != NULL) {
		...
		if (allowed_burst == 0) {
			ME90_LOG(state, ME90_DL_ERROR,
				"mckp_get_burst_sizes - empty allowed %02x & "
	...
	if ((allowed_burst & state -> system_burst) == 0) {
		ME90_LOG(state, ME90_DL_ERROR,
			"mckp_get_burst_sizes - empty allowed %02x & enable"
			" %02x burst sizes bitmap\n",
			allowed_burst,state -> system_burst);
		return EINVAL;
	}
	allowed_burst &= state -> system_burst;
	ME90_LOG(state, ME90_DL_TRACE,"mckp_get_burst_sizes, allowed_burst = %x\n", allowed_burst);
	cur_burst = 0x1;
	...
	ME90_LOG(state, ME90_DL_TRACE,"mckp_get_burst_sizes, cur_burst = %x\n", cur_burst);
	trans_base_addr_align = cur_burst | (cur_burst - 1);
	ME90_LOG(state, ME90_DL_TRACE,"mckp_get_burst_sizes, trans_base_addr_align = %x\n", trans_base_addr_align);
	cur_burst = 0x1;
	while (cur_burst <= allowed_burst) {
	...
	ME90_LOG(state, ME90_DL_TRACE,"mckp_get_burst_sizes, trans_byte_size_align = %x\n", trans_byte_size_align);
	*allowed_burst_sizes = trans_base_addr_align & trans_byte_size_align &
	...
	ME90_LOG(state, ME90_DL_TRACE,"mckp_get_burst_sizes successed, allowed_burst_sizes = %d\n", *allowed_burst_sizes);
	...
	trans_state_t	transfer_state)
{
        ...
        ME90_LOG(state, ME90_DL_TRACE,
		"mckp_free_channel for channel %d started\n", channel);

	/*
	 * lock mckp state structure
 	 */

	mutex_enter(&state->mutex);			/* start MUTEX */

        mckp_set_transfer_done(state, channel,transfer_state);

	/*
	 * Drop mckp state structure
 	 */

	mutex_exit(&state->mutex);			/* end MUTEX */

        ME90_LOG(state, ME90_DL_TRACE,
		"mckp_free_channel for channel %d successed\n", channel);

	return (0);
}

int   mckp_calculate_work_hr_time(
...
/*ARGSUSED*/
void
mckp_free_trans_bufs(
	mcb_state_t		*state,
	me90drv_trbuf_desc_t	*trans_buf_desc)
{
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_free_trans_bufs started for buffer 0x%08x\n",
		trans_buf_desc);
	
	mckp_dma_free_coherent(state, trans_buf_desc->dma.real_size,
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_free_trans_bufs succeeded\n");
}
	...
	me90drv_trans_buf_t	*trans_buf)
{
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_output_trans_state started for trans header 0x%08x\n",
		trans_buf);
...
/*		trans_buf -> trans_buf_desc.cookie.dmac_address,
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_output_trans_state successed for trans header 0x%08x\n",
		trans_buf);
}
	...
	int		char_data)
{
     ...
     ME90_LOG(state, ME90_DL_TRACE,"mckp_read_base_memory unimplemented now\n");
     return EINVAL;
}
                              ...
                              )
{
     ...
     {
        ME90_LOG(state, ME90_DL_TRACE,
                "mckp_bmem_data_transfer from SPARC memory 0x%08x"
     ...
     {
        ME90_LOG(state, ME90_DL_TRACE,
                "mckp_bmem_data_transfer from MP memory 0x%08x"
     ...
     {
        ME90_LOG(state, ME90_DL_ERROR,
                "mckp_bmem_data_transfer bad MP BMEM address 0x%08x and/or"
                " size 0x%x\n",
                transfer_desk -> mp_bmem_address,
                transfer_desk -> byte_size
               );
     ...
     {
        ME90_LOG(state, ME90_DL_ERROR,
                "mckp_bmem_data_transfer: kmem_alloc no memory is available\n"
               );
        return EINVAL;
     }
     if (write_op)
     {
           ...
           )
        {
           ...
           ME90_LOG(state, ME90_DL_ERROR,
		"mckp_bmem_data_transfer ddi_copyin failed\n");
           return (EFAULT);
        }
     }
     if (write_op)
        ...
        rval = mckp_read_base_memory(state,
                                &kmem_area[word_rem],
                                transfer_desk -> mp_bmem_address,
                                transfer_desk -> byte_size,
                                char_data
     ...
     if (rval != 0)
     {
        if (kmem_buf == NULL)
           kmem_free(kmem_area,kmem_size);
        ME90_LOG(state, ME90_DL_ERROR,
                "mckp_bmem_data_transfer read/write base memory failed\n"
               );
        return rval;
     }
     if (!write_op)
     {
           ...
           )
        {
           if (kmem_buf == NULL)
              kmem_free(kmem_area,kmem_size);
           ME90_LOG(state, ME90_DL_ERROR,
		"mckp_bmem_data_transfer ddi_copyout failed\n");
           return (EFAULT);
        }
     }
     if (kmem_buf == NULL)
     ...
     ME90_LOG(state, ME90_DL_TRACE,
             "mckp_bmem_data_transfer succeeded\n"
            );
     return 0;
}
	...
	int		mode)
{
...
#ifdef DEBUG
	printk("mckp_startup_mp started with command 0x%x\n", cmd);
#endif
	mutex_enter(&state->mutex);			/* start MUTEX */
...
#ifdef DEBUG
		printk(	"mckp_startup_mp: MP reset and halt finished with errors\n");
#endif
	}
	if (cmd == MCBIO_STARTUP_MP_ROM_DRV) {
		rval = mckp_write_base_memory(state,
				...
				sizeof(rom_drv_init_code), 1);
		if (rval != 0) {
			printk(	"mckp_startup_mp - write ROM driver "
				"init code in bmem failed\n");
		}
	} else {
		if (!state -> mp_drv_loaded) {
			mutex_exit(&state->mutex);		/* end MUTEX */
			printk(	"mckp_startup_mp MP driver code did not loaded"
				" in the BMEM\n");
			return (EINVAL);
		}
		...
		if (rval != 0) {
			mutex_exit(&state->mutex);		/* end MUTEX */
			printk("mckp_startup_mp MP startup failed\n");
			return rval;
		}
	}
	if (state -> mp_init_code.mp_drv_init_info != NULL   &&
		...
		if (cmd == MCBIO_STARTUP_MP_CODE) {
			...
			if (rval != 0) {
				mutex_exit(&state->mutex);	/* end MUTEX */
				printk(	"mckp_startup_mp MP init info "
					"load failed\n");
				return rval;
			}
		} else {
			if (state -> mp_init_code.mp_drv_init_info_size >
				...
				mutex_exit(&state->mutex);	/* end MUTEX */
				printk(	"mckp_startup_mp too long MP "
			...
			if (copy_from_user ((void *)state -> mp_drv_init_info.args_area,
					    state -> mp_init_code.mp_drv_init_info,
					    state -> mp_init_code.mp_drv_init_info_size) != 0) {
				printk(	"mckp_startup_mp ddi_copyin "
	...
	if (cmd == MCBIO_STARTUP_MP_CODE) {
		...
		mutex_exit(&state->mutex);			/* end MUTEX */
		if (rval != 0) {
			printk(	"mckp_startup_mp MP code startup "
				"finished with error\n");
		} else {;
#ifdef DEBUG
			printk(	"mckp_startup_mp MP code stratup "
				"succeeded\n");
#endif
		}
		return (rval);
	...
	if (rval != 0) {
		...
		mutex_exit(&state->mutex);		/* end MUTEX */
#ifdef DEBUG
		printk("mckp_startup_mp MP driver init failed\n");
#endif
		return rval;
	}
	state -> mp_drv_started = 1;
	...
	mutex_exit(&state->mutex);			/* end MUTEX */
#ifdef DEBUG
	printk("mckp_startup_mp MP driver init succeeded\n");
#endif
	return 0;
}
...
int rmv_dev(int inst, me90drv_chnl_state_t * chd, uchar_t channel) 
{
	...
	(void) sprintf(name, "%s_%d_%s:%d", mod_name, inst,
                       (channel & 0x1) ? "in" : "out", channel >> 1);
	error = ddi_unlink(MCKP_DIR, name);
	if (error){
 ...
 */
/*ARGSUSED*/
int mckp_open(struct inode *inode, struct file *file)
{
...
int
mckp_close(struct inode *inode, struct file *file)
{
	...
	ME90_LOG(NULL, ME90_DL_TRACE,"mckp_close started\n");
...
//	rval = cv_spin_timedwait(&state -> channel_cv, &state->lock,timeout_clock_ticks);	
		if ( rval < 0 ) {
			ME90_LOG(state, ME90_DL_ERROR,
				"mckp_close waiting for freeing of channel %d timeouted\n",
				channel
				);

			if ( !channel_state -> streaming || channel_state -> pseudostreaming ) {
				mckp_delete_all_exec_trans(state, channel, ETIME, 0, 1);
			}
	...
	if ( channel_state -> completed_trans_start != NULL ||
         channel_state -> term_trans_processed ) {
		mutex_exit(&state->mutex);
		mckp_terminate_dma_trans(state,channel);
		mutex_enter(&state->mutex);
	}

	if ( channel_state -> ready_atrans_start != NULL ) {
		mutex_exit(&state->mutex);
		mckp_release_all_async_trans(state, channel);
		mutex_enter(&state->mutex);
	}
	...
	if ( state->opened == 0 ) {
		...
		if ( state -> trans_mode_inited ) {
			ME90_LOG(state, ME90_DL_ERROR,
				"mckp_close: transfer mode did not halted\n"
				);

			mutex_exit(&state->mutex);
			rval = mckp_halt_trans_mode(state);
			mutex_enter(&state->mutex);

			if ( rval != 0 )
				ME90_LOG(state, ME90_DL_ERROR,
					"mckp_close: halt transfer mode failed\n"
					);
		}
...
//	     rval = cv_spin_timedwait(&state->drv_comm_cv, &state->lock,timeout_clock_ticks);
			...
			if ( rval < 0 ) {
				ME90_LOG(state, ME90_DL_ERROR,
					"mckp_close waiting for freeing of interdriver  "
	...
	ME90_LOG(state, ME90_DL_TRACE,"mckp_close succesed, instance %d channel %d\n",
		instance, channel
		);
#ifdef DEBUG_BUF_USE
	if ( !state->opened )
		me90_out_debug_msg_buf();
#endif /* DEBUG_BUF_USE */
...
/*ARGSUSED*/
int
mckp_halt_trans_mode(mcb_state_t		*state)
{
     int		cur_chnl = 0;
     int		rval = 0;

     ME90_LOG(state, ME90_DL_TRACE,	/*!!!!!!*/
             "mckp_halt_trans_mode started\n"
            );
     mutex_enter(&state->mutex);			/* start MUTEX */
     if (!state -> trans_mode_inited && state -> trans_mode_init_error == 0)
     {
        mutex_exit(&state->mutex);			/* end MUTEX */
        ME90_LOG(state, ME90_DL_ERROR,
                "mckp_halt_trans_mode: all transfers halted already\n"
               );
        return EINVAL;
     }
     rval = submit_mp_task(state,halt_trans_mode_mp_task,
                           NULL,
                           1,
                           NULL,
                           NULL,
     ...
     if (rval != 0)
     {
        state -> trans_mode_init_error = rval;
        mutex_exit(&state->mutex);			/* end MUTEX */
        ME90_LOG(state, ME90_DL_ERROR,
                "mckp_halt_trans_mode: halt transfer mode failed\n"
               );
        return rval;
     }
     for (cur_chnl = 0; cur_chnl < MAX_MC_BOARD_CHANNEL_NUM; cur_chnl ++)
     {
        me90drv_chnl_state_t *channel_state = &state -> all_channels_state[cur_chnl];
        channel_state -> timetable_seted = 0;
        channel_state -> timetable_set_error = 0;
     }
     state -> trans_mode_inited = 0;
     state -> trans_mode_init_error = 0;
     mutex_exit(&state->mutex);			/* end MUTEX */
     ME90_LOG(state, ME90_DL_TRACE,"mckp_halt_trans_mode finished\n");
     return 0;
}
...
static	ssize_t
mckp_read(struct file *pfile, char *buf, size_t sz, loff_t *lf)
{
	...
	ME90_LOG(NULL, ME90_DL_TRACE,"mckp_read started\n");
...
/*ARGSUSED*/
static ssize_t
mckp_write(struct file *pfile, const char *buf, size_t sz, loff_t *lf)
{
	mcb_state_t	*state = (mcb_state_t *)pfile->private_data;
	dev_t		dev;
	struct uio	*uio_p;
	...
	ME90_LOG(NULL, ME90_DL_TRACE,"mckp_write started\n");
...
//   struct buf *	buf_p = NULL;
	...
	if ( state == NULL ) {
		ME90_LOG(NULL, ME90_DL_ERROR,"mckp_rdwr - bad instance %d\n",
			instance
			);
	...
	ME90_LOG(NULL, ME90_DL_TRACE,
		"mckp_rdwr inst %d channel %d started %s\n",
		instance ,channel, (flag == B_READ) ? "to read" : "to write");

	dbgmckpspin("MCKP: mcb_rdwr spin %s\n", raw_spin_is_locked(&state->lock) ? "LOCKED" : "UNLOCKED");	
	channel_state = &state -> all_channels_state[channel];

	if ( transfer_spec_p != NULL ) {
		io_mode_flags = transfer_spec_p -> io_mode_flags;
		mckp_init_trans_results(transfer_spec_p);
	}
	...
	if ( !state -> trans_mode_inited                &&
		...
		ME90_LOG(state, ME90_DL_ERROR,
			"mcb_rdwr transfer mode did not inited\n"
			);

		return -EINVAL;
	} else if ( !channel_state -> timetable_seted         &&
			state -> mp_state == started_mp_state     &&
			!(io_mode_flags & BMEM_TRANSFER_IO_MODE) ) {
		ME90_LOG(state, ME90_DL_ERROR,
			"mcb_rdwr works timetable mask did not set for channel %d\n",
			channel
			);
	...
	printk("%s RDWR %2x\n",(user_transfer_spec->read_write_flag == B_READ) ? "READ" : "WRITE",trans_base_addr[0]);
	rval = mckp_get_burst_sizes(state,
						channel_state,
						transfer_spec_p,
						trans_base_addr,
						transfer_len,
	...
	if ( rval != 0 ) {
		ME90_LOG(state, ME90_DL_TRACE,
			"mcb_rdwr instance %d channel %d mckp_get_burst_sizes failed\n",
			instance,channel
			);
	...
	if ( transfer_spec_p == NULL ) {
		...
		transfer_spec.timing_interval_t0 = 0;
	...
	if ( (transfer_spec_p -> io_mode_flags & ONLY_UNBUF_IO_MODE)  &&
		...
		if ( state -> drv_general_modes & ONLY_BUF_IO_DRV_MODE ) {
			ME90_LOG(state, ME90_DL_ERROR,
				"mckp_rdwr cannot use unbuffered I/O when ONLY BUF mode"
				"of driver is set\n"
			);
		} else {
			ME90_LOG(state, ME90_DL_ERROR,"mcb_rdwr buf used in the unbuf mode\n");
		}
	...
	ME90_LOG(state, ME90_DL_TRACE,"mckp_rdwr: drv_buf_using = %d\n", drv_buf_using);
...
/*         buf_p = getrbuf(KM_NOSLEEP);
           if (buf_p == NULL)
           {
              MCB_LOG(mcb, MCB_DL_ERROR,"mcb_rdwr cannot allocate buf\n");
              return (EINVAL);
           };
           buf_p -> b_private = transfer_spec_p;
           rval =(physio(mcb_strategy, buf_p, dev, flag, minphys, uio_p));*/
		   uio_p->op_flags = flag;
		   uio_p->transfer_spec = transfer_spec_p;
		   uio_p->dev = dev;
		   rval = mckp_strategy(state, channel, uio_p);

			if ( !transfer_spec_p -> async_trans || rval != 0 ) {
				mckp_buf_trans_done(state, channel, /*buf_p*/uio_p);
/*		if (buf_p != NULL)
			freerbuf(buf_p);*/
			}
			...
			dbgmckpspin("MCKP: mcb_rdwr before mckp_drv_buf_strategy: spin %s\n", raw_spin_is_locked(&state->lock) ? "LOCKED" : "UNLOCKED");	
			rval = mckp_drv_buf_strategy(state,channel,uio_p,flag,transfer_spec_p);
		}
...
/*
     mcb = (mcb_state_t *) ddi_get_soft_state(mcb_state,instance);
     if (mcb == NULL)
     {
        MCB_LOG(NULL, MCB_DL_ERROR,"mcb_strategy - bad instance %d\n",
                 instance
                );
...
*/
	...
	multi_buf_flag = 1;
     ...
     if (multi_buf_flag)
     {
        ME90_LOG(state, ME90_DL_TRACE,"mckp_strategy - multi-buffer used\n");
/*   	bp->b_resid = bp->b_bcount;
...
*/
     ...
     if (transfer_spec != NULL)
     {
        ...
        {
           ME90_LOG(state, ME90_DL_TRACE,
                   "mckp_strategy channel %d will be waiting for DRQ"
...
//                 bp -> b_un.b_addr,bp -> b_bcount
     ...
     ME90_LOG(state, ME90_DL_TRACE,
             "mckp_strategy channel %d finished for"
             " addr 0x%08x len 0x%x\n",
             channel,
//             bp -> b_un.b_addr,bp -> b_bcount
	     uio_p->uio_iov[0].iov_base, uio_p->uio_iov[0].iov_len
            );
     return mckp_dostrategy(/*bp*/uio_p,state);
}
...
//   trans_spec_t *    transfer_spec = bp -> b_private;
     ...
     ME90_LOG(NULL, ME90_DL_TRACE,
             "mckp_dostrategy inst %d channel %d started for"
             " addr 0x%08x len 0x%x\n",
             instance,channel,
//           bp -> b_un.b_addr,
//	     bp -> b_bcount
...
//      if (bp->b_flags & B_WRITE)
	if (uio_p->op_flags & B_WRITE)
     ...
     rval = mckp_create_trans_header(state,
               /*bp->b_bcount,*/
...
//             bp,
     ...
     {
        ME90_LOG(state, ME90_DL_TRACE,
                "mckp_dostrategy cannot create buffer for channel"
                " %d\n",
                channel
               );
/*      bp->b_resid = bp->b_bcount;
...
*/
	...
	rval = mckp_take_new_trans(state,channel ,trans_buf_p);
	if (rval != 0) {
		ME90_LOG(state, ME90_DL_WARNING,
			"mckp_dostrategy: mcb_take_new_trans failed for"
			" channel %d\n", channel);
...
*/
     ...
     ME90_LOG(state, ME90_DL_TRACE,
             "mckp_dostrategy channel %d successed for"
             " addr 0x%08x len 0x%x\n",
             channel,
	     uio_p->uio_iov[0].iov_base, uio_p->uio_iov[0].iov_len
//    	     bp -> b_un.b_addr,bp -> b_bcount
...
{ iovec_t *iovec_p = uio_p->uio_iov;
  ...
  case UIO_READ :   
	if (UIO_USERSPACE == uio_p->uio_segflg ) {
		dbgmckp("uiomove: UIO_READ, writing 0x%lx bytes to user 0x%lx adress from 0x%lx\n", 
  ...
  case UIO_WRITE : 
	if (UIO_USERSPACE == uio_p->uio_segflg ) {
		dbgmckp("uiomove: UIO_WRITE, writing 0x%lx bytes to kernel 0x%lx adress from 0x%lx\n", 
     ...
     )
{
     ...
     ME90_LOG(state, ME90_DL_TRACE,
             "mckp_drv_buf_strategy started for channel %d\n",
             channel
            );
     dbgmckpspin("MCKP: mckp_drv_buf_strategy: before mckp_create_drv_buf spin %s\n", 
		raw_spin_is_locked(&state->lock) ? "LOCKED" : "UNLOCKED");
     rval = mckp_create_drv_buf(state,uio_p,op_flags,transfer_spec,
                               &trans_drv_buf_p
                              );
     dbgmckpspin("MCKP: mckp_drv_buf_strategy: after mckp_create_drv_buf spin %s\n", 
		raw_spin_is_locked(&state->lock) ? "LOCKED" : "UNLOCKED");
     if (rval != 0)
     {
        ME90_LOG(state, ME90_DL_TRACE,
                "mckp_drv_buf_strategy cannot create driver buffer header "
                "for channel %d\n",
                channel
               );
        return rval;
     ...
     flags |= DDI_DMA_STREAMING;
	   transfer_spec -> trans_res_info -> event_start_time = ddi_gethrtime();
     dbgmckpspin("MCKP: mckp_drv_buf_strategy: before mckp_create_trans_header spin %s\n", 
		raw_spin_is_locked(&state->lock) ? "LOCKED" : "UNLOCKED");
     ...
     dbgmckpspin("MCKP: mckp_drv_buf_strategy: after mckp_create_trans_header spin %s\n", raw_spin_is_locked(&state->lock) ? "LOCKED" : "UNLOCKED");

     if (rval != 0)
     {
        ME90_LOG(state, ME90_DL_TRACE,
                "mckp_drv_buf_strategy cannot create driver private buffer "
                "for channel %d\n",
                channel
               );
        mckp_delete_drv_buf(state,trans_drv_buf_p);
        return rval;
     } else {
        ME90_LOG(state, ME90_DL_TRACE,
                "mckp_drv_buf_strategy start trans with driver private buffer "
     ...
     if (trans_drv_buf_p -> op_flags & B_WRITE)
     {
      rval = uiomove(trans_buf_p -> trans_buf_desc.buf_address,
        ...
        if (rval != 0)
        {
           ...
           mckp_finish_trans(state, channel, trans_buf_p, 0, 1);	/* canceled */
           mckp_delete_drv_buf(state,trans_drv_buf_p);
           return rval;
        }
		rval = mckp_dma_sync(state,
        ...
        {
           ME90_LOG(state, ME90_DL_ERROR,
                   "mckp_drv_buf_strategy - ddi_dma_sync failed for channel"
                   " %d\n",
                    channel
                   );
	...
	dbgmckpspin("MCKP: mckp_drv_buf_strategy: before mckp_take_new_trans spin %s\n", 
		raw_spin_is_locked(&state->lock) ? "LOCKED" : "UNLOCKED");	
	rval = mckp_take_new_trans(state, channel, trans_buf_p);
	if (rval != 0) {
		ME90_LOG(state, ME90_DL_TRACE,
			"mckp_drv_buf_strategy: mckp_take_new_trans failed "
			"for channel %d\n", channel);
		mcb_delete_trans_header(state, trans_buf_p);
		mckp_delete_drv_buf(state, trans_drv_buf_p);
		return rval;
	...
	if (transfer_spec -> async_trans) {
		ME90_LOG(state, ME90_DL_TRACE,
			"mckp_drv_buf_strategy successed for channel %d async."
			" transfer\n", channel);
		return 0;
	}
	...
	while (!trans_drv_buf_p -> trans_completed) {
		ME90_LOG(state, ME90_DL_TRACE,
			"mckp_drv_buf_strategy: waiting for transfer completed "
			"for channel %d\n", channel);
...
//		cv_spin_wait(&trans_drv_buf_p -> trans_finish_cv, &state->lock);
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_drv_buf_strategy succeeded for channel %d\n", channel);
	...
	trans_buf_t	*trans_buf_p)
{
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_finish_drv_buf_trans started for channel %d\n", channel);

	channel_state = &state -> all_channels_state[channel];
	if (!trans_buf_p -> trans_buf_desc.drv_buf_used ||
		trans_buf_p -> drv_buf_p == NULL) {
		ME90_LOG(state, ME90_DL_TRACE,
			"mckp_finish_drv_buf_trans the transfer has not private"
			" buffer\n");
		return EINVAL;
	}
	...
	if (trans_drv_buf_p -> op_flags & B_READ) {
		...
		if  (real_trans_size > source_size) {
			ME90_LOG(state, ME90_DL_ERROR,
				"mckp_finish_drv_buf_trans real transfered bytes"
		...
		if (trans_buf_p -> real_trans_size > 0) {
			...
			ME90_LOG(state, ME90_DL_TRACE,
				"mckp_finish_drv_buf_trans move %d byte(s) of"
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_finish_drv_buf_trans finished with result %d for channel"
		" %d\n", rval, channel);
	return rval;
}
     ...
     )
{
	...
	size_t		buf_byte_size = 0;
     ...
     ME90_LOG(state, ME90_DL_TRACE,
             "mckp_take_new_trans started for channel %d\n",
             channel
            );
     ...
     {
	ME90_LOG(state, ME90_DL_TRACE,
             "mckp_take_new_trans : drv_buf_used = 0\n"); 
...
*/
	...
	dbgmckp("mckp_take_new_trans: trans_base_addr = 0x%lx, buf_byte_size = 0x%lx\n",
		(u_long)trans_base_addr, (u_long)buf_byte_size);
     }
     else
     {
	dbgmckpspin("MCKP: mckp_take_new_trans: spin %s\n", 
		raw_spin_is_locked(&state->lock) ? "LOCKED" : "UNLOCKED");	
...
/*	if (!trans_buf_p -> trans_buf_desc.drv_buf_used &&
	    mcb_channel -> multi_buf_lock == trans_buf_p -> trans_buf_desc.bp) {*/
	if (!trans_buf_p -> trans_buf_desc.drv_buf_used &&
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_take_new_trans successed for channel %d\n", channel);
	return (0);
}
...
//	buf_t			*bp = NULL;
     ...
     ME90_LOG(state, ME90_DL_TRACE,
             "mckp_start_new_trans started for channel %d\n",
             channel
            );
	channel_state = &state -> all_channels_state[channel];
        mutex_enter(&state->mutex);				/* start MUTEX */
     if (get_channel_state(state,channel,started_trans_state) != 0)
     {
        mutex_exit(&state->mutex);			/* end MUTEX */
        ME90_LOG(state, ME90_DL_TRACE,
                "mckp_start_new_trans - channel %d busy so far or yet\n",
                 channel
                );
        return;
     }
     if (state -> mp_state != started_mp_state)
     {
        ...
        ME90_LOG(state, ME90_DL_ERROR,
                "mcb_start_new_trans MP driver is in abnormal"
                " state\n"
               );
        mutex_exit(&state->mutex);			/* end MUTEX */
	...
	if (state -> trans_mode_init_error != 0 ||
	    channel_state -> timetable_set_error != 0) {
		mutex_exit(&state->mutex);		/* end MUTEX */
		rval = mcb_recover_trans_state(state, 0, 1);
		if (rval != 0) {
			ME90_LOG(state, ME90_DL_ERROR,
				"mckp_start_new_trans cannot recover transfer"
				" state\n");
			if (mckp_free_channel(state, channel,
				aborted_trans_state) != 0) {
				ME90_LOG(state, ME90_DL_TRACE,
					"mckp_start_new_trans - cannot free "
					"channel %d access\n", channel);
			}
			return;
		}
	...
	if (channel_state -> in_progress_start == NULL) {
		...
		channel_state -> in_progress_end -> next_trans_buf =
			new_trans_buf;
	}
...
/*					new_trans_buf -> trans_buf_desc.bp;*/
	...
	mckp_set_trans_timeout(state,channel,0);
	rval = start_mckp_dma_engine(state,channel);
	...
	if (rval != 0 && rval != EMPRESTART) {
		...
		while (cur_trans_buf != NULL) {
			if (cur_trans_buf == new_trans_buf) {
				...
				channel_state -> in_progress_size --;
				break;
			} else if (cur_trans_buf -> next_trans_buf ==
				   new_trans_buf) {
				cur_trans_buf -> next_trans_buf =
					new_trans_buf -> next_trans_buf;
				if (cur_trans_buf -> next_trans_buf == NULL)
					channel_state -> in_progress_end =
						cur_trans_buf;
				channel_state -> in_progress_size --;
				break;
			}
		...
		if (channel_state -> in_progress_start == NULL)
			mckp_delete_timeout(state, channel);
		mutex_exit(&state->mutex);		/* end MUTEX */
		if (mckp_free_channel(state, channel, aborted_trans_state) != 0) {
			ME90_LOG(state, ME90_DL_TRACE,
				"mckp_start_new_trans - cannot free channel %d"
				" access\n", channel);
		}
		new_trans_buf -> sparc_error_code = rval;
		mckp_finish_trans(state, channel, new_trans_buf, 0, 0);
		ME90_LOG(state, ME90_DL_TRACE,
			"mckp_start_new_trans: start_mckp_dma_engine failed for"
			" channel %d\n", channel);
		me90drv_start_new_trans(state,channel);
		return;
	}
        if (state -> drv_general_modes & MULTI_REQ_CHANNEL_DRV_MODE) {
		if (mckp_free_channel(state, channel, started_trans_state) != 0) {
			ME90_LOG(state, ME90_DL_TRACE,
				"mckp_start_new_trans - cannot free channel %d"
				" access\n", channel);
		}
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_start_new_trans successed for channel %d\n", channel);
     ...
     )
{
     ...
     ME90_LOG(state, ME90_DL_TRACE,"mckp_buf_trans_done started for channel %d\n",
             channel
            );
     ...
     if (channel_state -> multi_buf_lock == 0)
     {
        mutex_exit(&state->mutex);			/* end MUTEX */
        ME90_LOG(state, ME90_DL_TRACE,
                "mckp_buf_trans_done no any lock for channel %d\n",
                channel
               );
        return;
     }
     else if /*(channel_state -> multi_buf_lock != bp)*/
	       (channel_state -> multi_buf_lock != uio_p)
     {
        mutex_exit(&state->mutex);			/* end MUTEX */
        ME90_LOG(state, ME90_DL_TRACE,
                "mckp_buf_trans_done lock of other transfer for channel %d\n",
                channel
               );
        return;
     }
     ...
     ME90_LOG(state, ME90_DL_TRACE,"mckp_buf_trans_done successed for channel %d\n",
             channel
            );
}
        ...
        if (transfer_spec != NULL)
           if (transfer_spec -> trans_res_info != NULL)
              transfer_spec -> trans_res_info -> intr_drq_received =
                 intr_drq_received;
/*	return mcb_dostrategy(cur_transfer -> bp,mcb);*/
	return mckp_dostrategy(cur_transfer -> uio_p,state);
}
     ...
     )
{
	...
	if (mp_timer_intr_spec -> total_intr_num > 1)
	   timer_interval =
              mckp_calculate_work_hr_time(mp_timer_intr_spec -> last_intr_time,
			intr_mp_time);
...
//	int                rval = 0;
	...
	intr_request -> intr_info -> waiting_time = 
	   mckp_calculate_work_hr_time(intr_request -> enqueue_time,
		intr_receiving_time);
...
//	int                rval = 0;
	...
	mp_timer_intr_info -> waiting_time = 
	   ...
	   mckp_calculate_work_hr_time(request_receiving_time,
     ...
     )
{
	...
	dbgmckp("get_channel_state for channel %d started\n", channel);
        ME90_LOG(state, ME90_DL_TRACE,"get_channel_state for channel %d started\n",
                channel
               );
	/*
...
/*		    new_trans_buf -> trans_buf_desc.bp !=
 	...
 	*/
        ...
        ME90_LOG(state, ME90_DL_TRACE,"get_channel_state for channel %d successed\n",
                channel
               );
	dbgmckp("get_channel_state for channel %d successed\n",
                channel
               );
	return 0;
}
...
int
mckp_mmap(struct file *file, struct vm_area_struct *vma)
{
	mcb_state_t	*state = (mcb_state_t *)file->private_data;
	dev_t		dev;
	int			instance = 0;
	...
	ME90_LOG(NULL, ME90_DL_TRACE,"mckp_mmap started\n");
	...
	ME90_LOG(NULL, ME90_DL_REGS_MAP,
		"mckp_mmap instance %d channel %d started with off 0x%x\n",
		instance,channel,
		off
		);
#endif /* DEBUG */
	...
	if ( off >= MC_EPROM_REG_SET_OFFSET                         &&
		...
		ME90_LOG(state, ME90_DL_REGS_MAP,"mckp_mmap : register set is EPROM\n");
		...
		if ( state->MC_EPROM_CADDR != NULL ) { 	/* With eprom */
			...
			if ( mckp_io_remap_page(state, 0, reg_set_offset, reg_set_len, vma) ) {
				printk ("Error map renge array \n");
				return -EAGAIN;
			}
		} else {				/* Without eprom */
			ME90_LOG(state, ME90_DL_ERROR,
				"mckp_mmap - invalid register set off 0x%x\n",
				off
				);

			return -1;
		}
	} else if ( off >= MC_CNTR_ST_REG_SET_OFFSET                          &&
				off < MC_CNTR_ST_REG_SET_OFFSET + MC_CNTR_ST_REG_SET_LEN ) {
		ME90_LOG(state, ME90_DL_REGS_MAP,
			"mckp_mmap : register set is GEN REGS\n"
			);
		...
		if ( state->MC_EPROM_CADDR != NULL ) { 	/* With eprom */
			if ( mckp_io_remap_page(state, 1, reg_set_offset, reg_set_len, vma) ) {
				printk ("Error map renge array \n");
    			return -EAGAIN;
			}
		} else {				/* Without eprom */
	...
	} else if ( off >= MC_BMEM_REG_SET_OFFSET                       &&
                  off < MC_BMEM_REG_SET_OFFSET + MC_BMEM_REG_SET_LEN ) {
		ME90_LOG(state, ME90_DL_REGS_MAP,"mckp_mmap : register set is BMEM\n");
		reg_set_offset = off - MC_BMEM_REG_SET_OFFSET;
		reg_set_len = MC_BMEM_REG_SET_LEN;
		vma->vm_flags |= (VM_IO | VM_LOCKED | VM_READ | VM_WRITE );

		if ( state->MC_EPROM_CADDR != NULL ) { 	/* With eprom */
			if ( mckp_io_remap_page(state, 2, reg_set_offset, reg_set_len, vma) ) {
				printk ("Error map renge array \n");
				return -EAGAIN;
			}
		} else {				/* Without eprom */
...
#ifdef DEBUG 
	printk (" mmap_mckp succeeded\n");
#endif

	ME90_LOG(state, ME90_DL_REGS_MAP,
		"mckp_mmap : successed for 0x%x + 0x%x\n",
	...
	switch ( state -> type_unit ) {
		...
		case MCPM_UT :
			if ( state -> MC_EPROM_CADDR != NULL ) {
				...
				mckp_iounmap(state, 0, state->MC_EPROM_CADDR);
				state -> MC_EPROM_CADDR = NULL;
				i_reg_gr ++;
			}
			...
			if ( state -> MC_CNTR_ST_REGS != NULL ) {
				...
				mckp_iounmap(state, 1, state->MC_EPROM_CADDR);
				state -> MC_CNTR_ST_REGS = NULL;
				i_reg_gr ++;
			}
			...
			if ( state -> MC_BMEM != NULL ) {
				...
				mckp_iounmap(state, 2, state->MC_EPROM_CADDR);
				state -> MC_BMEM = NULL;
				i_reg_gr ++;
			}
		...
		default      :
			ME90_LOG(state, ME90_DL_ERROR,
				"Unmap_reg_sets : bad board type %d\n",
				state -> type_unit
				);
	}
...
mcb_init_drv_state(mcb_state_t	*state)
{
	...
	ME90_LOG(state, ME90_DL_TRACE, "mckp_init_drv_state started");
	...
	ME90_LOG(state, ME90_DL_TRACE, "mckp_init_drv_state succeeded");
}
 ...
 * Free additional resources allocated in mcb_attach
 */
/*ARGSUSED*/
int
mckp_dodetach_add(mcb_state_t	*state)
{
	int           cur_chnl = 0;
	
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_dodetach_add started\n");
	...
	for (cur_chnl = 0; cur_chnl < MAX_MC_BOARD_CHANNEL_NUM; cur_chnl ++) {
		...
		channel_state = &state -> all_channels_state[cur_chnl];
        	...
        	if (channel_state -> multi_buf_lock != NULL)
			ME90_LOG(state, ME90_DL_ERROR,
				"mckp_dodetach_add: detach busy or with "
				"transfers channel %d\n", cur_chnl);
     };
	/*
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_delete_trans_header Driver detached\n");
 ...
 */
static irqreturn_t
mckp_intr_thread_handler(int irq, void *arg)
{
	mcb_state_t *		state = (mcb_state_t *)arg;
	me90drv_chnl_state_t *	channel_state = NULL;
...
#if 0
     daemonize("mckp_intr_handlerd");

     do
     {
waiting_mode:
	ME90_LOG(state, ME90_DL_TRACE, "MCKP: waiting for interrupt, in_interrupt = %ld !!!!\n", in_interrupt());
	current->policy = SCHED_FIFO;
	e2k_cli();
	state -> wakeup_flag = 1; // Handler is ready
	interruptible_sleep_on(&state->state_mckp_intr_handler);

	if (state->waking_up_mckp_intr_handler == 1){
		dbgmckp(KERN_ALERT "waking_up_mckp_intr_handler !!!\n");
		state->waking_up_mckp_intr_handler = 0;
		break;
	}
	if (state->state_mckp_intr_handler_shutdown == 1) {
		ME90_LOG(state, ME90_DL_TRACE, "MCKP: mckp_intr handler exit by signal\n");
		state->state_mckp_intr_handler_shutdown = 0; 
		return 0;
	}
     }  while (1);
#endif
	...
	switch ( intr_reason ) {
		...
		ME90_LOG(state, ME90_DL_TRACE,"mckp_intr reject interrupt\n");
//	   goto waiting_mode; 
			return IRQ_HANDLED;
		...
		case dma_trans_halt_intr_reason :
			...
			ME90_LOG(state, ME90_DL_TRACE,
				"mckp_intr takes DMA channel %d transfer end interrupt\n",
				channel
				); /* !!!!! */
			channel_state = & state -> all_channels_state[channel];
			mckp_terminate_dma_trans(state,channel);
			...
			ME90_LOG(state, ME90_DL_TRACE,
				"mckp_intr DMA channel %d transfer end interrupt claimed\n",
				channel
				); /* !!!!! */
//	   goto waiting_mode;
			return IRQ_HANDLED;
		case drq_receive_intr_reason :
			...
			ME90_LOG(state, ME90_DL_TRACE,
				"mckp_intr received DRQ from channel %d interrupt\n",
				channel
				);
...
//	   goto waiting_mode;
        ...
        case board_error_intr_reason :
			ME90_LOG(state, ME90_DL_TRACE,
				"mckp_intr - board internal error interrupt reason\n"
				);
//	   goto waiting_mode;
			return IRQ_HANDLED;
		case mp_timer_expired_intr_reason :
			ME90_LOG(state, ME90_DL_TRACE,"mckp_intr MP timer expired interrupt\n");
...
//	   goto waiting_mode;
		...
		case aborted_intr_reason :
			ME90_LOG(state, ME90_DL_TRACE,
				"mckp_intr - aborted transfer interrupt reason\n"
				);
//	   goto waiting_mode;
			return IRQ_HANDLED;
		case init_trans_mode_end_intr_reason :
			...
			state -> init_trans_modes_spec.mckp.mp_error_code =
			...
			if ( interrupt_args.init_trans_results.mp_error_code != 0 ) {
				...
				ME90_LOG(state, ME90_DL_ERROR,
					"mckp_intr - init transfer modes finished with error"
...
//	   goto waiting_mode;
		...
		case cnct_polling_bad_intr_reason  :
			...
			ME90_LOG(state, ME90_DL_TRACE,
				"mckp_intr connection polling %s interrupt at time %s\n",
				(intr_reason == cnct_polling_good_intr_reason) ? "good" : "bad"
				);

			mckp_connection_polling_intr(state, intr_reason == cnct_polling_bad_intr_reason, intr_receiving_time);
//	   goto waiting_mode;
			return IRQ_HANDLED;
...
irqreturn_t
mckp_interrupt(int irq, void* arg)
{
	mcb_state_t *	state = (mcb_state_t *)arg;
...
#if 0
	if (state -> wakeup_flag == 0){ /* Handler is not ready */
		mutex_exit(&state->mutex);
		return (IRQ_NONE);
	}
#endif     
...
//    	state->waking_up_mckp_intr_handler = 1;
...
//   	wake_up(&state->state_mckp_intr_handler);
//	state -> wakeup_flag = 0;
//	mutex_exit(&state->mutex); 
	return IRQ_HANDLED;
     }
    ...
    )
{
     ...
     mutex_enter(&state->mutex);			/* start MUTEX */
//   read_value.RGEN_read = mcb -> MC_CNTR_ST_REGS -> MC_TI_read;
	...
	trans_buf_t	*trans_buf_p)
{
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_release_async_trans started for channel %d\n", channel);
	...
	if (trans_buf_p -> trans_buf_desc.drv_buf_used &&
		...
		mckp_delete_drv_buf(state, trans_buf_p -> drv_buf_p);
		trans_buf_p -> drv_buf_p = NULL;
	} /*else if (trans_buf_p -> trans_buf_desc.bp != NULL) {
		mcb_buf_trans_done(mcb, channel,
		...
		mckp_buf_trans_done(state, channel,
			trans_buf_p -> trans_buf_desc.uio_p);
		transfer_spec = trans_buf_p -> trans_buf_desc.uio_p -> transfer_spec;
		kmem_free(trans_buf_p -> trans_buf_desc.uio_p -> uio_iov,sizeof(iovec_t));
		kmem_free(trans_buf_p -> trans_buf_desc.uio_p, sizeof(uio_t));
		trans_buf_p ->trans_buf_desc. uio_p = NULL;
	} else {
		if (trans_buf_p -> trans_buf_desc.drv_buf_used) {
			ME90_LOG(state, ME90_DL_ERROR,
				"mckp_release_async_trans NULL pointer of drv"
				" buf in trans buf 0x%x\n", trans_buf_p);
		} else {
			ME90_LOG(state, ME90_DL_ERROR,
				"mckp_release_async_trans NULL pointer of bp"
				" in trans buf 0x%x\n", trans_buf_p);
		}
	}
	if (transfer_spec != NULL) {
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_release_async_trans finished for channel %d\n", channel);
}
...
/*   buf_t *            bp = NULL;*/
     ...
     ME90_LOG(state, ME90_DL_TRACE,"start_mckp_dma_engine started for channel %d\n",
             channel
            );
     mutex_enter(&state->mutex);			/* start MUTEX */
     cur_trans_buf = channel_state -> in_progress_start;
...
/*      bp = cur_trans_buf -> trans_buf_desc.bp;
	...
	trans_buf_size = uio_p->uio_iov[0].iov_len;
     ...
     {
        ME90_LOG(NULL, ME90_DL_TRACE,
                "start_mckp_dma_engine channel %d mckp_get_burst_sizes"
                " failed\n",
                channel
               );
        return rval;
     ...
     {
        ME90_LOG(state, ME90_DL_ERROR,
                "start_mckp_dma_engine - cannot find right burst size for"
	...
	if (trans_res_info != NULL)
     ...
     {
        ME90_LOG(state, ME90_DL_ERROR,
                "start_mckp_dma_engine - bad I/O address and size alignment"
                " burst sizes 0x%x bytes, address 0x%08x size 0x%x\n",
                allowed_burst,
     ...
     ME90_LOG(state, ME90_DL_TRACE,
             "start_mckp_dma_engine transaction burst size %d bytes\n",
             allowed_burst
            );
...
/*     if (bp != NULL)
     ...
     {
        ME90_LOG(state, ME90_DL_ERROR,
                "start_mckp_dma_engine - bad I/O operation for channel %d\n",
                channel
               );
        return EINVAL;
     }
     ...
     {
        ME90_LOG(state, ME90_DL_ERROR,
                "start_mckp_dma_engine - bad I/O operation b_flags 0x%x",
                op_flags
               );
        return EINVAL;
     }
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"start_mckp_dma_engine - data cache flush has been done"
...
/*   transfer_args.transfer.address = cur_trans_buf -> trans_buf_desc.cookie.dmac_address;*/
     ...
     dbgmckp("start_mckp_dma_engine: transfer_args.transfer.address = 0x%lx, transfer_args.transfer.size = 0x%x\n",
		(u_long)transfer_args.transfer.address, transfer_args.transfer.size);
     if (transfer_spec == NULL)
     {
        transfer_args.transfer.timing_interval_t0 = 0;
     ...
     else
     {
        ...
        else if (transfer_spec -> io_mode_flags & TIMING_TRANSFER_IO_MODE)
        {
           transfer_args.transfer.mode = timing_trans_mode;
           transfer_args.transfer.timing_interval_t0 = transfer_spec -> timing_interval_t0;
        }

        else
        {
           ME90_LOG(state, ME90_DL_ERROR,
                   "start_mckp_dma_engine - bad I/O transfer mode 0x%x\n",
     ...
     {
        ME90_LOG(state, ME90_DL_ERROR,
                "start_mckp_dma_engine - bad I/O device access mode 0x%x\n",
                dev_access_mode
               );
        return EINVAL;
     }
     ME90_LOG(state, ME90_DL_TRACE,"start_mckp_dma_engine finished for channel %d\n",
             channel
            );
     return rval;
}
...
//   buf_t *		bp = NULL;
     ...
     ME90_LOG(state, ME90_DL_TRACE,"finish_mckp_dma_engine started for channel %d\n",
             channel
            );
     source_size = trans_buf_p -> trans_size;
...
/*      bp = trans_buf_p ->trans_buf_desc. bp;
	...
	write_op_flag = uio_p -> op_flags;
     ...
     if (transfer_spec != NULL)
     {
        if (transfer_spec -> io_mode_flags & BMEM_TRANSFER_IO_MODE)
        {
           ...
           transfer_size = source_size -
                           transfer_size * transfer_spec ->
				trans_res_info -> burst_byte_size;
           trans_buf_p -> real_trans_size = transfer_size;
        }
     }
     if (transfer_size > source_size)
     {
           bad_trans_size = 1;
        ME90_LOG(state, ME90_DL_ERROR,
                "finish_mckp_dma_engine: I/O transfer real size %d > "
                " source size %d\n",
                transfer_size,
                source_size
               );
...
/*        else if (bp != NULL)*/
        ...
        {
           do_sync = (/*bp -> b_flags*/uio_p -> op_flags & B_READ);
           if (do_sync)
				sync_rval = mckp_dma_sync(state,
					trans_buf_p->trans_buf_desc.dma.prim_dev_mem,
					trans_buf_p->trans_buf_desc.uio_p->
        ...
        {
           ME90_LOG(state, ME90_DL_ERROR,
                   "finish_mckp_dma_engine - ddi_dma_sync failed for channel"
                   " %d\n",
                    channel
                   );
           sync_rval = EFAULT;
...
/*        if (bp != NULL)
	...
	if (drv_buf_p != NULL)
           mckp_drv_buf_io_error(trans_buf_p,rval);
        if (trans_buf_p -> sparc_error_code != 0     &&
            trans_buf_p -> sparc_error_code != ETIME
           )
        {
           ME90_LOG(state, ME90_DL_ERROR,
                   "channel %d I/O transfer finished with error %d"
     ...
     {
        mckp_set_dma_trans_results(state,channel,trans_buf_p,transfer_spec,-1);
	if (!transfer_spec -> async_trans)
        	(/*bp*/ uio_p != NULL) ? /*biodone(bp)*/ :
			mckp_drv_buf_io_done(state, trans_buf_p, mutex_locked);
     }

     ME90_LOG(state, ME90_DL_TRACE,
             "finish_mckp_dma_engine I/O transfer successed for channel %d\n",
             channel
            );
}
     ...
     )
{
     ...
     ME90_LOG(state, ME90_DL_TRACE,
             "mckp_set_dma_trans_results started for channel %d trans_buf 0x%08x"
             " trans_spec 0x%08x\n",
             channel,
             trans_buf_p,
             transfer_spec
	...
	trans_res_info -> trans_num = trans_buf_p -> trans_num;
     ...
     ME90_LOG(state, ME90_DL_TRACE,
             "mckp_set_dma_trans_results successed for channel %d trans_buf 0x%08x"
             " trans_spec 0x%08x\n",
             channel,
             trans_buf_p,
             transfer_spec
	...
	int		trans_aborted)
{
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_handle_trans_finish started for channel %d\n", channel);
	channel_state = &state -> all_channels_state[channel];
	cur_trans_buf = channel_state -> in_progress_start;
	if (cur_trans_buf == NULL) {

		ME90_LOG(state, ME90_DL_ERROR,
			"mckp_handle_trans_finish no any transfer in progress"
			" for channel %d\n", channel);
		return;
	}
	if (trans_results != NULL) {
		...
		if (trans_results -> mp_error_code == ADAPTER_ABEND_MCKP_ERROR
		    &&
		    (state -> drv_general_modes & RETRIEVE_ADPT_ABEND_DRV_MODE)) {
			if (state -> mp_state != crash_mp_state) {
				ME90_LOG(state, ME90_DL_ERROR,
					"mcb_handle_trans_finish adapter abend"
					" occured : SB=0x%02x"
					" SPSB=0x%02x"
					"\n",
					trans_results -> state_byte
					,trans_results -> sp_state_byte
					);
		...
		cur_trans_buf -> sp_state_byte = trans_results -> sp_state_byte;
	...
	if (channel_state -> in_progress_start != NULL) {
		if (!channel_state -> streaming)
			mckp_set_trans_timeout(state, channel, 1);
	...
	if (!(state -> drv_general_modes & MULTI_REQ_CHANNEL_DRV_MODE) &&
		...
		mckp_set_transfer_done(state, channel, completed_trans_state);
	else {
		cv_broadcast(&state -> channel_cv);
	}
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_handle_trans_finish successed for channel %d\n", channel);
}



/*
 * Init transfers mode, mutex must be locked by caller
 */
...
mcb_init_trans_mode(
	...
	int		drv_comm_area_locked)
{
     mp_drv_args_t	init_trans_modes_args;
     u_long		cur_clock_ticks = 0;
     u_long		timeout_clock_ticks = 0;
     sparc_drv_args_t	init_mode_results;
     int                rval = 0;

     ME90_LOG(state, ME90_DL_TRACE,	/* !!!!!!!! */
             "mcb_init_trans_mode started\n"
             );

     state -> trans_mode_inited = 0;
     state -> trans_mode_init_error = 0;

     state -> init_trans_modes_spec.mckp.mp_error_code = 0;
     init_trans_modes_args.init_trans_modes.offset =
        state -> init_trans_modes_spec.mckp.offset;
     init_trans_modes_args.init_trans_modes.test_mode =
        state -> init_trans_modes_spec.mckp.test_mode_flag;
     rval = submit_mp_task(state,init_trans_mode_mp_task,
                           &init_trans_modes_args,
                           1,
                           NULL,
                           &init_mode_results,
                           drv_comm_area_locked
                          );
     if (rval != 0)
     {
        state -> trans_mode_inited = 1;
     ...
     drv_getparm(LBOLT,&cur_clock_ticks);
     timeout_clock_ticks =
        cur_clock_ticks + drv_usectohz(INIT_TRANS_MODE_TIMEOUT_DEF);
     while (!state -> trans_mode_inited)
     {
          rval =
           cv_timedwait(&state -> channel_cv,
                        &state->mutex,
                        timeout_clock_ticks
                       );
//	  rval = cv_spin_timedwait(&state -> channel_cv,   &state->lock,timeout_clock_ticks);	
        ...
        {
           state -> trans_mode_inited = 1;
           state -> trans_mode_init_error = ETIME;
           ME90_LOG(state, ME90_DL_ERROR,
                   "init transfer modes: waiting for end of init transfer modes"
                   " timeouted\n"
                  );
           break;
        }
     ...
     ME90_LOG(state, ME90_DL_TRACE,"mcb_init_trans_mode finished \n");
     return state -> trans_mode_init_error;
}


/*
 * Set timetable of work, mutex must be locked by caller
 */

/*ARGSUSED*/
int
mcb_set_work_timetable(
	mckp_state_t	*state,
	int		channel,
	int		drv_comm_area_locked)
{
     ...
     ME90_LOG(state, ME90_DL_TRACE,
             "mcb_set_work_timetable started for channel %d\n",
             channel
            );
     channel_state = &state -> all_channels_state[channel];
     channel_state -> timetable_seted = 0;
     channel_state -> work_timetable.set_work_timetable.dev_num = channel;
     rval = submit_mp_task(state,
                           set_timetable_mp_task,
                           &channel_state -> work_timetable,
                           1,
                           NULL,
                           NULL,
                           drv_comm_area_locked
     ...
     if (rval != 0)
     {
        channel_state -> timetable_seted = 1;
        channel_state -> timetable_set_error = rval;
        ME90_LOG(state, ME90_DL_ERROR,
                "mcb_set_work_timetable: set timetable mask for channel %d"
                " failed\n",
                channel
               );
        return rval;
     ...
     channel_state -> timetable_seted = 1;
     channel_state -> timetable_set_error = 0;
     ME90_LOG(state, ME90_DL_TRACE,"mcb_set_work_timetable finished\n");
     return rval;
}
	...
	cnct_poll_set_t	*polling_setup_spec)
{
     ...
     ME90_LOG(state, ME90_DL_TRACE,"mckp_set_connection_polling started "
             "interval %d cpu %d\n",
     ...
     if (state -> connection_state & MODE_ON_CONNECTION_STATE)
     {
        mutex_exit(&state->mutex);			/* end MUTEX */
        ME90_LOG(state, ME90_DL_ERROR,
                "mckp_set_connection_polling polling mode is set already\n"
               );
        return EINVAL;
     }
     ...
     if (polling_setup_spec -> connection_events_num > 0)
     {
        ...
        {
           ME90_LOG(state, ME90_DL_ERROR,
                   "mckp_set_connection_polling events buffer is created"
                   " already\n"
                  );
        }
        else
        {
           ...
           {
              ME90_LOG(state, ME90_DL_ERROR,
                      "mckp_set_connection_polling cannot allocate kernel"
     ...
     if (rval != 0)
     {
        ...
        mutex_exit(&state->mutex);			/* end MUTEX */
        ME90_LOG(state, ME90_DL_ERROR,
                "mckp_set_connection_polling: mp task failed\n"
               );
        return rval;
     }
     state -> connection_state |= MP_TAKE_CONNECTION_STATE;
...
//	   rval = cv_spin_wait(&state -> cnct_polling_cv, &state->lock);	
     ...
     if (rval != 0)
     {
        ...
        ME90_LOG(state, ME90_DL_TRACE,"mckp_set_connection_polling finished with"
                                  " reset connection\n"
               );
        return rval;
     }
     ...
     {
        ME90_LOG(state, ME90_DL_ERROR,
                "mckp_set_connection_polling finished with bad connection state"
                " 0x%x\n",
     ...
     mutex_exit(&state->mutex);				/* end MUTEX */
     ME90_LOG(state, ME90_DL_TRACE,"mckp_set_connection_polling finished \n");
     return rval;
}
	...
	int		reset_error)
{
     ...
     ME90_LOG(state, ME90_DL_TRACE,"mckp_reset_connection_polling started \n");
     ...
     if (!(state -> connection_state & MODE_ON_CONNECTION_STATE))
     {
        mutex_exit(&state->mutex);			/* end MUTEX */
        ME90_LOG(state, ME90_DL_ERROR,
                "mckp_reset_connection_polling polling mode is reset already\n"
               );
        return EINVAL;
     }
     if (state -> connection_state & MP_TAKE_CONNECTION_STATE)
     {
        ...
        {
           ME90_LOG(state, ME90_DL_ERROR,
                   "mckp_reset_connection_polling: mp task failed\n"
                  );
        }
        else
        {
     ...
     mutex_exit(&state->mutex);				/* end MUTEX */
     ME90_LOG(state, ME90_DL_TRACE,"mckp_reset_connection_polling finished \n");
     return rval;
}
	...
	poll_cnct_state_t	*state_spec)	/* requested connection state */
{
     ...
     ME90_LOG(state, ME90_DL_TRACE,"mckp_poll_connection_state started\n");
     ...
     if (state_mask == 0)
     {
        ...
        mutex_exit(&state->mutex);			/* end MUTEX */
        ME90_LOG(state, ME90_DL_TRACE,
                "mckp_poll_connection_state polling mode with empty state mask\n"
               );
        return 0;
     }
     if (!(state -> connection_state & MODE_ON_CONNECTION_STATE) ||
        ...
        )
     {
        ...
        mutex_exit(&state->mutex);			/* end MUTEX */
        ME90_LOG(state, ME90_DL_TRACE,
                "mckp_poll_connection_state polling mode is not set\n"
               );
        return EINTR;
     }
     ...
     if (waiting_time == 0)
     {
        ...
        mutex_exit(&state->mutex);			/* end MUTEX */
        ME90_LOG(state, ME90_DL_TRACE,
                "mckp_poll_connection_state null waiting time\n"
               );
        return rval;
     }
     if (waiting_time != -1)
...
//	rval = cv_spin_wait(&state -> cnct_polling_cv, &state->lock);
        if (rval < 0)
        {
           rval = ETIME;
           ME90_LOG(state, ME90_DL_TRACE,
                   "mckp_poll_connection_state: waiting for state"
                   " timeouted\n"
                  );
           break;
        }
     ...
     mutex_exit(&state->mutex);				/* end MUTEX */
     ME90_LOG(state, ME90_DL_TRACE,"mckp_poll_connection_state finished \n");
     return rval;
}
     ...
     )
{
     ...
     ME90_LOG(state, ME90_DL_TRACE,"mckp_connection_polling_intr started \n");
     ...
     if (!(connection_state & MODE_ON_CONNECTION_STATE))
     {
        mutex_exit(&state->mutex);			/* end MUTEX */
        ME90_LOG(state, ME90_DL_ERROR,
                "mckp_connection_polling_intr polling is not set\n"
               );
        return;
     }
     if (connection_state & IS_SET_CONNECTION_STATE)
     {
        if (!connection_refused)
        {
           if (connection_state & ALIVE_CONNECTION_STATE)
           {
              ...
              mutex_exit(&state->mutex);			/* end MUTEX */
              ME90_LOG(state, ME90_DL_TRACE,
                      "mckp_connection_polling_intr good state continues\n"
                     );
              return;
           }
           else if (connection_state & REFUSED_CONNECTION_STATE)
           {
              ...
              ME90_LOG(state, ME90_DL_TRACE,
                      "mckp_connection_polling_intr recovery of alive state\n"
                     );
           }
           else
           {
              ME90_LOG(state, ME90_DL_ERROR,
                      "mckp_connection_polling_intr invalid state # 1\n"
                     );
              return;
           }
        }
        else if (connection_state & ALIVE_CONNECTION_STATE)
        {
           ...
           ME90_LOG(state, ME90_DL_TRACE,
                   "mckp_connection_polling_intr goto connection refused state\n"
                  );
        }
        else if (connection_state & REFUSED_CONNECTION_STATE)
        {
           ...
           mutex_exit(&state->mutex);			/* end MUTEX */
           ME90_LOG(state, ME90_DL_TRACE,
                   "mckp_connection_polling_intr refused state continues\n"
                  );
           return;
        }
        else
        {
           ME90_LOG(state, ME90_DL_ERROR,
                   "mckp_connection_polling_intr invalid state # 2\n"
                  );
           return;
        }
     }
     else
     {
        if (!connection_refused)
        {
           ...
           ME90_LOG(state, ME90_DL_TRACE,
                   "mckp_connection_polling_intr initial connection is set\n"
                  );
        }
        else
        {
           ...
           mutex_exit(&state->mutex);			/* end MUTEX */
           ME90_LOG(state, ME90_DL_TRACE,
                   "mckp_connection_polling_intr initial connection setting"
                   " interrupted\n"
                  );
           return;
        }
     ...
     mutex_exit(&state->mutex);				/* end MUTEX */
     ME90_LOG(state, ME90_DL_TRACE,"mckp_connection_polling_intr finished \n");
     return;
}
...
mcb_delete_connection_polling(
	...
	int		reset_error)
{
     ME90_LOG(state, ME90_DL_TRACE,"mcb_delete_connection_polling started \n");
     ...
     {
        ME90_LOG(state, ME90_DL_TRACE,
                "mcb_delete_connection_polling polling mode is not set\n"
               );
     }
     state -> connection_state &= ~MODE_ON_CONNECTION_STATE;
     state -> connection_state |= MODE_OFF_CONNECTION_STATE;
     ...
     cv_broadcast(&state -> cnct_polling_cv);
     ME90_LOG(state, ME90_DL_TRACE,"mcb_delete_connection_polling successed \n");
     return;
}
	...
	int		mutex_locked)
{
	...
	ME90_LOG(state, ME90_DL_TRACE,"mckp_recover_trans_state started \n");

	for (cur_chnl = 0; cur_chnl < MAX_MC_BOARD_CHANNEL_NUM; cur_chnl ++) {
		cur_channel_state = &state -> all_channels_state[cur_chnl];
		if (!mutex_locked)
			mutex_enter(&state->mutex);	/* start MUTEX */
		if (cur_channel_state -> timetable_seted ||
		    cur_channel_state -> timetable_set_error != 0) {
			rval = mcb_set_work_timetable(state, cur_chnl, drv_comm_area_locked);
			if (rval != 0) {
				if (!mutex_locked)
           				mutex_exit(&state->mutex); /* MUTEX */
				ME90_LOG(state, ME90_DL_ERROR,
					"mckp_recover_trans_state: set work"
					" timetable failed for channel %d\n",
					cur_chnl);
				return rval;
			}
		}
	...
	if (!mutex_locked)
		mutex_enter(&state->mutex);		/* start MUTEX */
	if (state -> trans_mode_inited) {
		rval = mcb_init_trans_mode(state,drv_comm_area_locked);
		if (rval != 0) {
			if (!mutex_locked)
				mutex_exit(&state->mutex); /* end of MUTEX */
			ME90_LOG(state, ME90_DL_ERROR,
				"mckp_recover_trans_state: mckp_init_trans_mode"
				" failed\n");
			return rval;
		}
	}
	...
	ME90_LOG(state, ME90_DL_TRACE, "mckp_recover_trans_state finished\n");
	return rval;
}
...
/*ARGSUSED*/
static void mckp_init_trans_results(
     trans_spec_t *	transfer_spec
     )
{
     trans_info_t *	trans_res_info = NULL;
     ...
     ME90_LOG(NULL, ME90_DL_TRACE,
             "mckp_init_trans_results started for trans_spec 0x%08x\n",
             transfer_spec
            );
     if (transfer_spec != NULL)
     {
     ...
     ME90_LOG(NULL, ME90_DL_TRACE,
             "mckp_init_trans_results successed for trans_spec 0x%08x\n",
             transfer_spec
            );
}
	...
	trans_buf_t	**new_trans_buf_p)
{
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_create_trans_header started with buffer byte size %d\n",
		buf_byte_size);

	new_trans_buf = kmem_alloc(sizeof(trans_buf_t),KM_NOSLEEP);
	if (new_trans_buf == NULL) {
		ME90_LOG(state, ME90_DL_ERROR,
			"mckp_create_trans_header cannot allocate kernel"
			" memory\n");
		return EINVAL;
	}
	new_trans_buf -> next_trans_buf = NULL;
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! */
		...
		mckp_init_trans_header(state, new_trans_buf);
		*new_trans_buf_p = new_trans_buf;
		ME90_LOG(state, ME90_DL_TRACE,
			"mckp_create_trans_header successed with only link of"
			" buffer\n");
		return 0;
	}
	dbgmckp("mckp_create_trans_header: buf_byte_size (bytes) = 0x%x\n", buf_byte_size);
	...
	rval = mckp_alloc_trans_bufs(state, &new_trans_buf -> trans_buf_desc,
			buf_byte_size, flags);
	if (rval != 0) {
		kmem_free(new_trans_buf, sizeof(trans_buf_t));
		ME90_LOG(state, ME90_DL_TRACE,
			"mckp_create_trans_header failed with buffer byte "
			"size %d\n", buf_byte_size);
		return rval;
	}
	mckp_init_trans_header(state, new_trans_buf);
	*new_trans_buf_p = new_trans_buf;
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_create_trans_header successed with buffer byte size %d\n",
		buf_byte_size);
	return 0;
}
...
{
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_delete_trans_header started for buffer 0x%08x\n",
		trans_buf_p);

	if (!trans_buf_p -> trans_buf_desc.only_link)
		mckp_free_trans_bufs(state, &trans_buf_p -> trans_buf_desc);
	kmem_free(trans_buf_p, sizeof(trans_buf_t));
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_delete_trans_header succeeded\n");
}


/*ARGSUSED*/
void
mckp_fill_trans_bufs(
	mcb_state_t		*state,
	me90drv_trbuf_desc_t	*trans_buf_desc)
{
	ME90_LOG(NULL, ME90_DL_TRACE,
		"mckp_fill_trans_bufs started for buffer 0x%08x\n",
		trans_buf_desc);
	...
	ME90_LOG(NULL, ME90_DL_TRACE,
		"mckp_fill_trans_bufs succeeded for buffer 0x%08x\n",
		trans_buf_desc);
}
...
/*ARGSUSED*/
static	void
mckp_init_trans_header(
	mcb_state_t		*state,
	trans_buf_t		*trans_buf_p)
{
	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_init_trans_header started for buffer 0x%08x\n",
		trans_buf_p);
	...
	mckp_fill_trans_bufs(state, &trans_buf_p -> trans_buf_desc);

	ME90_LOG(state, ME90_DL_TRACE,
		"mckp_init_trans_header succeeded for buffer 0x%08x\n",
 		trans_buf_p);
}
     ...
     )
{
     ...
     ME90_LOG(state, ME90_DL_TRACE,
             "mckp_create_drv_buf started with uio_p 0x%08x\n",
             uio_p
            );
     ...
     {
        ME90_LOG(state, ME90_DL_ERROR,
                "mckp_create_drv_buf cannot allocate kernel memory"
               );
        return EINVAL;
     }
     ...
     *new_trans_drv_buf_p = new_trans_drv_buf;
     ...
     ME90_LOG(state, ME90_DL_TRACE,
             "mckp_create_drv_buf successed for uio_p 0x%08x\n",
             uio_p
            );
     return 0;
}
...
{
     ME90_LOG(state, ME90_DL_TRACE,
             "mckp_delete_drv_buf started for buffer 0x%08x\n",
             trans_drv_buf_p
            );
     cv_destroy(&trans_drv_buf_p -> trans_finish_cv);
     kmem_free(trans_drv_buf_p,sizeof(mcb_drv_buf_t));
     ME90_LOG(state, ME90_DL_TRACE,"mckp_delete_drv_buf successed\n");
}
...
/*******************************************************************************/
/*
 * Create device transfer control words for DMA transfer
 */
...
static	int	mckp_init_minor_created = 0;

/*
 * Driver chpoll entry point
 */
/*ARGSUSED*/
unsigned int mckp_chpoll(struct file *file, struct poll_table_struct *wait)
{
     ME90_LOG(NULL, ME90_DL_TRACE,"mckp_chpoll called, but not implemented\n");
     return  ENXIO;
}
     ...
     )
{
	...
	int			rval = 0;
     ...
     dbgmckp("submit_mp_task started with task # %d\n",
             mp_task
            );
     drv_communication =
        (drv_intercom_t *) &state -> MC_BMEM[TR_CNTR_BUF_BMEM_ADDR];
     switch (mp_task)
     {
        ...
        case init_trans_mode_mp_task :
           args_num =
              (sizeof(init_trans_mode_t) +
               (sizeof(*drv_communication -> mp_args.args_area)-1)
              ) / sizeof(*drv_communication -> mp_args.args_area);
           if (mp_task_results != NULL)
              mp_task_results -> init_trans_results.mp_error_code = 0;
           break;
     ...
     dbgmckp("MCKP: submit_mp_task: before spin_mutex_enter, mutex_enter_done = %d\n", mutex_enter_done);
     if (!mutex_enter_done)
        mutex_enter(&state->mutex);			/* start MUTEX */
...
//	cv_spin_wait(&state -> drv_comm_cv, &state->lock);
	...
	dbgmckp("MCKP: submit_mp_task: mp_drv_started = %d, mp_task #%d\n", state -> mp_drv_started, mp_task);
     if (!state -> mp_drv_started && mp_task != drv_load_mp_task)
     {
	dbgmckp("MCKP: submit_mp_task: !state -> mp_drv_started && mp_task != drv_load_mp_task case\n");
        if (!restart)
        {
           state -> drv_comm_busy = 0;
           cv_broadcast(&state -> drv_comm_cv);
	...
	dbgmckp("MCKP: submit_mp_task: mp_task #%d\n", mp_task);
...
//        drv_usecwait(TASK_ACCEPT_BY_MP_DELAY_TIME);
	...
	wait_mp_rom_drv_disable = (mp_task == drv_load_mp_task);
     rval = mckp_interrupt_mp(state,mp_task == drv_load_mp_task,
        ...
        )
     {
        ...
        while (waiting_time < TASK_ACCEPT_BY_MP_TIME && !task_accepted)
        {
           ...
           if (task_accepted)
              break;
/*           delay(drv_usectohz(TASK_ACCEPT_BY_MP_DELAY_TIME)); */
/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!! */
//           drv_usecwait(TASK_ACCEPT_BY_MP_DELAY_TIME);
/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!! */
           waiting_time += TASK_ACCEPT_BY_MP_DELAY_TIME;
...
{
   mc_wr_reg_t    intr_reset_value;
   mc_wr_reg_t    gen_reset_value;
   mc_wr_reg_t    trcwd_gen_reset_value;
...
//   benchmark_value.RTM_read = get_mcb_module_type(mcb -> type_unit);
   benchmark_value.RTM_read = mckp_rtm_encode; 
/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! */
...
/* !!!!!!!!!!!!!!!!!!!! */
   gen_reset_value.RGEN_write = 0;
   trcwd_gen_reset_value.RGEN_write = 0;
   write_all_general_regs(state -> MC_CNTR_ST_REGS,  /* general reset of module */
                      TGRM_mc_reg_type,
                      gen_reset_value,
                      &benchmark_value,
                      trcwd_gen_reset_value
                     );
...
/* !!!!!!!!!!!!!!!!!!!!!! */
   ...
   if (mp_state != 2)
   {
      if (mp_state == 1)
      {
         ...
         mp_init_area -> MP_INIT_AREA_u_long[0] =
            mckp_rotate_word_bytes(MP_HALT_OPCODE);
      };
   ...
   {
#ifdef DEBUG
      ME90_LOG(state, ME90_DL_TRACE,"reset_general_regs finished\n");
#endif
   }
   else
   {
#ifdef DEBUG
      ME90_LOG(state, ME90_DL_TRACE,"reset_general_regs successed\n");
#endif
   }
...
static int mckp_sbus_probe(struct of_device *op, const struct of_device_id *match)
{
	int		i;
	int		instance = mckp_instances++;
	int		minor;
	mcb_state_t		*state = NULL;
	int		rval;
	char	name[64];
	 ...
	 */
CHP
	dbgmckp("%s(): instance %d started\n", __func__, instance);

CHP
	mckp_major = register_chrdev(0, MCKP_NAME, &mckp_fops);
CHP
	if ( mckp_major < 0 ) {
		return mckp_major;
	}
...
CHP
	...
	mckp_states[state->inst] = state;

	dev_set_drvdata(&op->dev, state);

	for ( cur_chnl = 0; cur_chnl < MAX_ME90DRV_BOARD_CHANNEL_NUM; cur_chnl++ ) {
		me90drv_chnl_state_t	*mckp_channel = NULL;

		mckp_channel = &state -> all_channels_state[cur_chnl];
		mckp_channel -> busy = 0;
		mckp_channel -> in_progress = 0;
		mckp_channel -> transfer_state = no_trans_state;
		mckp_channel -> timeout_type = no_timeout_type;
		mckp_channel -> timeout_rem = 0;
		mckp_channel -> wait_list_start = NULL;
		mckp_channel -> wait_list_end = NULL;
		mckp_channel -> wait_list_size = 0;
		mckp_channel -> in_progress_start = NULL;
		mckp_channel -> in_progress_end = NULL;
		mckp_channel -> in_progress_size = 0;
		mckp_channel -> completed_trans_start = NULL;
		mckp_channel -> completed_trans_end = NULL;
		mckp_channel -> completed_trans_size = 0;
		mckp_channel -> ready_atrans_start = NULL;
		mckp_channel -> ready_atrans_end = NULL;
		mckp_channel -> ready_atrans_size = 0;
		mckp_channel -> async_trans_num = 0;
		mckp_channel -> term_trans_processed = 0;
		mckp_channel -> last_term_trans_buf = NULL;
		mckp_channel -> trans_num = 0;
	}

	me90drv_init_drv_state(state);
	state->type_unit  = MC53_UT;

	/*
	 * SBUS and MP clock-frequency definition
	 */

//	me90_sbus_clock_freq = ddi_getprop(DDI_DEV_T_ANY, dip, 0, "clock-frequency", 1);
	me90_sbus_clock_freq = mckp_clock_freq(state);
	dbgmckp("%s(): me90_sbus_clock_freq = %d\n", __func__, me90_sbus_clock_freq);
	if ( me90_sbus_clock_freq < 10 * 1000000 ||
		me90_sbus_clock_freq > 25 * 1000000 ) {
		ME90_LOG(state, ME90_DL_ERROR,
			"%s(): SBus clock frequency %d out "
	...
	for ( i = 0; i < n_regs; i++ ) {
		regs = mckp_ioremap(state, i);
		if ( !me90drv_put_reg_set_pointer(state, i + without_eprom, (caddr_t)regs) ) {
			ME90_LOG(state, ME90_DL_ERROR,
				"%s(): "
	...
	/*
	 * Initialize interrupt and handler for this instance
	 */
...
/* interrupt handler and watchdog threads are defined here */
	...
	init_waitqueue_head(&(state->state_mckp_intr_handler));
	state->pid_state_mckp_intr_handler = kernel_thread(state_mckp_intr_handler, dip, 0);

	init_waitqueue_head(&(state->mckp_watchdog_handler));
	state->pid_mckp_watchdog_handler = kernel_thread(mckp_watchdog_handler, state, 0);
#endif
//	INIT_WORK(&(state->interrupt_tqueue), state_mckp_intr_handler, dip);
	INIT_WORK(&(state->watchdog_tqueue), mckp_watchdog_handler);
/* end of interrupt handler and watchdog routines */

	rval = mckp_request_irq(state);
	if ( rval ) {
		printk("%s(): mcka_request_irq() failed\n", __func__);
		goto m_err;
	}
...
//	mcst_write(INTERRUPT_REG_BASE + INTERRUPT_MASK_CLEAR, bit);
	...
	for ( cur_chnl = mckp_init_minor_created; cur_chnl < MAX_ME90DRV_BOARD_CHANNEL_NUM;  cur_chnl++ ) {
		minor = MCB_MINOR(instance, cur_chnl);

		(void) sprintf(name, "%s_%d_%s:%d", mod_name, instance,
                       (cur_chnl & 0x1) ? "in" : "out", cur_chnl >> 1);


			if ( ddi_create_minor(MCKP_DIR, name, S_IFCHR, new_encode_dev(MKDEV(state->major, minor))) ) {
				ME90_LOG(state, ME90_DL_ERROR,
					"%s(): channel %d :"
					" ddi_create_minor() failed\n", __func__, 
					cur_chnl);
				goto  m_err;
			}
	...
	dbgmckp("%s(): is finished for Driver %s inst %d\n", __func__, MCKP_NAME, state->inst);
...
m_err:
	...
	if ( attach_flags & MUTEX_ADDED ) {
		// TODO: replace mutex_destroy() with what?
...
//		ddi_rm_dir(dip);
	...
	unregister_chrdev(mckp_major, MCKP_NAME);
...
free_chan(mcb_state_t *state, me90drv_chnl_state_t	*channel_state, int cur_chnl)
{
	...
	if (state->opened) {
		ME90_LOG(state, ME90_DL_ERROR,
			"mckp_detach cannot detach opened device\n");
		return  DDI_FAILURE;
	}

	if (channel_state -> in_progress)
		ME90_LOG(state, ME90_DL_ERROR,
			"mckp_detach: detach channel %d with "
	...
	if (channel_state -> busy			||
	   ...
	   channel_state -> wait_list_end != NULL	||
		...
		ME90_LOG(state, ME90_DL_ERROR,
			"mckp_detach: detach busy or with transfers "
			"channel %d\n", cur_chnl);
		}
	if (channel_state -> completed_trans_start != NULL  	||
	    channel_state -> completed_trans_end != NULL    	||
	    ...
	    channel_state -> term_trans_processed != 0 ) {
		ME90_LOG(state, ME90_DL_ERROR,
		"mckp_detach: detach channel %d with not "
	...
	if (channel_state -> ready_atrans_start != NULL  ||
	    ...
	    channel_state -> async_trans_num != 0) {
		ME90_LOG(state, ME90_DL_ERROR,
		"me90_dodetach: detach channel %d with not "
		"empty queue of ready asynchronous transfers\n",
			cur_chnl);
		mckp_release_all_async_trans(state, cur_chnl);
		}
		if (channel_state -> last_term_trans_buf != NULL)
			ME90_LOG(state, ME90_DL_ERROR,
				"mckp_detach: last transfer buf not freed "
				"channel %d\n", cur_chnl);
	/*
	 * Reset board and MP
	 */
...
//	me90drv_unmap_reg_sets(me90);
 
	return  DDI_SUCCESS;
}
/*
 * Detach
 * Free resources allocated in %s():
 */
/*ARGSUSED*/
static int mckp_sbus_remove(struct of_device *op)
{
	struct mckp_state *xsp = dev_get_drvdata(&op->dev);
	int cur_chnl = 0;
	int error = 0;
...
//	kill_proc(xsp->pid_state_mckp_intr_handler , SIGTERM, 1);
	xsp->state_mckp_intr_handler_shutdown = 1;
	wake_up(&xsp->state_mckp_intr_handler);
	while (xsp->state_mckp_intr_handler_shutdown != 0){
		schedule_timeout(1);
	}
//	kill_proc(xsp->pid_mckp_watchdog_handler , SIGTERM, 1);
	xsp->mckp_watchdog_handler_shutdown = 1;
	wake_up(&xsp->mckp_watchdog_handler);
	while (xsp->mckp_watchdog_handler_shutdown != 0){
		schedule_timeout(1);
	}
#endif
	for (cur_chnl = 0; cur_chnl < MAX_ME90DRV_BOARD_CHANNEL_NUM;
	...
	unregister_chrdev(xsp->major, MCKP_NAME);

	mckp_states[xsp->inst] = NULL;
	kfree(xsp);
...
static const struct of_device_id mckp_sbus_match[] = {
	{
		.name = MCKP_NAME,
	},
	{},
...
MODULE_DEVICE_TABLE(of, mckp_sbus_match);

static struct of_platform_driver mckp_sbus_driver = {
	.name           = MCKP_NAME,
	.match_table    = mckp_sbus_match,
	.probe          = mckp_sbus_probe,
	.remove         = mckp_sbus_remove,
};
...
static int
__init mckp_init(void)
{
	int ret;
	mckp_instances = 0;

	if ( check_hardware(MCKP_NAME) == 0 )
		return -ENODEV;

	dbgmckp("********* MCKP_INIT: START for %s *********\n", MCKP_NAME);

	ret = of_register_driver(&mckp_sbus_driver, &of_bus_type);

	dbgmckp("********* MCKP_INIT: FINISH. Found %d MCKA instances. %s() returs %d *********\n", mckp_instances, __func__, ret);

	return ret;
}

static void  
__exit mckp_exit(void)
{
	dbgmckp("********* MCKP_EXIT: start for %s *********\n", MCKP_NAME);

	of_unregister_driver(&mckp_sbus_driver);

	dbgmckp("********* MCKP_EXIT: FINISH *********\n"); 
}

module_init(mckp_init);
module_exit(mckp_exit);
MODULE_LICENSE("Copyright by MCST 2004");
MODULE_DESCRIPTION("MCKP driver");
