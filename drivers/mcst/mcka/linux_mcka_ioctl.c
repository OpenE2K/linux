...
/*ARGSUSED*/
int   mcka_get_driver_trace_ioctl(
	mcb_state_t	*state,
	caddr_t		arg,
	int		mode)
{
	...
	if (me90_debug_buf_overflow && me90_debug_msg_buf != NULL) {
		...
		if (rval != 0) {
			ME90_LOG(state, ME90_DL_ERROR,
				"mcka_get_driver_trace_ioctl: ddi_copyout"
				" failed \n");
			return EFAULT;
		}
	}
	...
	if (ddi_copyout((caddr_t) & drv_tracer_msg, arg,
		sizeof (drv_trace_msg_t)/*, mode*/)) {
		ME90_LOG(state, ME90_DL_ERROR,
			"mcka_get_driver_trace_ioctl: ddi_copyout failed\n");
		return EFAULT;
	}
	ME90_LOG(state, ME90_DL_TRACE,
		"mcka_get_driver_trace_ioctl succeeded\n");
	return 0;
}
 ...
 */
/*ARGSUSED*/
int   mcka_load_mp_drv_code_ioctl(
	mcb_state_t	*state,
	caddr_t		arg,
	int		mode)
{
	...
	if (ddi_copyin(arg, (caddr_t)&mp_driver_code,
		...
		ME90_LOG(state, ME90_DL_ERROR,
			"mcka_load_mp_drv_code_ioctl ddi_copyin failed\n"
                     );
		return (EFAULT);
	}
	ME90_LOG(state, ME90_DL_TRACE,
		"mcka_load_mp_drv_code_ioctl from 0x%08x to 0x%08x"
	...
	if (rval != 0) {
		...
		ME90_LOG(state, ME90_DL_ERROR,
			"mcka_load_mp_drv_code_ioctl BMEM load failed\n");
		return rval;
	}
	state -> mp_drv_loaded = 1;
	mutex_exit(&state->mutex);			/* end MUTEX */
	ME90_LOG(state, ME90_DL_TRACE,
		"mcka_load_mp_drv_code_ioctl succeeded\n");
	return 0;
}
	...
	int		mode)
{
	...
	if (arg != NULL) {
		...
		if (ddi_copyin(arg, (caddr_t) &state -> mp_init_code,
			...
			mutex_exit(&state->mutex);	/* end MUTEX */
			ME90_LOG(state, ME90_DL_ERROR,
				"mcka_startup_mp_ioctl ddi_copyin failed for "
				"MP init code desk\n");
			return (EFAULT);
		}
		mutex_exit(&state->mutex);		/* end MUTEX */
		if (state -> mp_init_code.byte_size > MP_INIT_AREA_BMEM_SIZE) {
			ME90_LOG(state, ME90_DL_ERROR,
				"mcka_startup_mp_ioctl too long MP init code "
				"%x > %x\n",
		...
		ME90_LOG(state, ME90_DL_TRACE,
			"mcka_startup_mp_ioctl MP init code load from 0x%08x "
		...
		ME90_LOG(state, ME90_DL_TRACE,
			"mcka_startup_mp_ioctl started to load MP ROM"
			" driver\n");
	}
	rval = me90_startup_mp(state, cmd, mode);
	if (rval != 0) {
		ME90_LOG(state, ME90_DL_TRACE,
			"mcka_startup_mp_ioctl MP driver init failed\n");
	} else {
		ME90_LOG(state, ME90_DL_TRACE,
			"mcka_startup_mp_ioctl MP driver init succeeded\n");
	}
	return (rval);
}
 ...
 */
/*ARGSUSED*/
int   mcka_reset_mp_ioctl(
	mcb_state_t	*state,
	int		clean_bmem)
{
	int	rval = 0;

	ME90_LOG(state, ME90_DL_TRACE,"mcka_reset_mp_ioctl started\n");
	mutex_enter(&state->mutex);			/* start MUTEX */
	rval = me90_reset_mp(state, 1, clean_bmem);
	mutex_exit(&state->mutex);			/* end MUTEX */
	if (rval != 0) {
		ME90_LOG(state, ME90_DL_TRACE,
			"mcka_reset_mp_ioctl finished with errors\n");
	} else {
		ME90_LOG(state, ME90_DL_TRACE,
			"mcka_reset_mp_ioctl succeeded\n");
	}
	return 0;
}
 ...
 */
/*ARGSUSED*/
int   mcka_set_mp_state_ioctl(
	mcb_state_t	*state,
	int		mp_state)
{
	int	rval = 0;

	ME90_LOG(state, ME90_DL_TRACE,"mcka_set_mp_state_ioctl started\n");
	mutex_enter(&state->mutex);			/* start MUTEX */
	...
	if (rval != 0) {
		ME90_LOG(state, ME90_DL_TRACE,
			"mcka_set_mp_state_ioctl finished with errors\n");
	} else {
		ME90_LOG(state, ME90_DL_TRACE,
			"mcka_set_mp_state_ioctl succeeded\n");
	}
	return 0;
}
 ...
 */
/*ARGSUSED*/
int   mcka_get_driver_info_ioctl(
	mcb_state_t	*state,
	caddr_t		arg,
	int		mode)
{
	...
	ME90_LOG(state, ME90_DL_TRACE,
                   "mcka_get_driver_info_ioctl started\n");
#ifdef	_MP_TIME_USE_
        drv_communication =
	...
	if (ddi_copyout((caddr_t) & driver_info, arg,
		...
		ME90_LOG(state, ME90_DL_ERROR,
			"mcka_get_driver_info_ioctl: ddi_copyout failed\n");
		rval = EFAULT;
	}
	if (rval != 0) {
		ME90_LOG(state, ME90_DL_TRACE,
			"mcka_get_driver_info_ioctl: finished\n");
	} else {
		ME90_LOG(state, ME90_DL_TRACE,
			"mcka_get_driver_info_ioctl succeeded\n");
	}
	return rval;
}
 ...
 */
/*ARGSUSED*/
int   mcka_lock_reset_module_on_error_ioctl(mcb_state_t	*state)
{
	int		rval = 0;
	mc_wr_reg_t	tlrm_write_value;

	ME90_LOG(state, ME90_DL_TRACE,
		"mcka_lock_reset_module_on_error_ioctl started\n");
	...
	state -> set_tlrm = 1;
	ME90_LOG(state, ME90_DL_TRACE,
		"mcka_lock_reset_module_on_error_ioctl succeeded\n");
	return rval;
}
 ...
 */
/*ARGSUSED*/
int   mcka_unlock_reset_module_on_error_ioctl(mcb_state_t	*state)
{
	int		rval = 0;
	mc_wr_reg_t    tlrm_write_value;

	ME90_LOG(state, ME90_DL_TRACE,
		"mcka_unlock_reset_module_on_error_ioctl started\n");
	tlrm_write_value.RGEN_write = 0;
	...
	state -> set_tlrm = 0;
	ME90_LOG(state, ME90_DL_TRACE,
		"mcka_unlock_reset_module_on_error_ioctl succeeded\n");
	return rval;
}
 ...
 */
/*ARGSUSED*/
int   mcka_write_adapter_reg_ioctl(
	mcb_state_t	*state,
	caddr_t		arg,
	int		mode)
{
	...
	if (ddi_copyin(arg, (caddr_t) &device_reg_write_args,
		...
		ME90_LOG(state, ME90_DL_ERROR,
			"mcka_write_adapter_reg_ioctl: ddi_copyin failed\n");
		return EFAULT;
	}
	ME90_LOG(state, ME90_DL_TRACE,
		"mcka_write_adapter_reg_ioctl started\n");
	...
	if (rval != 0) {
		ME90_LOG(state, ME90_DL_TRACE,
			"mcka_write_adapter_reg_ioctl failed\n");
		return rval;
	}
	ME90_LOG(state, ME90_DL_TRACE,
		"mcka_write_adapter_reg_ioctl finished\n");
	return 0;
}
 ...
 */
/*ARGSUSED*/
int   mcka_read_adapter_reg_ioctl(
	mcb_state_t	*state,
	caddr_t		arg,
	int		mode)
{
	...
	if (ddi_copyin(arg, (caddr_t) &device_reg_read_args,
		sizeof (dev_reg_spec_t)/*, mode*/)) {
		ME90_LOG(state, ME90_DL_ERROR,
			"mcka_read_adapter_reg_ioctl: ddi_copyin failed\n");
		return EFAULT;
	}
	ME90_LOG(state, ME90_DL_TRACE,
		"mcka_read_adapter_reg_ioctl started\n");
	device_reg_access_args.dev_adapter_access.address =
	...
	if (rval != 0) {
		ME90_LOG(state, ME90_DL_TRACE,
			"mcka_read_adapter_reg_ioctl failed\n");
		...
		ME90_LOG(state, ME90_DL_TRACE,
			"mcka_read_adapter_reg_ioctl failed with error"
	...
	if (ddi_copyout((caddr_t) &device_reg_read_args, arg,
		sizeof (dev_reg_spec_t)/*, mode*/)) {
		ME90_LOG(state, ME90_DL_ERROR,
			"mcka_read_adapter_reg_ioctl: ddi_copyout failed\n");
		return EFAULT;
	}
	ME90_LOG(state, ME90_DL_TRACE,
		"mcka_read_adapter_reg_ioctl finished\n");
	return rval;
}
 ...
 */
/*ARGSUSED*/
int   mcka_set_drv_general_mode_ioctl(
	mcb_state_t	*state,
	int		drv_mode_to_set)
{
	ME90_LOG(state, ME90_DL_TRACE,
		"mcka_set_drv_general_mode_ioctl to 0x%x\n",
		drv_mode_to_set);
	mutex_enter(&state->mutex);			/* start MUTEX */
	state -> drv_general_modes |= drv_mode_to_set;
	mutex_exit(&state->mutex);			/* end MUTEX */
	ME90_LOG(state, ME90_DL_TRACE,
		"mcka_set_drv_general_mode_ioctl succeeded\n");
	return 0;
}
 ...
 */
/*ARGSUSED*/
int   mcka_reset_drv_general_mode_ioctl(
	mcb_state_t	*state,
	int		drv_mode_to_reset)
{
	ME90_LOG(state, ME90_DL_TRACE,
		"mcka_reset_drv_general_mode_ioctl to 0x%x\n",
		drv_mode_to_reset);
	mutex_enter(&state->mutex);			/* start MUTEX */
	state -> drv_general_modes &= ~drv_mode_to_reset;
	mutex_exit(&state->mutex);			/* end MUTEX */
	ME90_LOG(state, ME90_DL_TRACE,
		"mcka_reset_drv_general_mode_ioctl succeeded\n");
	return 0;
}
 ...
 */
/*ARGSUSED*/
int   mcka_get_drv_general_mode_ioctl(
	mcb_state_t	*state,
	caddr_t		arg,
	int		mode)
{
	ME90_LOG(state, ME90_DL_TRACE,
		"mcka_get_drv_general_mode_ioctl started\n");
	if (ddi_copyout((caddr_t) & state -> drv_general_modes, arg,
		sizeof (state -> drv_general_modes)/*, mode*/)) {
		ME90_LOG(state, ME90_DL_ERROR,
			"mcka_get_drv_general_mode_ioctl: ddi_copyout "
			"failed\n");
		return EFAULT;
	}
	ME90_LOG(state, ME90_DL_TRACE,
		"mcka_ioctl get current driver general mode flags"
		" succeeded\n");
	return 0;
}
 ...
 */
/*ARGSUSED*/
int   mcka_put_drv_general_mode_ioctl(
	mcb_state_t	*state,
	int		drv_modes_to_put)
{

	ME90_LOG(state, ME90_DL_TRACE,
		"mcka_put_drv_general_mode_ioctl to 0x%x\n",
		drv_modes_to_put);
	mutex_enter(&state->mutex);			/* start MUTEX */
	state -> drv_general_modes = drv_modes_to_put;
	mutex_exit(&state->mutex);			/* end MUTEX */
	ME90_LOG(state, ME90_DL_TRACE,
		"mcka_put_drv_general_mode_ioctl succeeded\n");
	return 0;
}
...
/*ARGSUSED*/
int
mcka_wait_for_async_trans_ioctl(
	mcb_state_t	*state,
	int		channel,
	caddr_t		arg,
	int		mode)
{
	...
	if (ddi_copyin(arg, (caddr_t) &waiting_request,
		...
		ME90_LOG(state, ME90_DL_ERROR,
			"mcka_wait_for_async_trans_ioctl: ddi_copyin "
			"failed in the channel %d\n", channel);
		return EFAULT;
	}
	ME90_LOG(state, ME90_DL_TRACE,
		"mcka_wait_for_async_trans_ioctl in the "
	...
	if (user_trans_res_info_pp == NULL) {
		ME90_LOG(state, ME90_DL_ERROR,
			"mcka_wait_for_async_trans_ioctl: NULL pointer of I/O "
	...
	if (ddi_copyout((caddr_t) &user_results_p,
		...
		sizeof (caddr_t)/*, mode*/)) {
		ME90_LOG(state, ME90_DL_ERROR,
			"mcka_wait_for_async_trans_ioctl: ddi_copyout failed "
...
/*ARGSUSED*/
int
mcka_wait_for_trans_in_progress_ioctl(
	mcb_state_t	*state,
	int		channel,
	caddr_t		arg,
	int		mode)
{
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mcka_wait_for_trans_in_progress_ioctl wait for transfer in "
	...
	if (rval != 0) {
		ME90_LOG(state, ME90_DL_TRACE,
			"mcka_ioctl wait for transfer in progress timeouted "
			"for channel %d error %d\n", channel, rval);
	} else {
		ME90_LOG(state, ME90_DL_TRACE,
			"mcka_ioctl wait for transfer in progress successed "
			"for channel %d\n", channel);
	}
	return rval;
}
...
/*ARGSUSED*/
int
mcka_output_last_trans_state_ioctl(
	mcb_state_t	*state,
	int		channel)
{
	me90drv_chnl_state_t	*channel_state = NULL;
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mcka_output_last_trans_state_ioctl started\n");
	channel_state = &state -> all_channels_state[channel];
	if (channel_state -> last_term_trans_buf != NULL)
		...
		ME90_LOG(state, ME90_DL_ERROR,
			"mcka_output_last_trans_state_ioctl: empty last "
			"transfer buf pointer\n");
	ME90_LOG(state, ME90_DL_TRACE,
		"mcka_output_last_trans_state_ioctl finished\n");
	return 0;
}
...
/*ARGSUSED*/
int
mcka_restart_board_ioctl(mcb_state_t	*state)
{
	int			rval = 0;
	me90drv_rd_reg_t	gen_reg_state;

	ME90_LOG(state, ME90_DL_TRACE, "mcka_restart_board_ioctl started\n");
	gen_reg_state.ME90DRV_RGEN_read = 0;
	rval = me90_retrieve_trans_mode(state, 0, 1, gen_reg_state);
	if (rval != 0) {
		ME90_LOG(state, ME90_DL_TRACE,
			"mcka_restart_board_ioctl cannot restart the board\n");
	} else {
		ME90_LOG(state, ME90_DL_TRACE,
			"mcka_restart_board_ioctl succeeded\n");
	}
	return rval;
}
...
int
mcka_ioctl ( struct inode *inode, struct file *file,
...
//   printk("#################### mcka_ioctl started cmd = 0x%x #####################\n", cmd);
     ME90_LOG(NULL, ME90_DL_TRACE,"mcka_ioctl started for instance %d with cmd 0x%x\n", instance, cmd);
...
//			printk("####### MCBIO_LOAD_MP_DRV_CODE #######\n");
			return mcka_load_mp_drv_code_ioctl(state,
						(caddr_t)arg, mode);
		case MCBIO_STARTUP_MP_DRV  :
//			printk("####### MCBIO_STARTUP_MP_DRV #######\n");
		case MCBIO_STARTUP_MP_ROM_DRV  :
//			printk("####### MCBIO_STARTUP_MP_ROM_DRV #######\n");
		case MCBIO_STARTUP_MP_CODE :
//			printk("####### MCBIO_STARTUP_MP_CODE #######\n");
			return mcka_startup_mp_ioctl(state, cmd, (caddr_t)arg,
					mode);
		case MCBIO_RESET_MP :
//			printk("####### MCBIO_RESET_MP #######\n");
			return mcka_reset_mp_ioctl(state, arg);
		case MCBIO_SET_MP_STATE :
//			printk("####### MCBIO_SET_MP_STATE #######\n");
			return mcka_set_mp_state_ioctl(state, arg);
		case MCBIO_GET_DRIVER_INFO :
//			printk("####### MCBIO_GET_DRIVER_INFO #######\n");
			return mcka_get_driver_info_ioctl(state, (caddr_t)arg,
					mode);
		case MCBIO_LOCK_RESET_MODULE_ON_ERROR :
//			printk("####### MCBIO_LOCK_RESET_MODULE_ON_ERROR #######\n");
			return mcka_lock_reset_module_on_error_ioctl(state);
		case MCBIO_UNLOCK_RESET_MODULE_ON_ERROR :
//			printk("####### MCBIO_UNLOCK_RESET_MODULE_ON_ERROR #######\n");
			return mcka_unlock_reset_module_on_error_ioctl(state);

		case MCBIO_GET_DRIVER_TRACE_MSG :
//			printk("####### MCBIO_GET_DRIVER_TRACE_MSG #######\n");
			return mcka_get_driver_trace_ioctl(state, (caddr_t)arg,
					mode);
		case MCBIO_WRITE_DEV_ADAPTER_REG :
//			printk("####### MCBIO_WRITE_DEV_ADAPTER_REG #######\n");
			return mcka_write_adapter_reg_ioctl(state, (caddr_t)arg,
					mode);
		case MCBIO_READ_DEV_ADAPTER_REG :
//			printk("####### MCBIO_READ_DEV_ADAPTER_REG #######\n");
			return mcka_read_adapter_reg_ioctl(state, (caddr_t)arg,
					mode);
		case MCBIO_SET_DRV_GENERAL_MODE :
//			printk("####### MCBIO_SET_DRV_GENERAL_MODE #######\n");
			return mcka_set_drv_general_mode_ioctl(state, arg);
		case MCBIO_RESET_DRV_GENERAL_MODE :
//			printk("####### MCBIO_RESET_DRV_GENERAL_MODE #######\n");
			return mcka_reset_drv_general_mode_ioctl(state, arg);
		case MCBIO_GET_DRV_GENERAL_MODES :
//			printk("####### MCBIO_GET_DRV_GENERAL_MODES #######\n");
			return mcka_get_drv_general_mode_ioctl(state,
					(caddr_t)arg, mode);
		case MCBIO_PUT_DRV_GENERAL_MODES :
//			printk("####### MCBIO_PUT_DRV_GENERAL_MODES #######\n");
			return mcka_put_drv_general_mode_ioctl(state, arg);
...
//	   printk("####### MCBIO_SPECIFIED_TRANSFER #######\n");
           ...
           {
              ME90_LOG(state, ME90_DL_ERROR,
                      "mcka_ioctl ddi_copyin failed for transfer specification\n"
                     );
              return (EFAULT);
           }
#ifdef	DEBUG
           ...
           ME90_LOG(state, ME90_DL_TRACE,
                   "mcka_ioctl specified transfer addr 0x%lx len 0x%lx %s %s"
                   " %s burst size 0x%02x"
                   " repeat %d\n",
           ...
           {
              ME90_LOG(state, ME90_DL_ERROR,
                      "mcka_ioctl invalid read/write flad %d\n",
           ...
           {
              ME90_LOG(state, ME90_DL_ERROR,
                      "mcka_ioctl invalid I/O transfer mode %d\n",
		...
		&&
           ...
           {
              ME90_LOG(state, ME90_DL_ERROR,
                      "mcka_ioctl invalid I/O device access mode %d\n",
	   ...
	   if (user_transfer_spec.burst_sizes == 0)
		user_transfer_spec.burst_sizes = MCB_ENABLE_BURST_SIZES;
           ...
           {
              ME90_LOG(state, ME90_DL_ERROR,
                      "mcka_ioctl empty mask of allowed 0x%02x"
	   ...
	   if (user_transfer_spec.async_trans) {
		if (user_trans_res_info == NULL) {
			ME90_LOG(state, ME90_DL_ERROR,
				"mcka_ioctl cannot do asynchronous "
...
/* Memory allocation */
		...
		if (transfer_spec_p == NULL) {
			ME90_LOG(state, ME90_DL_ERROR,
				"mcka_ioctl cannot allocate kernel memory for"
...
//		transfer_spec_p -> buf_base = kmalloc(sizeof(user_transfer_spec.buf_byte_size), GFP_KERNEL);
		...
		if (transfer_spec_p -> buf_base <= 0) { 
		  printk ("mcka_ioctl: Error allocated memory. 
		...
		if (ddi_copyin(user_transfer_spec.buf_base, transfer_spec_p -> buf_base,
		  ...
		  printk ("mcka_ioctl: Error copy_from_user\n");
		  return EFAULT;
		}
	   } else {
...
/* mcka_rdwr for testing only */
	   rval = mcb_rdwr(dev, uio_p,
		...
		if (set_rval != 0)
           ...
           if (rval != 0)
              ME90_LOG(state, ME90_DL_ERROR,
                      "mcka_ioctl transfer finished with error %d for channel"
                      " %d\n",
                      rval,channel
                     );
	   if (delete_kmem_bufs) {
...
/*	   if ((unsigned long)user_transfer_spec.buf_base < TASK_SIZE && !delete_kmem_bufs) {
		if (ddi_copyout(transfer_spec_p -> buf_base, user_transfer_spec.buf_base,
				user_transfer_spec.buf_byte_size )) {
		  printk ("mcka_ioctl: Error copy_to_user\n");
		}
	   }*/
           return rval;
	}
	case MCBIO_WAIT_FOR_ASYNC_TRANS_END :
//		printk("####### MCBIO_WAIT_FOR_ASYNC_TRANS_END #######\n");
		return mcka_wait_for_async_trans_ioctl(state, channel,
				(caddr_t)arg, mode);
        case MCBIO_DRQ_WAITING_TRANSFER_NUM :
        {
           int cur_transfer_num = 0;
           me90drv_chnl_state_t *channel_state = NULL;
//	   printk("####### MCBIO_DRQ_WAITING_TRANSFER_NUM #######\n");

           ME90_LOG(state, ME90_DL_TRACE,
                   "mcka_ioctl get transfer num waiting for DRQ channel %d\n",
                   channel
                  );
           channel_state = &state -> all_channels_state[channel];
	   mutex_enter(&state->mutex);			/* start MUTEX */
           ...
           {
              ME90_LOG(state, ME90_DL_ERROR,
                      "mcka_ioctl ddi_copyout failed for get waiting DRQ num\n"
                     );
              rval = EFAULT;
           }
	   mutex_exit(&state->mutex);			/* end MUTEX */
           ...
           {
              ME90_LOG(state, ME90_DL_TRACE,
                      "mcka_ioctl get transfer num waiting for DRQ finished "
                      "with errors\n"
                     );
           }
           else
           {
              ME90_LOG(state, ME90_DL_TRACE,
                      "mcka_ioctl get transfer num waiting for DRQ successed\n"
                     );
           }
           return rval;
        }
        case MCBIO_SET_MP_TIMER_INTR :
        {
           ...
           int               rval = 0;
//		printk("####### MCBIO_SET_MP_TIMER_INTR #######\n");
           ...
           {
              ME90_LOG(state, ME90_DL_ERROR,
                      "mcka_ioctl: ddi_copyin failed for set MP timer intr\n"
                     );
              return EFAULT;
           }
           ME90_LOG(state, ME90_DL_TRACE,
                   "mcka_ioctl set MP timer interrupt mode with interval %d"
                   " max queue size %d\n",
	   ...
	   mutex_enter(&state->mutex);			/* start MUTEX */
	      ...
	      mutex_exit(&state->mutex);			/* end MUTEX */
              ME90_LOG(state, ME90_DL_ERROR,
                      "mcka_ioctl: MP timer intr is seted already\n"
                     );
              return EBUSY;
           }
	   ...
	   mutex_exit(&state->mutex);			/* end MUTEX */
           ...
           {
              ME90_LOG(state, ME90_DL_TRACE,
                      "mcka_ioctl: MP timer intr set failed\n"
                     );
           }
           else
           {
              ME90_LOG(state, ME90_DL_TRACE,
                      "mcka_ioctl: MP timer intr set successed\n"
                     );
           }
           return rval;
        }
        case MCBIO_RESET_MP_TIMER_INTR :
        {
           ...
           int                 rval = 0;
//	   printk("####### MCBIO_RESET_MP_TIMER_INTR #######\n");
           ME90_LOG(state, ME90_DL_TRACE,
                   "mcka_ioctl reset MP timer interrupt mode\n"
                  );
	   mutex_enter(&state->mutex);			/* start MUTEX */
           mp_timer_intr_spec = &state -> mp_timer_intrs;
           if (mp_timer_intr_spec -> mp_intr_mode_on != 1)
           {
              ME90_LOG(state, ME90_DL_ERROR,
                      "mcka_ioctl: MP timer intr is reseted already\n"
                     );
           }
	   ...
	   mutex_exit(&state->mutex);			/* end MUTEX */
           ...
           {
              ME90_LOG(state, ME90_DL_ERROR,
                      "mcka_ioctl: ddi_copyout failed for reset MP timer intr\n"
                     );
              rval = EFAULT;
           }
           if (rval != 0)
           {
              ME90_LOG(state, ME90_DL_TRACE,
                      "mcka_ioctl: MP timer intr reset finished\n"
                     );
           }
           else
           {
              ME90_LOG(state, ME90_DL_TRACE,
                      "mcka_ioctl: MP timer intr reset successed\n"
                     );
           }
           return rval;
        }
        case MCBIO_WAIT_MP_TIMER_INTR :
        {
           ...
           int                 rval = 0;
//		printk("####### MCBIO_WAIT_MP_TIMER_INTR #######\n");
#ifdef	_MP_TIME_USE_
...
#endif	/* _MP_TIME_USE_ */
           ME90_LOG(state, ME90_DL_TRACE,
                   "mcka_ioctl wait for MP timer interrupt mode\n"
                  );
           ...
           {
              ME90_LOG(state, ME90_DL_ERROR,
                      "mcka_ioctl: ddi_copyout failed for wait for MP timer"
	              " intr\n"
                     );
              rval = EFAULT;
           }
           ...
           {
              ME90_LOG(state, ME90_DL_TRACE,
                      "mcka_ioctl: waiting for MP timer intr finished\n"
                     );
           }
           else
           {
              ME90_LOG(state, ME90_DL_TRACE,
                      "mcka_ioctl: waiting for MP timer intr successed\n"
                     );
           }
           return rval;
        }
		...
		case MCBIO_WAIT_FOR_TRANSFER_IN_PROGRESS :
			return mcka_wait_for_trans_in_progress_ioctl(state,
        ...
        case MCBIO_RESET_SBUS_LOGER :
        {
           ...
           int            clean_bmem = arg;
           int            rval = 0;
//	   printk("####### MCBIO_RESET_SBUS_LOGER #######\n");
           ME90_LOG(state, ME90_DL_TRACE,
                   "mcka_ioctl reset SBus loger and log memory\n"
                  );
	   ...
	   b2l_convertor_off(state -> dip);
           ...
           if (clean_bmem)
              me90_clean_base_memory(state);
           ME90_LOG(state, ME90_DL_TRACE,
                   "mcka_ioctl: reset SBus loger and log memory successed\n"
                  );
           return rval;
        }

		case MCBIO_OUT_LAST_TRANS_STATE :
			return mcka_output_last_trans_state_ioctl(state,
					channel);
		case MCBIO_RESTART_BOARD :
			return mcka_restart_board_ioctl(state);

        case MCBIO_SET_CONNECTION_POLLING :
        {
           cnct_poll_set_t	polling_setup_spec;
           int			rval = 0;
//         printk("####### MCBIO_SET_CONNECTION_POLLING #######\n");
           if (ddi_copyin((caddr_t) arg,
           ...
           {
              ME90_LOG(state, ME90_DL_ERROR,
                      "mcka_ioctl: ddi_copyin failed for set connection"
                      " polling\n"
                     );
              return EFAULT;
           }
           ME90_LOG(state, ME90_DL_TRACE,
                   "mcka_ioctl set connection polling with interval %d"
           ...
           ME90_LOG(state, ME90_DL_TRACE,
                   "mcka_ioctl: set connection polling finished\n"
                  );
           return rval;
        }
        ...
        {
           int			rval = 0;
//         printk("####### MCBIO_RESET_CONNECTION_POLLING #######\n");
           ME90_LOG(state, ME90_DL_TRACE,
                   "mcka_ioctl reset connection polling\n"
                  );
           rval = mcb_reset_connection_polling(state, 0);
           ME90_LOG(state, ME90_DL_TRACE,
                   "mcka_ioctl: reset connection polling finished\n"
                  );
           return rval;
        }
...
//         printk("####### MCBIO_POLL_CONNECTION_STATE #######\n");
           if (ddi_copyin((caddr_t) arg,
                          (caddr_t) & state_spec,
           ...
           {
              ME90_LOG(state, ME90_DL_ERROR,
                      "mcka_ioctl: ddi_copyin failed for poll connection state\n"
                     );
              return EFAULT;
           }
           ME90_LOG(state, ME90_DL_TRACE,
                   "mcka_ioctl poll connection state with mask 0x%x and"
                   " timeout %d msec\n",
              ...
              )
           {
              ...
              else
              {
                 ...
                 {
                    ME90_LOG(state, ME90_DL_ERROR,
                            "mcka_ioctl: events info ddi_copyout failed for "
           ...
           {
              ME90_LOG(state, ME90_DL_ERROR,
                      "mcka_ioctl: ddi_copyout failed for poll connection"
                      " state\n"
                     );
              return EFAULT;
           }
           if (usr_time_info != NULL)
           {
              ...
              {
                 ME90_LOG(state, ME90_DL_ERROR,
                         "mcka_ioctl: time info ddi_copyout failed for poll "
                         "connection state\n"
                        );
                 return EFAULT;
              }
           ...
           ME90_LOG(state, ME90_DL_TRACE,
                   "mcka_ioctl: poll connection state finished\n"
                  );
           return rval;
        }
        default :
           ME90_LOG(state, ME90_DL_ERROR,
                   "mcka_ioctl invalid 'ioctl' command 0x%x\n",
                   cmd
                  );
           return (ENOTTY);
     }
	...
	int			mode)
{
	...
	ME90_LOG(state, ME90_DL_TRACE,
		"mcka_set_trans_results started\n");
	...
	if (ddi_copyout((caddr_t) drv_trans_res_info_p,
		...
		ME90_LOG(state, ME90_DL_ERROR,
			"mcka_set_trans_results ddi_copyout failed for"
			" transfer results\n");
		rval = EFAULT;
	}

	ME90_LOG(state, ME90_DL_TRACE,
		"mcka_set_trans_results finished\n");
	return rval;
}
