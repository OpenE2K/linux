...
#define MPV_EIOH_DEVID		0x8025
#define MPV4_DEVID		0x8023
#define MPV_KPI2		1	/* DEVID=8014 2 irq lines */
#define MPV_4			2
#define MPV_EIOH		3	/* DEVID=8025 3 irq lines */
...
typedef struct mpv_intrk
{
	...
	long long	time_get_comm;		/* Obsolete. */
	...
	/* ns monotonic time when interrupt was appear in controler */
	long long	intr_appear_nsec_mono;
	/* The previous time when interrupt was */
	long long	time_prev_intrpt;
} mpv_intrk_t;
...
typedef struct mpv_state_struct {
	...
	int			pps4mgb_nunber;	/* in number for pps for mgb */
