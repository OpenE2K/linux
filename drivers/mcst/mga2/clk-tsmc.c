...
#define	 MGA2_DC0_CLKCTRL		0x00000	/* регистр управления пиксельной частотой */
...
#define to_mga2(m) container_of(m, struct mga2_clk, hw)
...
static int __mga2_set_pll(struct clk_hw *hw, struct mclk *clk)
{
	unsigned val;
	int ret = 0;
...
static int mga2_set_pll(struct clk_hw *hw, struct mclk *clk)
{
	u32 val;
	int ret = 0;
	int int_pll = 1;
...
static int mga2_pll_enable(struct clk_hw *hw)
{
...
static void mga2_pll_disable(struct clk_hw *hw)
{
}

static unsigned long mga2_pll_recalc_rate(struct clk_hw *hw,
...
static int mga2_pll_set_rate(struct clk_hw *hw, unsigned long rate,
		unsigned long parent_rate)
{
	struct mclk clk;
	struct mga2_clk *m = to_mga2(hw);
...
static const struct clk_ops mga2_pll_ops = {
	.enable      = mga2_pll_enable,
	.disable     = mga2_pll_disable,
	.recalc_rate = mga2_pll_recalc_rate,
	.round_rate  = mga2_pll_round_rate,
	.set_rate    = mga2_pll_set_rate,
};
...
static int mga2_pll_probe(struct platform_device *pdev)
{
	int ret = 0;
	struct clk *clk;
	...
	struct mga2_clk *m = devm_kzalloc(dev, sizeof(*m), GFP_KERNEL);
...
static int mga2_pll_remove(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct device_node *np = dev->of_node;
...
static const struct mga2_pll tcitsmcn40 = {
...
static const struct mga2_pll tcitsmcn16 = {
	.reference_freq = 100 * 1000,
	.min_feedback_div = 1, /* NF */
...
static const struct of_device_id __maybe_unused mga2_pll_dt_ids[] = {
	{
...
MODULE_DEVICE_TABLE(of, mga2_pll_dt_ids);

struct platform_driver mga2_pll_driver = {
	.driver = {
		.name = "tcitsmc-pll",
		.of_match_table = of_match_ptr(mga2_pll_dt_ids),
	},
	.probe = mga2_pll_probe,
	.remove = mga2_pll_remove,
};
