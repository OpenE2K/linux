...
static unsigned mga2_get_table_val(const struct clk_div_table *table,
							unsigned int div)
{
...
static struct clk_div_table *mga2_get_div_table(struct device *dev)
{
		...
		unsigned long parent_rate)
{
	...
	u32 v = mga2_get_table_val(m->tlb, rate);
...
static int mga2_clk_mux_probe(struct platform_device *pdev)
{
	int ret = 0;
	struct clk *clk;
...
static int mga2_clk_mux_remove(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct device_node *np = dev->of_node;
...
static const struct of_device_id __maybe_unused mga2_clk_mux_dt_ids[] = {
	{
		.compatible = "mcst,clk-mux-mga2",
	},
	{ }
};
MODULE_DEVICE_TABLE(of, mga2_clk_mux_dt_ids);

struct platform_driver mga2_clk_mux_driver = {
	.driver = {
		   .name = "mga2-clk-mux",
		   .of_match_table = of_match_ptr(mga2_clk_mux_dt_ids),
		    },
	.probe = mga2_clk_mux_probe,
	.remove = mga2_clk_mux_remove,
};
