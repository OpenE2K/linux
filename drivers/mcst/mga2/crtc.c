...
static int mga2_crtc_atomic_check(struct drm_crtc *crtc,
		struct drm_crtc_state *state)
{
	struct mga2_crtc *mcrtc = to_mga2_crtc(crtc);
	if (mcrtc->force_mode_changed) /* FIXME: */
...
static const struct drm_crtc_helper_funcs mga2_crtc_helper_funcs = {
	...
	.atomic_check	= mga2_crtc_atomic_check,
};
...
static irqreturn_t mga2_crtc_irq_handler(int irq, void *arg)
{
	struct mga2_crtc *mcrtc = arg;
...
static int mga2_crtc_enable_vblank(struct drm_crtc *crtc)
{
	struct mga2_crtc *mcrtc = to_mga2_crtc(crtc);
...
static void mga2_crtc_disable_vblank(struct drm_crtc *crtc)
{
	struct mga2_crtc *mcrtc = to_mga2_crtc(crtc);
...
static int mga2_crtc_init(struct mga2_crtc *mcrtc, struct drm_device *drm)
{
	int i, ret;
	struct drm_plane *primary = NULL, *cursor = NULL;
	struct drm_crtc *crtc = &mcrtc->base;
	struct drm_plane **planes = mga2_layers_init(mcrtc, drm);
	if (IS_ERR(planes)) {
	...
	drm_crtc_helper_add(crtc, &mga2_crtc_helper_funcs);
...
static int mga2_crtc_bind(struct device *dev, struct device *master, void *data)
{
	int ret, irq;
	struct drm_device *drm = data;
	struct platform_device *pdev = to_platform_device(dev);
	struct resource *res = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	struct mga2_crtc *mcrtc = devm_kzalloc(dev, sizeof(*mcrtc), GFP_KERNEL);
	if (!mcrtc)
		return -ENOMEM;

	mcrtc->dev = dev;
	mcrtc->dev_id = mga2_get_version(dev->parent);
	...
	mcrtc->irq = irq;
	 /* mga2_crtc_enable_vblank() will enable irq */
	irq_set_status_flags(irq, IRQ_NOAUTOEN | IRQ_DISABLE_UNLAZY);
	ret = devm_request_irq(dev, irq, mga2_crtc_irq_handler,
			       0, dev_name(dev), mcrtc);
	if (WARN_ON(ret))
		goto out;
	ret = mga2_crtc_init(mcrtc, drm);
	if (WARN_ON(ret))
		return ret;
	mga2_crtc_hw_init(&mcrtc->base);
	dev_set_drvdata(dev, mcrtc);
out:
	return ret;
}

static void mga2_crtc_unbind(struct device *dev, struct device *master, void *data)
{
...
static int mga2_crtc_probe(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	return component_add(dev, &mga2_crtc_component_ops);
}

static int mga2_crtc_remove(struct platform_device *pdev)
{
	component_del(&pdev->dev, &mga2_crtc_component_ops);
	return 0;
}

static const struct of_device_id mga2_crtc_driver_dt_match[] = {
	{ .compatible = "mcst,mga2x-crtc"},
	{},
};
MODULE_DEVICE_TABLE(of, mga2_crtc_driver_dt_match);

struct platform_driver mga2_crtc_driver = {
	.probe = mga2_crtc_probe,
	.remove = mga2_crtc_remove,
	.driver = {
		.name = "mga2-crtc",
		.of_match_table = of_match_ptr(mga2_crtc_driver_dt_match),
	},
};
