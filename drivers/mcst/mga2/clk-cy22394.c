...
/* Fpll = Fref * (Pt / Qt) */
static const struct mga2_pll cy22394_pll = {
...
static long mga2_calc_pll(unsigned long rate,
				struct mclk *clk, bool verbose)
{
	...
	ret = mga2_pll_compute(&cy22394_pll, clock, &pll_clock,
		...
		unsigned long parent_rate)
{
	...
	long ret = mga2_calc_pll(rate, &clk, true);
	if (ret < 0)
		return ret;
