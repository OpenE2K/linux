...
#define mga2_hw_to_ptr(__p) (__p / 2)
#define mga2_ptr_to_hw(__p) (__p * 2)
...
static int mga25_append_desc(struct mga2 *mga2, struct mga2_gem_object *mo)
{
	...
	mga2->ring[mga2_ptr_to_hw(h)] = cpu_to_le64(dma_addr);
	mga2->head = circ_inc(h);
	mga2->fence_seqno++;
	wfb(mga2_ptr_to_hw(mga2->head), MGA2_AUC2_HEADTAIL);
...
{
	dma_addr_t addr;
	int sz = MGA2_HW_RING_SIZE * sizeof(*mga2->ring);
	void *b;
	if (!(b = dmam_alloc_coherent(mga2->drm->dev, sz, &addr, GFP_KERNEL)))
		goto err;
