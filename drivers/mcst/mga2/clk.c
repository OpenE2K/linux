...
/**
 * mga2_pll_reduce_ratio - fractional number reduction
 *
 * @nom: nominator
 * @den: denominator
 ...
 */
static void mga2_pll_reduce_ratio(unsigned *nom, unsigned *den,
...
/**
 * mga2_pll_get_fb_ref_div - feedback and ref divider calculation
 *
 * @nom: nominator
 * @den: denominator
 ...
 */
static void mga2_pll_get_fb_ref_div(unsigned nom, unsigned den, unsigned post_div,
...
/**
 * mga2_pll_compute - compute PLL paramaters
 *
 ...
 */
static void __mga2_pll_compute(const struct mga2_pll *pll,
			const u32 freq,
			u32 *dot_clock_p,
			u32 *fb_div_p,
			u32 *frac_fb_div_p,
			...
			u32 *post_div_p)
{
	...
	/* reduce the numbers to a simpler ratio */
	mga2_pll_reduce_ratio(&nom, &den, fb_div_min, post_div_min);
	...
	for (post_div = post_div_min; post_div <= post_div_max; ++post_div) {
		unsigned diff;
		mga2_pll_get_fb_ref_div(nom, den, post_div, fb_div_max,
	...
	mga2_pll_get_fb_ref_div(nom, den, post_div, fb_div_max, ref_div_max,
				  &fb_div, &ref_div);

	/* reduce the numbers to a simpler ratio once more */
	/* this also makes sure that the reference divider is large enough */
	mga2_pll_reduce_ratio(&fb_div, &ref_div, fb_div_min, ref_div_min);
			...
			u32 *post_div_p)
{
	...
	if (freq < pll->pll_out_min ||
		freq > pll->pll_out_max
			...
			return -1;
	__mga2_pll_compute(pll, freq, dot_clock_p,
