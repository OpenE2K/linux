...
static void __bctrl_wdesc(struct bctrl_cmd *c, u32 data, u32 reg,
...
static void bctrl_wdesc(struct mga2 *mga2, int n, u32 data, u32 reg,
			   bool first, bool last)
{
	struct bctrl_cmd *c = &mga2->bctrl->desc[mga2->head].cmd[n];
	__bctrl_wdesc(c, data, reg, first, last);
}
...
static int bctrl_append_desc(struct mga2 *mga2,
					struct mga25_gem_object *mo)
{
	...
	struct dma_fence *dfence = mga2->mga25_fence[l];
	u32 status;
	u32 v = 1 << mga2->hwirq;
	/*TODO:*/
	u32 int_regs_base = mga25(mga2->dev_id) ? 0x1c00 : 0x02000;

	fence_wdesc(0, circ_inc(h) << 16, MGA2_BCTRL_TAIL, 1, 0);
	fence_wdesc(1, MGA2_INT_B_SETRST | v,
			int_regs_base + MGA2_INTREQ, 0, 1);
...
static int __mga2fb_bctrl_init(struct mga2 *mga2)
{
	...
	mga2->bctrl = dmam_alloc_coherent(mga2->drm->dev, sizeof(*mga2->bctrl),
...
int mga25_bctrl_ioctl(struct drm_device *drm, void *data, struct drm_file *file)
{
	struct mga2 *mga2 = drm->dev_private;
	struct mga25_gem_object *mo;
	int ret = 0;
