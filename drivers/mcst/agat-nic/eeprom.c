...
static struct eeprom_fixup eeprom_fixups[] __initdata = {
...
static const char *block_name[] __initdata = {
	"21140 non-MII",
	"21140 MII PHY",
	"21142 Serial PHY",
	"21142 MII PHY",
 ...
 * non-parisc hardware.
 */
static void __init tulip_build_fake_mediatable(struct tulip_private *tp)
{
#ifdef __hppa__
...
void __init tulip_parse_eeprom(struct net_device *dev)
{
	...
	if (i >= 8) {
		if (ee_data[0] == 0xff) {
			if (last_mediatable && last_ee_data) {
				controller_index++;
...
#define EE_ENB_K64SMP_WA	(0x2800 | EE_CS)
...
int __init tulip_read_eeprom(long ioaddr, int location, int addr_len,
								int k64smp_wa)
{
	int i;
	unsigned retval = 0;
	...
	int ee_enb = k64smp_wa ? EE_ENB_K64SMP_WA : EE_ENB;
