...
int tulip_refill_rx(struct net_device *dev)
{
	...
	for (; tp->cur_rx - tp->dirty_rx > 0; tp->dirty_rx++) {
		...
		if (tp->rx_buffers[entry].skb == NULL) {
			...
			mapping = pci_map_single(tp->pdev, skb_tail_pointer(skb), PKT_BUF_SZ,
						 PCI_DMA_FROMDEVICE);
	...
	while ( ! (tp->rx_ring[entry].status & cpu_to_le32(DescOwned))) {
		...
		if ((status & 0x38008300) != 0x0300) {
			...
			if (pkt_len < tulip_rx_copybreak
				...
				skb_copy_to_linear_data(skb, skb_tail_pointer(tp->rx_buffers[entry].skb),
						pkt_len);
				skb_put(skb, pkt_len);
#else
...
#ifndef BAGET_FIXME
				volatile u32 *cdesc_addr;
				cdesc_addr = 
					(u32 *)	((unsigned long) tp->tx_ring +
					inl(ioaddr + CSR1) - 
					(unsigned long) tp->tx_ring_dma);

				/* 1890vg3t rev1 give current descriptor to
				 * host on underflow, give it back to
				 * controller.
				 */
				if (!((*cdesc_addr) &
					...
					(*cdesc_addr) |=
						cpu_to_le32 (0x80000000);
					printk(KERN_NOTICE "%s: Underflow, give current descriptor back to controller\n", dev->name);
				}
#endif
