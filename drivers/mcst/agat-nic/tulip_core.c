...
/*	{ 0x10d9, 0x0531, PCI_ANY_ID, PCI_ANY_ID, 0, 0, MX98725 },*/
	...
	{ 0x7777, 0x3310, PCI_ANY_ID, PCI_ANY_ID, 0, 0, DC21143 },
	{ 0x191e, 0x3310, PCI_ANY_ID, PCI_ANY_ID, 0, 0, DC21143 },
	{ 0x191e, 0x3311, PCI_ANY_ID, PCI_ANY_ID, 0, 0, DC21143 },
	{ } /* terminate list */
};
...
static void tulip_up(struct net_device *dev)
{
	...
	/* 1890VG3T software reset bug workaround */
	outl(0, ioaddr + CSR3);
	outl(0, ioaddr + CSR4);
...
static void tulip_init_ring(struct net_device *dev)
{
	...
	for (i = 0; i < RX_RING_SIZE; i++) {
		...
		if (skb == NULL)
			break;
		mapping = pci_map_single(tp->pdev, skb_tail_pointer(skb),
...
{
	long ioaddr = dev->base_addr;
	struct tulip_private *tp = (struct tulip_private *) netdev_priv(dev);
	unsigned long flags;
...
static int private_ioctl (struct net_device *dev, struct ifreq *rq, int cmd)
{
	...
	//printk("*************IOCTL1****************\n");
	tulip_mdio_write(dev, 1, 0, 0x1000);
	tulip_mdio_write(dev, 0, 0, 0x0400);
	return 0;
	
	//case SIOCSMIIREG:		
	case SIOCDEVPRIVATE+4:	
	//printk("*************IOCTL2****************\n");
	tulip_mdio_write(dev, 0, 0, 0x1000);
	tulip_mdio_write(dev, 1, 0, 0x0400);
	return 0;
				     ...
				     const struct pci_device_id *ent)
{
	...
	*/
	...
	if (pci_get_device(PCI_VENDOR_ID_INTEL, PCI_DEVICE_ID_INTEL_82424, NULL)) {
		csr0 = MRL | MRM | (8 << BurstLenShift) | (1 << CALShift);
		force_csr0 = 1;
	}
	/* The dreaded SiS496 486 chipset. Same workaround as above. */
	if (pci_get_device(PCI_VENDOR_ID_SI, PCI_DEVICE_ID_SI_496, NULL)) {
		csr0 = MRL | MRM | (8 << BurstLenShift) | (1 << CALShift);
		force_csr0 = 1;
	}
...
//#endif
	...
	/* K64SMP V0.1 embedded ethernet has strange serial EEPROM interface */
	if (pdev->vendor == 0x191e && pdev->device == 0x3311 && chip_rev == 1)
		k64smp_wa = 1;
	...
	if (chip_idx == DC21040) {
		...
		int sa_offset = 0;
		int ee_addr_size = tulip_read_eeprom(ioaddr, 0xff, 8, k64smp_wa) & 0x40000 ? 8 : 6;

		for (i = 0; i < sizeof(ee_data)/2; i++)
			((u16 *)ee_data)[i] =
				le16_to_cpu(tulip_read_eeprom(ioaddr, i, ee_addr_size, k64smp_wa));
	...
	/* On the Zynx 315 Etherarray and other multiport boards only the
	   ...
	   that here as well. */
	if (sum == 0  || sum == 6*0xff) {
#if defined(__sparc__) && !defined(CONFIG_E90S)
...
//		}
	...
	printk(KERN_INFO "%s: %s (%04x:%04x, rev %d) at %#3lx,",
	       dev->name, tulip_tbl[chip_idx].chip_name,
	       pdev->vendor, pdev->device, chip_rev, ioaddr);
