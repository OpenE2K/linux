...
#ifdef DEBUG_REG_HISTORY
    hist_add(HIST_TRY_CMD, pSRB->pcmd->pid, 0);
#endif
    pSRB->ScsiPhase = SCSI_NOP0;
/*
if(pSRB->pcmd->cmnd[0] == 0xff ){
index_COM++;
    if(index_COM == 3){
    index_COM = 0;
    return 0;
...
//      if(pSRB->pcmd->cmnd[0] == 0xff)pDCB->Inquiry7 = 0;
//printk("pDCB->SyncMode = %x,pDCB->Inquiry7 = %x,pDCB->TargetLUN = %x\n",pDCB->SyncMode,pDCB->Inquiry7,pDCB->TargetLUN);      

	if ( (pDCB->SyncMode & WIDE_ENABLE) && (pDCB->Inquiry7 & SCSI_INQ_WBUS16)
	...
	if (pDCB->cmd_cnt++ & 0xf)
		...
		DC390_write8 (ScsiFifo, ORDERED_QUEUE_TAG);
...
{
    if( pACB->pActiveDCB && pACB->pActiveDCB->pActiveSRB->SRBState & (SRB_START_+SRB_MSGOUT) )
	DC390_write8 (ScsiCmd, CLEAR_FIFO_CMD);
}
...
udelay(10);
    ...
    //DC390_LOCK_DRV;
#ifdef DC390_DMAINT_HANDLE
	dma_stat = DC390_read8 (DMA_Status);
    ...
    {
		if (pACB->res_intr_clear)
			DC390_write8(INT_Status, istatus & (SELECTED | SEL_ATTENTION));
		dc390_TargetDisconnect (pACB);
		goto unlock;
    }
...
static int dc390_count2zero(PACB pACB, u8 sstatus)
{
	if (pACB->has_ctz) {
...
dc390_DataOut_0( PACB pACB, PSRB pSRB, PUCHAR psstatus)
{
	...
	if (dc390_count2zero(pACB, sstatus))
	{
		...
		if (pACB->mdl_used) {
			...
			pci_unmap_sg(pACB->pScsiHost->pci_dev, pSRB->pSegmentList,
										pSRB->SGcount, PCI_DMA_TODEVICE);
		} else {
			pci_unmap_single(pACB->pScsiHost->pci_dev, pSRB->SGBusAddr,
								pSRB->SGToBeXferLen, PCI_DMA_TODEVICE);
		    pSRB->SGIndex++;
	...
	else
	{
		...
		if (pACB->ctc_pci) {
			/* ctc is decremented by pci bus transfers 
			 * on AM53C974 in data out phase 
			 */
...
dc390_DataIn_0( PACB pACB, PSRB pSRB, PUCHAR psstatus)
{
	...
	if (dc390_count2zero(pACB, sstatus))
	{
		...
		if (pACB->mdl_used) {
			pSRB->SGToBeXferLen = 0;
			pci_unmap_sg(pACB->pScsiHost->pci_dev, pSRB->pSegmentList,
							pSRB->SGcount, PCI_DMA_FROMDEVICE);
		}
		else {
			pci_unmap_single(pACB->pScsiHost->pci_dev, pSRB->SGBusAddr, 
						pSRB->SGToBeXferLen, PCI_DMA_FROMDEVICE);
		    pSRB->SGIndex++;
		    if(pSRB->SGIndex < pSRB->SGcount) {
				pSRB->pSegmentList++;
				psgl = pSRB->pSegmentList;
...
din_1:
		...
		if (wide)
			residual <<= 1;
		/* ctc is decremented by pci transfers during 
		 * sync data in on AM53C974
		 */
		if (pACB->ctc_pci && pSRB->pSRBDCB->SyncOffset != 0)
			xferCnt += residual;
		pSRB->TotalXferredLen += xferCnt;
		...
		while (bcount + pSRB->pSegmentList->length <= 
					pSRB->TotalXferredLen - residual) {
			bcount += pSRB->pSegmentList->length;
			pSRB->pSegmentList++;
			pSRB->SGIndex++;
		}

		/* read residual bytes from fifo */
		if (residual) {
			ptr = pSRB->pSegmentList->address + 
						pSRB->TotalXferredLen - bcount - residual;
		   	while(residual-- > 0)
			{
				if (wide)
					fifo2 = DC390_read8(ScsiFifo2);
				*ptr++ = DC390_read8(ScsiFifo);
				if (ptr == (unsigned char *) 
						pSRB->pSegmentList->address 
						+ pSRB->pSegmentList->length) {
					bcount += pSRB->pSegmentList->length;
					pSRB->pSegmentList++;
					pSRB->SGIndex++;
					ptr = pSRB->pSegmentList->address;
...
dc390_MsgIn_QTag (PACB pACB, PDCB pDCB, UCHAR tag)
{
  ...
  if (pSRB)
	...
	if (pSRB->TotalXferredLen != pSRB->Saved_Ptr && pSRB != pACB->pTmpSRB)
		dc390_restore_ptr(pACB, pSRB);
  return pSRB;
}

static void dc390_MsgIn_wdtr(PACB pACB, PSRB pSRB)
{
...
dc390_restore_ptr (PACB pACB, PSRB pSRB)
{
	...
	if(pSRB->pcmd->use_sg) {
		...
		DEBUG0(printk(KERN_INFO "DC390: Pointer restored. Segment %i, "
				"Total %li \n", pSRB->SGIndex, pSRB->Saved_Ptr);)
	} else if(pSRB->pcmd->request_buffer) {
		pSRB->SGcount = 1;
		pSRB->pSegmentList = (PSGL) &pSRB->Segmentx;
		pSRB->pSegmentList->address = pSRB->pcmd->request_buffer;
		pSRB->pSegmentList->length = pSRB->pcmd->request_bufflen;
		pSRB->SGToBeXferLen = pSRB->pSegmentList->length - pSRB->Saved_Ptr;
		DEBUG0(printk (KERN_INFO "DC390: Pointer restored. Total %li \n",
			pSRB->Saved_Ptr);)
    } else {
		pSRB->SGcount = 0;
		DEBUG0(printk(KERN_INFO "DC390: RESTORE_PTR message for io process "
				"without datain/dataout ??\n");)
	}
...
dc390_DataIO_Comm( PACB pACB, PSRB pSRB, UCHAR ioDir)
{
	...
	if ((pACB->Gmode2 & USE_MDL) && (pSRB->SGcount > 1) 
						&& (pSRB->TotalXferredLen == 0)) {
		int nents;
		nents = pci_map_sg(pACB->pScsiHost->pci_dev, pSRB->pSegmentList,
								pSRB->SGcount, dma_dir);
		if (dc390_fill_mdt(pACB->mdt_cpu, nents, pSRB->pSegmentList, 
													pACB->mdl_page_only)) {
			pci_unmap_sg(pACB->pScsiHost->pci_dev, pSRB->pSegmentList,
                                pSRB->SGcount, dma_dir);
			goto nomdl;
		}
		pSRB->SGToBeXferLen = pSRB->pcmd->request_bufflen;
		if (pDCB->SyncMode & WIDE_NEGO_DONE && pDCB->SyncMode & WIDE_ENABLE)
			lval = (pSRB->SGToBeXferLen >> 1) + (pSRB->SGToBeXferLen & 1);
		else
			lval = pSRB->SGToBeXferLen;
		pSRB->SRBState = SRB_DATA_XFER;
		pSRB->SGBusAddr = pSRB->pSegmentList->dvma_address & (DC390_PAGE_SIZE - 1);
		pACB->mdl_used = 1;
		...
		DC390_write8 (DMA_Cmd, DMA_START_CMD | ioDir | DMA_INT | MAP_TO_MDL);
		return;
	}
nomdl:
	pACB->mdl_used = 0;
	...
	/* pSegmentList alwais points to current memory block here.
	 * SGToBeXferLen can be equal to zero (but only on first transfer),
	 * in this case we must map and tranfer first memory block.
	 * If SGToBeXferLen is not zero, we must map and transfer only 
	 * last SGToBeXferLen bytes of current memory block.
	 */ 
	    psgl = pSRB->pSegmentList;
		if (pSRB->SGToBeXferLen != 0) {
			pSRB->SGBusAddr = pci_map_single(pACB->pScsiHost->pci_dev,
						psgl->address + psgl->length - pSRB->SGToBeXferLen, 
						pSRB->SGToBeXferLen, dma_dir);
		} else {
	    	pSRB->SGToBeXferLen = (ULONG) psgl->length;
			pSRB->SGBusAddr = pci_map_single(pACB->pScsiHost->pci_dev,
								psgl->address, (ULONG) psgl->length, dma_dir);
		}
...
void dc390_TargetDisconnect(PACB pACB)
{
  PDCB pDCB;
  PSRB pSRB;
  ...
  DC390_write8 (ScsiCmd, DISCONNECT_CMD);

  /* Restore reselect ability */
  DC390_write8 (ScsiCmd, EN_SEL_RESEL);

  printk (KERN_INFO "DC390: Target mode not supported!\n");

  pDCB = pACB->pActiveDCB;
  /* Check if last cmd failed to start */
  if (pDCB)
    {
      /* Move not started cmd to waiting queue */ 
      pSRB = pDCB->pActiveSRB;
      pSRB->SRBState = SRB_READY;
      dc390_freetag (pDCB, pSRB);
      dc390_Going_to_Waiting (pDCB, pSRB);
...
dc390_Reselect( PACB pACB )
{
    ...
    {	/* Arbitration lost but Reselection won */
		...
		pSRB = pDCB->pActiveSRB;
			/* FIXME: it seems that this check is unneded. 
			 * Currently scan_devices flag is wrong, and this check leads to errors 
			 */ 
		if( 1 /*!( pACB->scan_devices )*/ )
		{
		    pSRB->SRBState = SRB_READY;
    ...
    /* Get LUN */
    lun = DC390_read8 (ScsiFifo);
    if (!(lun & IDENTIFY_BASE)) 
		printk (KERN_ERR "DC390: Resel: Expect identify message! but get %02x \n", lun);
    ...
    {
	pSRB = pDCB->pActiveSRB;
	if (pSRB->TotalXferredLen != pSRB->Saved_Ptr)
		dc390_restore_ptr(pACB, pSRB);
...
ckc_e:
	...
	if ((ptr->DevType & SCSI_DEVTYPE) == TYPE_NODEV && !DCB_removed)
	...
	else
	  {
		...
		/* FIXME: It is wrong condition of scan end, we can finish even after fist lun  */
	if( (pcmd->target == pACB->pScsiHost->max_id - 1) &&
	    (pcmd->lun == pACB->pScsiHost->max_lun - 1) )
	  pACB->scan_devices = 0;
     };
...
static int __inline__ dc390_fill_mdt(u32 *mdt, int nents, struct scatterlist *sg, int mdl_page_only)
{
	/* check first entry if it has its own rule */
	if (!mdl_page_only) {
		if (sg->length > DC390_PAGE_SIZE || (sg->dvma_address + sg->length) 
													& (DC390_PAGE_SIZE - 1)) {
			return 1;
		}
		else {
			*mdt++ = cpu_to_le32(sg->dvma_address);
			sg++;
			nents--;
		}
	}
	...
	while (--nents > 0)
	{
		if (!(sg->dvma_address & (DC390_PAGE_SIZE - 1)) && 
	...
	/* check last entry */	
	if (sg->length > DC390_PAGE_SIZE || sg->dvma_address 
									& (DC390_PAGE_SIZE - 1))
		return 1;
	else
		*mdt = cpu_to_le32(sg->dvma_address);
