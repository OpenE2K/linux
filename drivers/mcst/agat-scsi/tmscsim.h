...
/* We have one mdt for all io process.
 * We have to rewrite it before each datain/dataout phase.
 * It is not optimal if drive likes to disconnect
...
/*;---ACBFeature */
#define WIDE_SCSI_EN		BIT0	/* Wide scsi bus support */
#define HAS_CTZ			BIT1	/* CTZ flag in status reg works correctly */
#define RES_INTR_CLEAR		BIT2	/* Some interrupts are cleared by writing to interrupt*/
					/* status register */
#define MDL			BIT3	/* Controller support mdl */
#define MDL_PAGE_ONLY		BIT4	/* First mdl entry has to be whole page */
#define CTC_PCI			BIT5	/* Ctc decremented by pci bus transfers (in some modes) */
#define TIMEOUT_REG_SWITCH	BIT7	/* Timeout register /interrupt status register writes routing*/ 
#define NO_RESEL		BIT8		/* Reselections support */
#define MDL_USED		BIT9
#define DMA_DONE		BIT10
...
/* FeatureReg (VG9T rev 1.1 specific) */
#define DC390_REG14_TIMEOUT	BIT12
...
#define FeatureReg	0x78	/* rw   VG9T rev 1.1 specific */
...
#define DC390_read8(address)			\
	(inb((unsigned long)pACB->IOPortBase + (address)))

#define DC390_read8_(address, base)		\
	(inb((base) + (address)))

#define DC390_read16(address)			\
	(inw((unsigned long)pACB->IOPortBase + (address)))

#define DC390_read32(address)			\
	(inl((unsigned long)pACB->IOPortBase + (address)))

#if 1
#define DC390_write8(address,value)		\
	outb((value), (unsigned long)pACB->IOPortBase + (address))


#define DC390_write16(address,value)		\
	outw((value), (unsigned long)pACB->IOPortBase + (address))

#define DC390_write32(address,value)		\
	outl((value), (unsigned long)pACB->IOPortBase + (address))
#else
#if 1
#define DC390_write8(address,value)		\
	do{							\
	printk(KERN_DEBUG":%s:%s\t\t%s:%d\n",#address, #value, __FUNCTION__, __LINE__);		\
