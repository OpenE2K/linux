...
#define DC390_PAGE_SIZE	(1UL << 12)
#define DC390_DMAINT_HANDLE
#define DC390_DEVICE_RESET
...
dc390_StartSCSI( struct dc390_acb* pACB, struct dc390_dcb* pDCB, struct dc390_srb* pSRB )
{
    ...
    u8 cmd, disc_allowed, try_sync_nego = 0, try_wide_nego = 0;
    char tag[2];
	...
	disc_allowed = 0;
    if ( (pDCB->SyncMode & WIDE_ENABLE)
		&& (!(pDCB->SyncMode & WIDE_NEGO_DONE)) && (pDCB->TargetLUN == 0) ){
		try_wide_nego = 1;
		}
    else
...
//no_tag:
	...
	cmd = SEL_W_ATN_STOP;
      ...
      if (try_wide_nego) {
		DEBUG0(printk (KERN_INFO "DC390: Wide Nego code triggered (%i %i)\n", pDCB->TargetID, pDCB->TargetLUN);)
		pSRB->MsgOutBuf[0] = EXTENDED_MESSAGE;
		pSRB->MsgOutBuf[1] = 2;
		pSRB->MsgOutBuf[2] = EXTENDED_WDTR;
		pSRB->MsgOutBuf[3] = 1;
		pSRB->MsgCnt = 4;
...
DC390_Interrupt(void *dev_id)
{
 ...
 //   sstatus = DC390_read8 (Scsi_Status);
#ifdef DC390_DMAINT_HANDLE
	u8 dma_stat = DC390_read8 (DMA_Status);
	dc390_laststatus &= ~0xff000000; 
	dc390_laststatus |= dma_stat << 24;
	if (dma_stat & (DMA_XFER_ABORT | DMA_XFER_ERROR)) {
		panic("DC390: dma error");
	}
	if (dma_stat & DMA_XFER_DONE) {
		if (pACB->pActiveDCB)
			pACB->ACBFeature |= DMA_DONE;
		else
			printk(KERN_WARNING "DC390: dma_done without dma transfer \n");
	}
		
	if (!(dma_stat & SCSI_INTERRUPT))
		return IRQ_NONE;
    sstatus = DC390_read8 (Scsi_Status);
#else
    ...
    {
	/* Clear reselect, sel, sela and scsi_reset interrupts */
	if (pACB->ACBFeature & RES_INTR_CLEAR)
		DC390_write8(INT_Status, istatus & 
			(SCSI_RESET | RESELECTED | SELECTED | SEL_ATTENTION));
    ...
    {
	if (pACB->ACBFeature & RES_INTR_CLEAR)
		DC390_write8(INT_Status, RESELECTED);
	dc390_Reselect( pACB );
	goto unlock;
    }
    ...
    {
	if (pACB->ACBFeature & RES_INTR_CLEAR)
		DC390_write8(INT_Status, SELECTED | SEL_ATTENTION);
...
static int dc390_count2zero(struct dc390_acb* pACB, u8 sstatus)
{
	if (pACB->ACBFeature & HAS_CTZ) {
		if (!(sstatus & COUNT_2_ZERO))
			return 0;

		if (pACB->ACBFeature & CTC_PCI)
			return !(DC390_read8 (Current_Fifo) & 0x1f);
		
		return 1;
	} else {
		return !(DC390_read8(CtcReg_Low) | 
...
dc390_DataOut_0(struct dc390_acb* pACB, struct dc390_srb* pSRB, u8 *psstatus)
{
	...
	if( dc390_count2zero(pACB, sstatus) )
	{
	    ...
	    if(pACB->ACBFeature & MDL_USED)
	    	pSRB->SGIndex = pSRB->SGcount;
	    else
	    	pSRB->SGIndex++;
	...
	{
	    ResidCnt = (pACB->ACBFeature & CTC_PCI ? 
			    ((u32) DC390_read8 (Current_Fifo) & 0x1f):0) +
		    (((u32) DC390_read8 (CtcReg_High) << 16) |
		     ((u32) DC390_read8 (CtcReg_Mid) << 8) |
		     (u32) DC390_read8 (CtcReg_Low));

	    if (pSRB->pSRBDCB->SyncMode & WIDE_NEGO_DONE && 
			pSRB->pSRBDCB->SyncMode & WIDE_ENABLE)
		ResidCnt <<= 1;
	    ...
	    while (bcount + pSRB->pSegmentList->length 
			  <= pSRB->TotalXferredLen) {
		    bcount += pSRB->pSegmentList->length;
		    pSRB->pSegmentList++;
		    pSRB->SGIndex++;
			  }
			  pSRB->SGToBeXferLen = pSRB->pSegmentList->length 
					  - (pSRB->TotalXferredLen - bcount);
...
dc390_DataIn_0(struct dc390_acb* pACB, struct dc390_srb* pSRB, u8 *psstatus)
{
	...
	else	/* phase changed */
	{
	    ...
	    if (!j)
		printk(KERN_INFO "DC390: fifo contains residuals after phase change \n");
din_1:
	    if (pACB->ACBFeature & MDL_USED)
	    	DC390_write8 (DMA_Cmd, READ_DIRECTION | DMA_BLAST_CMD | MAP_TO_MDL);
	    else
...
dc390_MsgIn_set_async (struct dc390_acb* pACB, struct dc390_srb* pSRB)
{
  ...
  pDCB->CtrlR3 = (pDCB->CtrlR3 & WIDE_SCSI_ON) | FAST_CLK;	/* fast clock / normal scsi */
  pDCB->CtrlR4 &= 0x3f;
...
static void dc390_MsgIn_wdtr(struct dc390_acb* pACB, struct dc390_srb* pSRB)
{
	struct dc390_dcb* pDCB = pSRB->pSRBDCB;
	int wide_exp;
	
	if (pSRB->MsgInBuf[0] == MESSAGE_REJECT) {
		pSRB->SRBState &= ~DO_WIDE_NEGO;
		pDCB->SyncMode &= ~(WIDE_ENABLE+WIDE_NEGO_DONE);
		return;
	}

	/* Wide data transfer negotiation reset sync data transfer negotiation */
	...
	if (!(pSRB->SRBState & DO_WIDE_NEGO)) {
		...
		if (!(pDCB->SyncMode & WIDE_ENABLE))
			wide_exp = 0;
		pSRB->MsgInBuf[3] = wide_exp;
      	memcpy (pSRB->MsgOutBuf, pSRB->MsgInBuf, 4);
      	pSRB->MsgCnt = 4;
      	DC390_ENABLE_MSGOUT;
		printk("target %d: %s data transfer negotiation "
                   "(initiated by target)\n", pDCB->TargetID,
                wide_exp ? "Wide" : "8-bit");
		goto wide_set;
	}

	printk(KERN_INFO "DC390: Target %d: %s data transfer negotiation\n",
               pDCB->TargetID, wide_exp ? "Wide" : "8-bit");

wide_set:
	pDCB->SyncMode |= WIDE_NEGO_DONE;
	if (wide_exp) 
		pDCB->CtrlR3 |= WIDE_SCSI_ON;	
	else
		pDCB->SyncMode &= ~WIDE_ENABLE;
	DC390_write8 (CtrlReg3, pDCB->CtrlR3);
	dc390_SetXferRate (pACB, pDCB); 
	return;
}
...
dc390_MsgIn_0( struct dc390_acb* pACB, struct dc390_srb* pSRB, u8 *psstatus)
{
	...
	switch (pSRB->MsgInBuf[0]) 
	  ...
	  case MESSAGE_REJECT: 
	    ...
	    if (pSRB->SRBState & DO_WIDE_NEGO)
			dc390_MsgIn_wdtr(pACB, pSRB);
	    break;
	    
	  case EXTENDED_MESSAGE:
	    switch (pSRB->MsgInBuf[2])
	      {
		case EXTENDED_SDTR:
...
static int __inline__ dc390_fill_mdt(u32 *mdt, int nents,
				      struct scatterlist *sg, int mdl_page_only)
{
	/* check first entry if it has its own rule */
	if (!mdl_page_only) {
		if (sg->length > DC390_PAGE_SIZE || (sg_dma_address(sg) + sg->length) 
				& (DC390_PAGE_SIZE - 1)) {
			return 1;
		}
		else {
			*mdt++ = cpu_to_le32(sg_dma_address(sg));
			sg++;
			nents--;
		}
	}
	...
	while (--nents > 0) {
		if (!(sg_dma_address(sg) & (DC390_PAGE_SIZE - 1)) && 
							sg->length == DC390_PAGE_SIZE) {
			*mdt++ = cpu_to_le32(sg_dma_address(sg));
			sg++;
		}
		else {
	...
	/* check last entry */	
	if (sg->length > DC390_PAGE_SIZE || sg_dma_address(sg) 
					& (DC390_PAGE_SIZE - 1))
		return 1;
	else
		*mdt = cpu_to_le32(sg_dma_address(sg));
...
dc390_DataIO_Comm( struct dc390_acb* pACB, struct dc390_srb* pSRB, u8 ioDir)
{
    ...
    pACB->ACBFeature &= ~DMA_DONE;
#endif
    if ((pACB->Gmode2 & USE_MDL) && (pSRB->SGcount > 1) 
		&& (pSRB->TotalXferredLen == 0)
	  	&& !dc390_fill_mdt(pACB->mdt_cpu, pSRB->SGcount, pSRB->pSegmentList, 
					pACB->ACBFeature & MDL_PAGE_ONLY))
    {
		int i;
		struct scatterlist *s;
		pSRB->SGToBeXferLen = 0;
		for_each_sg(pSRB->pSegmentList, s, pSRB->SGcount, i)
			pSRB->SGToBeXferLen += sg_dma_len(s);

		if (pDCB->SyncMode & WIDE_NEGO_DONE && pDCB->SyncMode & WIDE_ENABLE)
			lval = (pSRB->SGToBeXferLen >> 1) + (pSRB->SGToBeXferLen & 1);
		else
			lval = pSRB->SGToBeXferLen;
		pSRB->SRBState = SRB_DATA_XFER;
		pSRB->SGBusAddr = sg_dma_address(pSRB->pSegmentList)
				 			& (DC390_PAGE_SIZE - 1);
		pACB->ACBFeature |= MDL_USED;
		/* TODO: merge this with nonmdl adapter programming code */
		...
		DC390_write8 (DMA_Cmd, DMA_IDLE_CMD | ioDir | MAP_TO_MDL);
		DC390_write32(DMA_MDL_Addr, pACB->mdt_dma);
		DC390_write32(DMA_XferCnt, pSRB->SGToBeXferLen);	
		DC390_write32 (DMA_XferAddr, pSRB->SGBusAddr);
		DC390_write8 (ScsiCmd, DMA_COMMAND | INFO_XFER_CMD);
		DC390_write8 (DMA_Cmd, DMA_START_CMD | ioDir | MAP_TO_MDL);
		return;
    }
    pACB->ACBFeature &= ~MDL_USED;
    if( pSRB->SGIndex < pSRB->SGcount )
...
dc390_Disconnect( struct dc390_acb* pACB )
{
    ...
    if (!pDCB)
     {
	DEBUG0(printk(KERN_ERR "ACB:%p->ActiveDCB:%p IOPort:%p IRQ:%02x !\n",\
	       pACB, pDCB, pACB->IOPortBase, pACB->IRQLevel));
	mdelay(400);
	DC390_read8 (INT_Status);	/* Reset Pending INT */
	if (!(pACB->ACBFeature & NO_RESEL))
...
/* Route writes to 0x14 reg between interrupt status reg and scsi timeout reg
 * on VG9T rev 1.1.
 */
#define DC390_REG14_TOUT 1
#define DC390_REG14_INTS 0
static void dc390_reg14_route(struct dc390_acb* pACB, int route)
{
	u32 val;

	if (!(pACB->ACBFeature & TIMEOUT_REG_SWITCH))
		return;

	val = DC390_read32(FeatureReg);
	if (route == DC390_REG14_TOUT) {
		DC390_write32(FeatureReg, val | DC390_REG14_TIMEOUT);
	} else {
		DC390_write32(FeatureReg, val & ~DC390_REG14_TIMEOUT);
	}
}
...
dc390_ResetSCSIBus( struct dc390_acb* pACB )
{
    ...
    DC390_write8 (ScsiCmd, CLEAR_FIFO_CMD);
    /* route writes to Scsi_TimeOut reg to interrupt status register */
    dc390_reg14_route(pACB, DC390_REG14_INTS);
    DC390_write8 (DMA_Cmd, DMA_IDLE_CMD);
...
static int dc390_slave_alloc(struct scsi_device *scsi_device)
{
	...
	if (pDCB->DevMode & WIDE_NEGO_)
		...
		pDCB->SyncMode &= ~(WIDE_NEGO_DONE | WIDE_ENABLE);
...
#if 0
static void  dc390_sendstore_done (struct scsi_cmnd* cmd)
{
//printk("*************** SENDSTORE_DONE ********************\n ");
	printk (KERN_INFO "DC390: SENDSTORE (ID %02x LUN %02x) returned %08x\n",
		cmd->device->id, cmd->device->lun, cmd->result);
	kfree (cmd);
};
#endif
static void dc390_sendstore (struct scsi_device *sdev, struct dc390_acb* pACB, struct dc390_dcb* pDCB)
{
#if 0
	char* buffer;
	struct scsi_cmnd* cmd;
	cmd = kmalloc (sizeof(struct scsi_cmnd) + 256, GFP_ATOMIC);
   //if (!cmd) { printk ("DC390: kmalloc failed in sendstart!\n"); return; };
	buffer = (char*)cmd + sizeof(struct scsi_cmnd);
//printk("SEND*************ID = %d LUN = %d\n",pDCB->TargetID,pDCB->TargetLUN);

	memset (cmd, 0, sizeof(struct scsi_cmnd) + 256);
	cmd->cmnd[0] = 0x1f; /* START_STOP_UNIT */
	cmd->cmnd[1] = (pDCB->TargetLUN << 5) & 0xe0;
	cmd->cmnd[4] = 0x00; /* START */
	...
	cmd->done = dc390_sendstore_done;
	cmd->scsi_done = dc390_sendstore_done;
   ...
   printk (KERN_INFO "DC390: Queue SEND_START command to dev ID %02x LUN %02x\n",
	   pDCB->TargetID, pDCB->TargetLUN);
//printk("COMAND = %x\n",cmd->cmnd[0]);
	DC390_queuecommand (cmd, dc390_sendstore_done);
#endif
};

static int DC390_ioctl(struct scsi_device *dev, int cmd_ioctl,void *arg)
{
	struct Scsi_Host *host = 0;
	int i;
	struct dc390_acb *pACB;
	struct dc390_dcb *pDCB;

	host = dev->host;
	pACB = (struct dc390_acb *)(host->hostdata);
	pDCB = pACB->pLinkDCB;
	pDCB->TargetLUN = dev->lun;
	pDCB->TargetID = dev->id;

	switch(cmd_ioctl){
		case IOCTL_STORE:
			dc390_sendstore (dev, pACB, pDCB);
			for(i=0;i<4;i++)
				mdelay(1000000);
			break;
		default:
...
struct scsi_cntr {
	const char *name;
	unsigned short vendor, device, rev;
	/* Reselections support */
	int disc_en;
	/* Wide scsi bus support */
	int wide_scsi_en;
	/* CTZ flag in status reg works correctly */
	int has_ctz;
	/* Some interrupts are cleared by writing to interrupt
	 * status register */
	int res_intr_clear;
	/* Controller support mdl */
	int mdl;
	/* First mdl entry has to be whole page */
	int mdl_page_only;
	/* Ctc decremented by pci bus transfers (in some modes) */
	int ctc_pci;
	/* Timeout register /interrupt status register
	 * writes routing
	 */ 
	int timeout_reg_switch;
};

static struct scsi_cntr dev_list[] = {
	{
		name:	"1890VG9T rev 1",
		vendor: 0x7777,
...
/* Disable features, unsupported by this host controller */
static void dc390_disable_unsupported(struct pci_dev *pdev)
{
	u8 rev, i;
	struct scsi_cntr *b;
	pci_read_config_byte(pdev, PCI_REVISION_ID, &rev);
	for(i=0, b = NULL; i < ARRAY_SIZE(dev_list); i++, b = NULL){
		b = dev_list + i;
		if (pdev->vendor == b->vendor && pdev->device == b->device
				  && b->rev == rev)
			break;
	}
	if (!b)
		return;
	if (b->wide_scsi_en)
		tmscsim[2] |= WIDE_NEGO_;
	if (!b->disc_en)
		tmscsim[2] &= ~(EN_DISCONNECT_ | TAG_QUEUEING_);
	if (b->mdl)
		tmscsim[3] |= USE_MDL;
}
...
static void dc390_check_eeprom(struct pci_dev *pdev, u8 index)
{
	...
	if (wval != 0x1234) {
		...
		dc390_disable_unsupported(pdev);
...
static void dc390_init_hw(struct dc390_acb *pACB, u8 index)
{
	...
	dc390_reg14_route(pACB, DC390_REG14_TOUT);
...
static void dc390_init_features(struct pci_dev *pdev, 
						struct dc390_acb *pACB)
{
	...
	if (!b->disc_en)
		pACB->ACBFeature |= NO_RESEL;
	if (b->has_ctz)
		pACB->ACBFeature |= HAS_CTZ;
	if (b->res_intr_clear)
		pACB->ACBFeature |= RES_INTR_CLEAR;
	if (b->mdl_page_only)
		pACB->ACBFeature |= MDL_PAGE_ONLY;
	if (b->ctc_pci)
		pACB->ACBFeature |= CTC_PCI;
	if (b->timeout_reg_switch)
		pACB->ACBFeature |= TIMEOUT_REG_SWITCH;
	if (pACB->Gmode2 & USE_MDL)
		pACB->mdt_cpu = pci_alloc_consistent(pdev, 
						PAGE_SIZE, &pACB->mdt_dma);
				    ...
				    const struct pci_device_id *id)
{
	...
	if (!request_region(pci_resource_start(pdev, 0), pci_resource_len(pdev, 0), "tmscsim")) {
		printk(KERN_ERR "DC390: register IO ports error!\n");
		goto out_host_put;
	}
	if (!(io_port = pci_iomap(pdev, 0, 0))) {
		printk(KERN_ERR "DC390: map IO ports error!\n");
		goto out_release_region;
	};

	shost->this_id = dc390_eepromBuf[dc390_adapterCnt][EE_ADAPT_SCSI_ID];
	shost->irq = pdev->irq;
	shost->unique_id = (unsigned long)io_port;
	...
	pACB->pdev = pdev;
	dc390_init_features(pdev, pACB);

	/* Reset Pending INT */
	DC390_read8_(INT_Status, (unsigned long)io_port);
...
static void dc390_remove_one(struct pci_dev *dev)
{
	...
	pci_iounmap(dev, pACB->IOPortBase);
	release_region(pci_resource_start(dev, 0), pci_resource_len(dev, 0));
...
static struct pci_device_id tmscsim_pci_tbl[] = {
	{ 0x7777, PCI_DEVICE_ID_AMD_SCSI,
		PCI_ANY_ID, PCI_ANY_ID, 0, 0, 0 },
	{ 0x191e, PCI_DEVICE_ID_AMD_SCSI,
		PCI_ANY_ID, PCI_ANY_ID, 0, 0, 0 },
	{ 0x191e, PCI_DEVICE_ID_AMD_SCSI,
		PCI_ANY_ID, PCI_ANY_ID, 0, 0, 0 },
	{ }
