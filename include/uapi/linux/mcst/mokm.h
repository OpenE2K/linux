...
typedef struct {
    ...
    __u64       rx_size_all;        // Received bytes
    ...
    __u64       tx_size_all;        // Transmitted bytes
...
#define MOKM_ISR_REMOTE_REQ     (1 << 17)   // ERROR Transmitter resp. wait timeout - Запрос на передачу данных от удалённого модуля. Необходимо инициализировать приёмник
...
// Запрос на передачу данных h05 ???
#ifdef __sparc__ // ARCH: e90, e90s
  #if defined(VER_2614)
    #define MOKM_CR_ENDIAN    (0x00ffff00)  // кручение для регистров
    #define MOKM_TRC_ENDIAN   (1<<11)       // кручение для DMA в память
  #else
    #define MOKM_CR_ENDIAN    (0)           // не крутить, обращение через iowrite32
    #define MOKM_TRC_ENDIAN   (1<<11)       // кручение для DMA в память
  #endif // VER_2614
#else // ARCH: e2k
    #define MOKM_CR_ENDIAN    (0)           // не крутить, обращение через iowrite32
    #define MOKM_TRC_ENDIAN   (0)           // прямое DMA в память
#endif // __sparc__
...
typedef struct  trc_reg {
    ...
    u32     dbg_point   : 1; // 24    R/W enable "TET" - установлен запрос на передачу данных ???
...
/*13:0x30*/  trc_r_t    TRC;    /* R/W PC8374 Ctrl Reg      | Base + 0x0030 | defailt 0х000001b8*/
 ...
 * negative on failure:
 *  -EBUSY - Pear still not ready (TRC.b.pearrd_rdy == 0)
